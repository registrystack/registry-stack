...
"""Run mapped OpenID Foundation conformance-suite slices for Registry Lab."""
...
CONFIG_DIR = ROOT / "config" / "openid-conformance"
PLAN_MAP_PATH = CONFIG_DIR / "plan-map.json"
COMPOSE_OVERRIDE_PATH = CONFIG_DIR / "docker-compose.override.yaml"
DEFAULT_CACHE_DIR = ROOT / ".cache" / "openid-conformance"
DEFAULT_OUTPUT_ROOT = ROOT / "output" / "openid-conformance"
SCHEMA_VERSION = "registry.lab.openid_conformance_plan_map.v1"
...
def default_params(scenario: dict[str, Any], args: argparse.Namespace) -> dict[str, str]:
    ...
    issuer_url = (
        args.issuer_url
        or os.environ.get(defaults.get("issuer_url_env", ""))
        or os.environ.get("CITIZEN_OID4VCI_CREDENTIAL_ISSUER")
    )
    if not issuer_url:
        raise RunnerError(
            "issuer URL is required; pass --issuer-url or set "
            f"{defaults.get('issuer_url_env', 'REGISTRY_LAB_OPENID_CONFORMANCE_ISSUER_URL')}"
        )
    authorization_server = (
        args.authorization_server
        or os.environ.get(defaults.get("authorization_server_env", ""))
        or os.environ.get("CITIZEN_OID4VCI_AUTHORIZATION_SERVER")
        or issuer_url
    )
    credential_configuration_id = (
        args.credential_configuration_id
        or os.environ.get(defaults.get("credential_configuration_id_env", ""))
        or os.environ.get("CITIZEN_OID4VCI_CREDENTIAL_CONFIGURATION_ID")
...
def write_rendered_config(scenario: dict[str, Any], output_dir: Path, params: dict[str, str]) -> Path:
...
def ensure_suite_checkout(plan_map: dict[str, Any], args: argparse.Namespace) -> Path:
    ...
    if not git:
        raise RunnerError("git is required to prepare the OpenID conformance suite checkout")
    if checkout.exists():
...
def compose_command(checkout: Path, args: argparse.Namespace, *compose_args: str) -> list[str]:
...
def builder_command(checkout: Path, *compose_args: str) -> list[str]:
    return ["docker", "compose", "-f", str(checkout / "builder-compose.yml"), *compose_args]
...
def ensure_suite_artifact(checkout: Path, args: argparse.Namespace) -> Path:
    ...
    if not docker:
        raise RunnerError("docker is required to build the OpenID conformance suite artifact")
    ...
    run_checked(builder_command(checkout, "run", "--rm", "builder"), cwd=checkout, env=env)
    if not jar.exists():
...
def ensure_suite_python(checkout: Path, args: argparse.Namespace) -> Path:
    ...
    venv_dir = python.parents[1] if os.name == "nt" else python.parents[1]
...
def cmd_up(args: argparse.Namespace) -> int:
    ...
    wait_for_suite(suite_settings(plan_map, args)["base_url"], args.wait_seconds)
    print(suite_settings(plan_map, args)["base_url"])
    return 0
...
def cmd_render_config(args: argparse.Namespace) -> int:
    ...
    output_dir = output_dir_for(args, scenario["id"])
    config_path = write_rendered_config(scenario, output_dir, default_params(scenario, args))
    print(config_path)
    return 0
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if scenario.get("status") != "applicable" and not args.allow_blocked:
        raise RunnerError(
            f"scenario {scenario['id']} is {scenario.get('status')}; pass --allow-blocked to run it anyway"
        )
    ...
    result = subprocess.run(command, cwd=suite_dir(args), env=env, text=True, check=False)
    if result.returncode != 0:
        raise RunnerError(f"OpenID conformance run failed with status {result.returncode}; output: {output_dir}")
    print(output_dir)
    return 0
...
def add_config_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--client-id", default="registry-lab-openid-conformance-client")
    parser.add_argument("--client2-id", default="registry-lab-openid-conformance-client-2")
