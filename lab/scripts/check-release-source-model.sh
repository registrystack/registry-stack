...
mode="${1:-${REGISTRY_LAB_RELEASE_SOURCE_MODE:-monorepo}}"
...
print_gitlink_ref() {
	...
	local rel_path="$2"
	local object
	object="$(gitlink_head "${name}" "${rel_path}")"
	printf 'release-source external %s %s gitlink=%s\n' "${name}" "${rel_path}" "${object}"
}
...
monorepo)
	...
	require_path "registry-lab" "${stack_root}/lab/scripts/check-release-source-model.sh"
	...
	print_gitlink_ref "crosswalk" "vendor/crosswalk"
	print_gitlink_ref "registry-atlas" "vendor/registry-atlas"
	print_gitlink_ref "esignet-relay-authenticator" "vendor/esignet-relay-authenticator"
	;;
*)
	echo "usage: REGISTRY_LAB_RELEASE_SOURCE_MODE=vendor|source|monorepo scripts/check-release-source-model.sh [vendor|source|monorepo]" >&2
	exit 2
	;;
esac
