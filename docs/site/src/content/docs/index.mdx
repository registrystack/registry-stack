...
records from an existing source, and the Base Registry Engine source preview provides a
...
Use Base Registry Engine when you need a system of record, including create and update operations.
...
models, not built-in BReg types. Base Registry Engine is currently documented as an unreleased
source preview.

Start with the [Base Registry Engine overview](explanation/configuration-defined-registry/), then
[create and query your first registry](tutorials/first-breg/). Continue with
[configure your registry](configure/breg/) to extend the model and prepare a tested
deployment handoff.
...
Follow [bind webhook receivers](operate/breg/#bind-webhook-receivers) to notify another
service.
Use the [configuration reference](reference/breg-configuration/) to look up keys,
defaults, and constraints.
Use the [API reference](reference/breg-api/#history-reads) for saved snapshots,
effective-time queries, and correction context, and the
[events and webhooks section](reference/breg-api/#events-and-webhooks) for the receiver
contract, signatures, and delivery behavior. Use the
[BReg client API reference](reference/breg-client-api/) for the Rust client surface. For
retained-history erasure, see
[erase retained history](operate/breg/#erase-retained-history).

{/* Evidence: crates/registry-breg/src/contract.rs, RegistryProject and RegistryModule;
    crates/registry-breg/tests/postgres_pilot_acceptance.rs;
