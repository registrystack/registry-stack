...
| A signed minimum-disclosure answer about one subject | Evidence | A signed assertion carrying the answer, not the source record |
| A bounded answer or status | Registry Notary | A claim result without the source record |

Registry Relay and Registry Notary can work together: Registry Relay obtains a
limited source result, and Registry Notary evaluates a reviewed claim over
that result.
