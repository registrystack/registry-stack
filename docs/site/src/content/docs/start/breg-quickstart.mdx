...
Base Registry Engine is a PostgreSQL system of record whose data model and REST API are compiled
from a registry project. The runtime carries no built-in domain types: a business, a plot, a
...
relationships, access profiles, change requests, and events one registry exposes. A package is
...
compiled routes, and appends every admitted request to a hash-chained audit journal in the same
database. The process keeps no state of its own outside PostgreSQL.
...
Six more terms recur across the section. Each names something the product has, not a metaphor.
...
{/* Evidence: crates/registry-breg/src/package.rs, PackageTrustAnchor and TrustAnchorKey;
...
token. You need Linux on amd64 or arm64 or macOS on Apple Silicon, Git and Bash, Docker, OpenSSL,
Python 3.11 or later with uv, and curl 7.76 or later.
...
[Base Registry Engine client API reference](../../reference/breg-client-api/) open for the full
surface. If you call the REST API directly, the
