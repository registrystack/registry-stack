...
| Sign-in refuses the documented demo identity (`relay_auth_subject_denied`) | The hosted deployment's identity set has drifted from these instructions. | File a [registry-stack issue](https://github.com/registrystack/registry-stack/issues), and run the flow locally with [First run with Registry Lab](../../tutorials/first-run-with-registry-lab/) in the meantime. |
