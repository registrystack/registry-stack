...
description: "The normative contract for relayctl: seven commands, the schema-inspection boundary, compilation profiles, the sealed package output, exit statuses, and the process boundary that keeps relayctl out of the serving path."
status: draft
owner: registry-docs
source_repos:
  - registry-stack
...
Human-readable output text, the JSON report shape, diagnostic message wording, and any private
path inside a generated or packaged directory are not compatibility surfaces.

## 2. Command surface

`relayctl` exposes exactly seven commands.
...
REQ-PR-RELAYCTL-025: Every command MUST return one report carrying a status of `success` or
...
The JSON report carries no schema-version field and is not a versioned automation contract.
...
- Exposes exactly the seven commands, takes each project as a positional argument, and reads no
  environment variable (REQ-PR-RELAYCTL-001 through REQ-PR-RELAYCTL-003).
...
- Emits value-free reports and diagnostics under both output modes (REQ-PR-RELAYCTL-025 through
  REQ-PR-RELAYCTL-027).
...
{/* Evidence: crates/registry-relayctl/src/lib.rs defines the seven-command parser, the global
     --json flag, the required package --output, the exit-code constants, and report rendering. */}
...
{/* Evidence: crates/registry-relayctl/tests/cli_contract.rs pins the one-binary workflow, the
     absence of a row or value sampling surface, the refusal of an implicit package destination, and
     that the commands link the shared library and never spawn relay. */}
...
- The command line, its flags, its exit codes, and the no-spawn boundary are pinned by the
  `relayctl` crate and its command-line contract tests.
