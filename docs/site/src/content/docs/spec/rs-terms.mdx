...
**DID**: Decentralized Identifier. W3C DID Core 1.0. Registry Relay publishes its DID document (`did.json`) only in gateway issuer mode; in delegated issuer mode Relay MUST NOT publish `did.json` and the runtime refuses to serve it (REQ-PR-RELAY-013). Registry Notary parses `did:jwk` values for credential subjects.
...
**claim definition**: The Registry Notary configuration artifact that declares a named claim capability: its identity, the inputs and source bindings it reads, the single rule that decides its value, and the disclosure, format, and credential rules that shape what leaves the service. Registry Stack product term.
...
**issuer mode**: Registry Relay signed response credentials configuration axis with two values: gateway mode, where Relay hosts its own DID document and signs under its own DID, and delegated mode, where a delegating ministry hosts the DID document for its own domain and Relay MUST NOT publish `did.json`. Registry Stack product term.
...
**matching policy**: Registry Notary source-binding configuration that gates and shapes how a request resolves to a source record before the read runs, constraining the read by declared purpose, relationship context, and which inputs may identify the subject. A source binding with no matching policy falls back to unrestricted, identifier-only resolution. Registry Stack product term.

**source adapter sidecar**: A Registry Notary source connector kind that reaches external sources such as DHIS2 or FHIR servers through the built-in `http_json`, `http_flow`, or `fhir` engines. It is a private deployment surface behind Registry Notary, not a public subject-facing API; a static bearer token authenticates only the internal Notary-to-sidecar hop. Registry Stack product term.

**source connector**: The Registry Notary mechanism a source binding uses to reach its source data over HTTP. The supported connector kinds are `registry_data_api` (a Registry Relay instance), `dci` (an SP DCI search source), and the source adapter sidecar. There is no file-based or database connector. Registry Stack product term.
