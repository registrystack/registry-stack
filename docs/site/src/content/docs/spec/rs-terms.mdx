...
**SDMX**: Statistical Data and Metadata eXchange. Registry Relay 1.0 serves configured aggregates as experimental SDMX-JSON 2.1 data messages through content negotiation or `?f=sdmx-json`; that surface does not implement structure messages or SDMX REST routes. Relay V2 defines a separate profiled SDMX REST 2.2.2 read subset over governed pre-aggregated snapshots. The V2 subset defines data JSON and CSV 2.1.0 plus dataflow and data structure definition JSON 2.1.0, uses one fixed statistical-dataset access rule, and does not expose schema or availability placeholders. Neither surface claims full SDMX conformance or certification.
