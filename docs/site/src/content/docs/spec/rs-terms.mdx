...
**PDP**: Policy decision point. The shared Registry Platform primitive (`registry-platform-pdp`) that evaluates a governed policy against trusted request and source context and returns permit, permit with redaction, or deny. It survives as a vocabulary source for Registry Manifest metadata field names. No maintained runtime calls it: the retired Relay 1.0 governed path was its only consumer, and Evidence Gateway has never used one, because its authorization is one decision over the verified token, the requirement, and the matched grant. Registry Stack product term.
...
{/* Evidence: crates/registry-relay-v2/src/{contract,compiler,api,artifacts,package,startup}.rs
    ...
    crates/registry-relayctl/src/ carries the seven commands. crates/registry-manifest-core/src/lib.rs
    still holds REQUIRED_EVIDENCE_PACK_GATES, ODRL_ENFORCEMENT_PROFILE, and the policy-hash checks the
    Section 4 manifest terms describe, and crates/registry-platform-pdp has no consumer among the
    maintained runtimes. */}
