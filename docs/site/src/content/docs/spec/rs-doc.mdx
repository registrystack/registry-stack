...
| 0.3.0 | 2026-09-06 | draft | Tightened REQ-DOC-014: a `verified` specification now has to trace each load-bearing claim to an inspectable artifact of the stack itself, and a site page, another specification, or a register entry corroborates such an artifact rather than standing in for one. The evidence-anchor check enforces the mechanical part. |
...
**Evidence Gateway** is how true the document is against shipped code:

- `verified` is backed by code, tests, fixtures, generated artifacts, or released machine-readable contracts that a reader can inspect. A `verified` document carries evidence references to those artifacts (REQ-DOC-014); a claim a reader cannot trace to one is not `verified`.
...
REQ-DOC-014: A specification whose `evidence` is `verified` MUST carry evidence references: an Evidence section, per-requirement references, or evidence anchors in the page source. Each load-bearing claim MUST trace to at least one inspectable artifact of the stack itself: source code, a test, a fixture, a generated artifact, or a released machine-readable contract. A published site page, another specification, or a register entry MAY corroborate such an artifact, and MUST NOT be the only evidence a claim carries, because a document that proves itself by citing another document says nothing about shipped behavior. A `verified` specification MUST therefore carry at least one evidence anchor naming an artifact outside the documentation site's own content; the evidence-anchor check enforces that much and resolves every path and symbol an anchor names. A specification that cannot meet this bar MUST declare `partial` or `aspirational` and mark the claims shipped code does not back (REQ-DOC-007).
...
A document conforms to RS-DOC when it: is a distilled public contract, not copied internal evidence (REQ-DOC-001); carries a unique, well-formed `doc_id` and the `category` and `evidence` axes (REQ-DOC-004, REQ-DOC-006); states its evidence honestly, including the unbuilt case (REQ-DOC-007); backs a `verified` evidence level with inspectable artifacts of the stack rather than with other documents (REQ-DOC-014); carries a version-history table that records normative changes (REQ-DOC-012); and interprets normative keywords per Section 2. The frontmatter check enforces the mechanical requirements; the rest is a review responsibility.

## Evidence Gateway

This specification is `verified`: the mechanisms it defines exist and run, and a reader can inspect them.

{/* Evidence: docs/site/scripts/check-doc-frontmatter.mjs holds validCategory and validEvidence, the
    frontmatter vocabulary Sections 3 and 4 fix, and refuses a specification that declares neither.
    docs/site/scripts/check-evidence-anchors.mjs holds checkEvidenceAnchors, which resolves every
    path and symbol an anchor cites and refuses a verified specification whose anchors reach no
    artifact outside the site's own content. Its cases are in
    docs/site/scripts/check-evidence-anchors.test.mjs. */}
...
- `scripts/check-evidence-anchors.mjs` resolves every path and symbol an evidence anchor cites, and refuses a `verified` specification whose anchors reach no artifact outside this site (Section 4, REQ-DOC-014).
