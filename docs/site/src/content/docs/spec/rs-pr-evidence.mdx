...
- The four response formats: the signed JSON Web Signature (JWS) assertion, the separately typed
  ...
  same assertion, and the batch envelope that carries one holder-bound credential per presented
  holder key.
...
MUST be absent for unsigned output. The standalone authorization-refusal event MUST contain neither
...
- Batch linkability: A batch reduces deterministic key-based linkability and does not make
...
- audits at the refusal and authorized-material durable gates, validates every event against the
  closed schema, records response protection only for authorized material, records a minimal refusal
