...
after authorization and before credential acquisition or source access, and a disclosure-release
event after the final immutable response bytes are serialized and before those exact bytes are
released.
