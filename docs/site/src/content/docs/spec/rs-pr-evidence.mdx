...
Only a singular `no_match`, `ambiguous`, `required_fact_missing`, or
`derivation_input_unresolved` outcome MAY become `evidence_not_available`; every other failure MUST
abort the complete operation through the existing problem contract without releasing item material.
