...
| 0.8.1 | 2026-09-25 | draft | Described Relay's audit as a log without a hash chain in the serving flow and REQ-ARC-G-020. |
...
7. **Wallet delivery.** Where a deployment enables it, the wallet delivery front end carries an Evidence-signed credential to a holder wallet over OID4VCI. It adds no claim and no signature of its own.
...
REQ-ARC-G-020: Relay MUST own no source of truth. It MUST read adopter data through a bounded read-only boundary over the schema objects its governed contract declares, and MUST NOT persist a record value. The audit log is the only state a Relay deployment writes.
...
auditable, and the wallet delivery front end carries a signed credential to a holder wallet. The external Solmara Lab
