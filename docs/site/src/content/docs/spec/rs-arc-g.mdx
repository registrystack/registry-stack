...
product-neutral adaptation. Each Evidence Gateway evaluation answers one bounded question about one
complete subject-role set from its own configured sources. A request batch can group up to sixteen
independent audience-scoped evaluations. Registry Mint issues the access tokens a resource server such as Evidence Gateway
...
Evidence Gateway Version 1 supports its declared holder-bound subject-binding mode and bounded issuance envelope, but has no credential lifecycle, no OID4VCI surface, no interactive holder-binding ceremony or issuance session, no status list or revocation, no delegated or federated evaluation between peers, no policy decision point, no replay subsystem, and no worker or document subsystem. It does not depend on Registry Relay, does not consult it, and does not read the metadata manifest.
