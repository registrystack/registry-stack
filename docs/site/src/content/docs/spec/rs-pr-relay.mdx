...
Relay also parses four further request-context headers without a scope guard: `x-registry-subject-ref`, `x-registry-relationship`, `x-registry-on-behalf-of`, and `x-registry-credential-format`. These populate the request context for the PDP, but none of them are currently wired to any Relay policy decision.
