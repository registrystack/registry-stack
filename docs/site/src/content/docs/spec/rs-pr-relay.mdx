...
Registry Notary is retired, and Relay's evidence-offering surface still carries its vocabulary: runtime binding validation accepts an offering only when `access.kind` is `registry-notary`, and it rejects the configuration otherwise. An operator therefore cannot publish an offering that points at an Evidence deployment without that access kind being widened first. Treat this section as describing published surface with no maintained destination behind it.

## 8. Credential issuance boundary

Registry Relay does not issue credentials, host issuer DID documents, or manage issuer signing keys. Signing belongs to a separate evidence service; the product that used to own it is retired, and Evidence now signs its own assertions under its own contract.
