...
Project authoring can compile a Notary service without configured claims. Every configured claim
must derive from an exact compiler-pinned Registry Relay consultation. The compiler and end-to-end
tests enforce this in
...
REQ-PR-REGISTRYCTL-018: Registryctl MUST reject a Notary claim that does not
derive from at least one declared Registry Relay consultation. Caller-provided,
OIDC, and authorization-details data MUST NOT be compiled as claim evidence.
...
- Refuses claims without Registry Relay evidence (REQ-PR-REGISTRYCTL-018).
