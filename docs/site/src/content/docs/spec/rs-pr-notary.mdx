...
| 0.1.1 | 2026-06-20 | draft | Document governed source policy decisions, source-bound purpose and freshness constraints, subject access trust context, audit provenance, and scoped OID4VCI behavior. |
| 0.1.2 | 2026-06-21 | draft | Clarified source adapter sidecar isolation assumptions and demo/template freshness boundaries. |
| 0.1.3 | 2026-06-22 | draft | Document delegated subject access, proof-claim gating, explicit dependent-target binding, delegated denial reasons, and the OID4VCI delegated-token rejection boundary. |
...
| 0.2.0 | 2026-07-13 | draft | Removed direct registry source connectors and sidecars. Registry-backed claims now consume compiler-pinned Relay consultations. |
...
| 0.4.0 | 2026-07-30 | draft | Required every claim and federation profile to use compiler-pinned Relay evidence and renamed subject-access modes. |
| 0.5.0 | 2026-08-01 | draft | Excluded audit from REQ-PR-NOTARY-041's Notary-owned PostgreSQL correctness state: audit correctness is owned by the audit pipeline instead, so restoring the PostgreSQL database MUST NOT be treated as restoring audit history and active-active replicas sharing that schema do not thereby share one ordered audit state. |
...
- Delegated subject access for configured requester-dependent relationships.
...
Registry Notary is a standalone HTTP service for claim evaluation, disclosure policy, credential issuance, and audit (REQ-ARC-G-007). It can start without configured claims, but every evaluatable claim requires a compiler-pinned Registry Relay consultation.

REQ-PR-NOTARY-001: Registry Notary MUST be independently deployable. It MAY start without Registry Relay when no claims are configured. A configured claim MUST NOT become ready or evaluate without its pinned Relay consultation contract.
...
A matched client id, authorized party, or audience MAY participate in token verification, scope mapping, or subject access classification, but MUST NOT substitute for a missing configured principal claim.

When a claim declares governed policy, Notary evaluates the supported Evidence Gateway PDP profile before a Relay consultation. The supported profile covers policy identity and hash, supported ODRL purpose and spatial terms, claim purpose, jurisdiction, assurance, freshness represented by trusted Relay provenance, required legal basis and consent, relationship and relationship-purpose constraints, requested fact, requested disclosure, requested credential format, route identity, checked scope, redaction, and unsupported policy terms. Caller-supplied target fields do not satisfy trust-context gates that require a trusted principal authorization context.

REQ-PR-NOTARY-023: Before a Relay consultation, Registry Notary MUST obtain the applicable policy decision for the claim. A denial MUST fail closed with the stable PDP denial code produced for that gate and MUST NOT be retried through a less governed path.
...
A claim evaluation moves typed request inputs through a compiler-pinned Relay consultation, then a rule, disclosure policy, and response format. The narrative form of this pipeline, with worked examples, is in [Evidence issuance, end to end](../../explanation/evidence-issuance/).

```mermaid
flowchart LR
    subject["Typed request inputs<br/>(from authorized caller)"]
    evidence["Evidence mode<br/>Relay consultation · subject access"]
...
The diagram restates the pipeline: an authorized caller supplies the configured target and request inputs plus a claim id. The claim invokes exactly the pinned Relay consultation. The rule evaluates, disclosure constrains the output, and the result can be rendered as claim-result JSON or CCCEV-shaped JSON-LD. SD-JWT VC output is materialized through the credential issuance surface in Section 7. A caller does not supply the evaluated value.
...
Every configured claim derives from exactly one compiler-pinned Relay
consultation. Credential profiles, subject-access allow-lists, federation
profiles, and OID4VCI projections select only `registry_backed` claims.
...
but MUST appear once in the normalized execution set. Legacy, incomplete,
...
check before holder-proof replay mutation. OID4VCI MUST reject a credential
configuration without complete Relay-backed provenance, create and evaluate the registry transaction before
...
Subject access is a constrained authorization context, not a caller-provided claim value. It binds the authenticated citizen principal to configured claims, purposes, disclosures, formats, subject binding, assurance requirements, and token age limits before Relay-backed evaluation. Direct subject access can authorize issuance for a registry-backed claim with a mutually bound credential profile. Delegated subject access can authorize issuance only through an OID4VCI credential configuration that explicitly selects the digitally authenticated representative ceremony.

REQ-PR-NOTARY-026: For subject access flows, Registry Notary MUST derive the subject and assurance context from the authenticated principal and configured subject binding. It MUST NOT let the caller choose a different subject, claim, purpose, disclosure, format, or credential profile outside the subject access policy, and MUST audit subject access decisions without raw subject identifiers or bearer tokens.

REQ-PR-NOTARY-035: Where subject access evaluate receives a Notary-issued transaction token, Registry Notary MUST require scoped authorization details for the requested evaluation.
...
Externally issued OIDC tokens that use a standard access-token `typ` MAY rely on configured subject access scopes and MUST NOT be forced into the Notary transaction-details profile solely because of that `typ`.

Registry Notary has two subject access modes:

- `subject_bound`: the authenticated principal is the credential subject.
- `delegated_subject_access`: the authenticated principal is the requester, and a configured dependent target can be evaluated only when scoped authorization details name the same target, relationship, and proof claim.

Delegated subject access is distinct from Section 9 federation. It does not delegate trust to a peer Notary. It stays inside the citizen/OIDC trust context and uses a compiler-pinned Relay relationship proof before the dependent claim.

REQ-PR-NOTARY-029: For delegated subject access, Registry Notary MUST derive the requester from the authenticated principal, derive the relationship from scoped authorization details, require the scoped authorization details to bind the same dependent target by `target.id_type` and `target.id`, and reject caller-supplied `requester`, `relationship`, or `on_behalf_of` fields before any Relay consultation.
...
flow does not create a caller-supplied credential evidence path.
...
MUST NOT render an offer for a delegated, failed, ambiguous,
...
delegated-subject-access and registry-backed evaluation-only records
...
evaluations, and credential status. Explicit in-memory state MAY be used
only for local single-process development. Audit records are not part of that
PostgreSQL state: audit correctness is owned by the audit pipeline instead. That
pipeline hash-chains its records, and it MUST emit every evaluated request to the
configured sink and fail the request when it cannot (REQ-PR-NOTARY-020,
REQ-PR-NOTARY-021). Sink retention and off-host shipping are operator
responsibilities surfaced through the deployment evidence gates. Restoring the Notary PostgreSQL database
therefore MUST NOT be treated as restoring audit history, and active-active
replicas sharing that schema do not thereby share one ordered audit state.
...
REQ-PR-NOTARY-033: Subject access policy denials MUST use the public problem code `subject_access.denied` while preserving the granular denial reason in audit context. Delegated subject access denial reasons are `delegated.relationship_unproven`, `delegated.relationship_not_allowed`, `delegated.claim_denied`, `delegated.subject_not_permitted`, and `delegated.proof_denied`.
...
- Delegated issuance: Delegated subject access is evaluation and rendering only. Direct and OID4VCI credential issuance reject delegated evaluations, and delegated credential-profile bindings are invalid configuration (REQ-PR-NOTARY-030, REQ-PR-NOTARY-031, REQ-PR-NOTARY-032).
...
- Relay dependency: Claims require their exact compiler-pinned Relay consultation to be ready. Notary does not provide a direct-source or caller-input fallback (REQ-PR-NOTARY-006).
- Issuance provenance: Legacy evaluations are nonissuable. Every selected root must retain an exact compiler pin for every registry-backed claim in its executed dependency closure and one normalized execution record per unique consultation ULID. Direct issuance performs the exact-set check before holder-proof replay mutation; OID4VCI evaluates and stores the registry transaction before rendering an offer, then revalidates it after holder-proof nonce consumption (REQ-PR-NOTARY-027, REQ-PR-NOTARY-037).
...
- starts independently when no claims are configured, requires the exact Relay contract for every configured claim (REQ-PR-NOTARY-001), and gates capability discovery while publishing its JWKS openly (REQ-PR-NOTARY-002);
...
- evaluates claims from pinned Relay outputs, with machine or subject-access authorization and no direct source connectors, using only implemented rule types (REQ-PR-NOTARY-005, REQ-PR-NOTARY-006, REQ-PR-NOTARY-007);
...
- treats subject access as constrained authenticated trust context, including delegated subject access only when requester, dependent target, relationship proof, and stored-evaluation metadata all bind to the configured policy (REQ-PR-NOTARY-026, REQ-PR-NOTARY-029, REQ-PR-NOTARY-030, REQ-PR-NOTARY-031, REQ-PR-NOTARY-035);
- exposes OID4VCI as a registry-backed non-full-issuer subset with issuer-initiated pre-authorized code only, secure transaction-code defaults, EdDSA `did:jwk` holder proof, matching stored transaction details, one Notary per Relay authority, and no caller-supplied or delegated issuance (REQ-PR-NOTARY-016, REQ-PR-NOTARY-017, REQ-PR-NOTARY-027, REQ-PR-NOTARY-032, REQ-PR-NOTARY-036, REQ-PR-NOTARY-039, REQ-PR-NOTARY-040, REQ-PR-NOTARY-041);
...
- Subject-bound and delegated subject access modes, Relay consultation capabilities, stored metadata, and denial reasons are implemented in Registry Notary Core and Server.
...
- The issuer-bound subject access transaction-token gate (REQ-PR-NOTARY-035) is implemented in [`api.rs`](https://github.com/registrystack/registry-stack/blob/v0.11.0/crates/registry-notary-server/src/api.rs)'s `subject_access_requires_authorization_details` function.
