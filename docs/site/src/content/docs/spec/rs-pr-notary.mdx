...
PostgreSQL state: audit correctness is owned by the hash-chained audit pipeline
that writes to the configured sink (REQ-PR-NOTARY-020, REQ-PR-NOTARY-021), and
sink retention and off-host shipping are operator responsibilities surfaced
through the deployment evidence gates. Restoring the Notary PostgreSQL database
