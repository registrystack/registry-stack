...
- Admin reload: The admin reload route returns HTTP 501 with code `registry.admin.capability.not_supported` in the standalone router; it performs no reload; configuration changes generally require a service restart, with the narrow hot-apply exceptions listed in the [known limitations hub](../../explanation/known-limitations/).
