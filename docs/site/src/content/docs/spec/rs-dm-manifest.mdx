...
Registry Notary is retired, and no maintained runtime consumes this section. Registry Manifest still validates the vocabulary, because it is published schema surface that manifests in the field already carry, so the requirement below records what the validator enforces today rather than a capability an adopter can deploy. Whether the vocabulary is removed or renamed is a manifest version decision that has not been taken.

A manifest MAY advertise the metadata a partner needed to configure static-peer delegated evaluation against that retired product. This metadata is public discovery: it describes where a delegated evaluator is and what it evaluates, not who may call it.

REQ-DM-MANIFEST-010: Where an evidence offering declares `access.kind: registry-notary`, the manifest MUST carry a `federation` block with HTTPS issuer, JWKS, and federation-API URLs, a `did:web` node identifier bound to the issuer host, the supported protocol version `registry-notary-federation/v0.1`, and an `access.ruleset` that names a declared evaluation profile. This metadata is public discovery only and MUST NOT be read as an access grant (REQ-ARC-G-009). The service that would have decided at runtime which peers may call it is retired, so a manifest declaring this access kind advertises an endpoint no maintained runtime serves.

Federation discovery metadata is also not a trust bootstrap by itself. A consumer that reads federation metadata from an external manifest validates the issuer, JWKS, federation API, and `did:web` host against local trust policy before treating any key or endpoint as authoritative. The registry stack no longer has a runtime path that consumes this metadata at all.
...
- **Federation metadata is not a trust anchor.** Registry Manifest validates required federation fields and selected URL shapes, but a published manifest does not bind an external consumer to trust a JWKS endpoint, a federation API host, or a peer identity. Host pinning and trust bootstrap belong to the consumer's local policy, and the runtime that once held the peer configuration is retired (REQ-DM-MANIFEST-010).
...
- **The federation checks are keyed to a retired access kind.** They apply only where the manifest declares the `registry-notary` evidence access path, and they validate shape, not reachability. A generic `conforms_to` value or generic URI metadata alone must not be treated by consumers as proof that an endpoint set is complete and safe to trust, and the product behind that access kind is retired in any case.
