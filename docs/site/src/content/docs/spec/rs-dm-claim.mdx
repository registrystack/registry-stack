...
An authored mapping admits only that named value from the authenticated target request; it does
not authorize arbitrary traversal of the target object.
...
that membership and MUST reject unknown claims. Registry Notary MUST issue a claim only when the
compiled profile membership and derived reverse index agree. The issued credential is an SD-JWT VC
