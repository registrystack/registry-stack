...
REQ-DM-CLAIM-002: Each claim MUST declare `registry_backed` evidence and exactly one Relay consultation. It MUST bind that Relay consultation by profile id and contract hash and MUST map only declared typed request inputs. Registry Notary MUST enforce the claim's required scopes before Relay work.
