...
This document defines the target data model of a Registry Notary claim definition: the configuration artifact that declares what Relay-backed evidence Notary can evaluate, what typed inputs and Relay consultations it may use, what it discloses, and what it can issue. It is the structural counterpart to the [Registry Notary protocol](../rs-pr-notary/).
...
| 0.3.0 | 2026-07-13 | draft | Replaced direct source bindings and Notary matching with compiler-pinned Relay consultations. |
...
| 0.4.0 | 2026-07-17 | draft | Restricted credential membership to mutually bound registry-backed claims. |
| 0.5.0 | 2026-07-30 | draft | Removed claim evaluation without a declared Relay consultation. |
...
flowchart TD
    ...
    evidence["Evidence mode<br/>registry-backed consultation"]
...
The diagram restates the model: a claim receives compiler-pinned Relay outputs, a single rule decides the value, and the disclosure policy and formats govern the result. A claim definition describes one decision or one extracted value; a claim that tries to return a whole record over-collects and is hard to authorize.
...
A claim's inputs are the named request paths it may consume, drawn from the target subject, requester, and declared variables. The `registry_backed` evidence mode binds compiler-pinned Relay consultations whose typed outputs feed the rule. Notary has no direct registry source mode and no claim mode without Relay.

REQ-DM-CLAIM-002: Each claim MUST declare `registry_backed` evidence and at least one Relay consultation. It MUST bind every Relay consultation by profile id and contract hash and MUST map only declared typed request inputs. Registry Notary MUST enforce the claim's required scopes before Relay work.
...
that membership and MUST reject unknown claims or claims without a declared Relay consultation. Registry Notary MUST require every
...
REQ-DM-CLAIM-011: Batch evaluation MUST be semantically equivalent to evaluating each item independently. Batches MUST validate the complete outer request before the first Relay call, require an idempotency key, preserve item order, and MUST NOT coalesce duplicate subjects across item positions or bypass single-evaluation authorization and disclosure.
...
- The [Registry Notary operator configuration reference](../../products/registry-notary/operator-config-reference/) lists the claim, Relay, subject access, and credential-profile fields.
