...
returned in a posture document now lives in the audit log and in the process log, both of which
...
| 1.0.1 | 2026-09-25 | draft | Restated audit readiness in REQ-OP-POSTURE-105 as a live probe of the shared audit writer and its destination, with no chain head, and described Relay's audit entries without a hash chain. |
...
Audit readiness MUST require a live probe of the audit writer and its concrete destination, so
that a replaced, removed, or unwritable audit file, or a writer stopped by an earlier failed
append, makes the deployment unready.
...
Relay writes a value-free audit entry for every consultation it attempts, refuses,
...
- `crates/registry-relay-v2/src/audit.rs` defines audit readiness as a live probe of the shared
  audit writer, and the audit-failure branches in `api.rs` return `audit.unavailable` at every phase
  (Sections 3 and 4).
