...
For a spreadsheet project, `preflight` reads and validates the complete
selected sheet. An offline-only project may deliberately omit runtime
bindings. You can test and build that project, but you cannot start it until an
operator supplies the missing environment values.
