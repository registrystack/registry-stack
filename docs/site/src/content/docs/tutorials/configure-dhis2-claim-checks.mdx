...
A private source-adapter sidecar (a small service that runs next to Notary and translates the
source's own routes, auth, and response shape into the Registry Data API-shaped lookup route
Notary expects) calls the DHIS2 Tracker API, Registry Notary evaluates
...
The local demo defaults to the public DHIS2 play credentials. The variable names carry an
`OPENFN_` prefix, and the DHIS2 sidecar's service name keeps a lowercase `openfn` prefix, for
historical reasons. To point the profile at another DHIS2 environment, set these values before you
run the profile:
...
- `OPENFN_SIDECAR_TOKEN_RAW`, the variable name Registry Notary uses when it calls the
  private DHIS2 sidecar.
- `OPENFN_SIDECAR_TOKEN_HASH`, the variable name the sidecar uses to authenticate Notary.
...
```mermaid
flowchart LR
    caller["Caller<br/>DHIS2_EVIDENCE_CLIENT_BEARER"]
    notary["Registry Notary<br/>config/notary/dhis2-health-notary.yaml"]
    sidecar["DHIS2 source adapter sidecar<br/>config/source-adapter/dhis2-health-sidecar.yaml"]
    dhis2["DHIS2 Tracker"]
    caller -- "bearer token" --> notary
    notary -- "sidecar token" --> sidecar
    sidecar -- "http_json engine" --> dhis2
```

The diagram restates the chain: a caller authenticates to Registry Notary with
`DHIS2_EVIDENCE_CLIENT_BEARER`, Notary calls the private sidecar with the shared sidecar token
(the `OPENFN_SIDECAR_TOKEN_RAW` variable), and the sidecar's built-in `http_json` engine
queries DHIS2 Tracker.
...
| `sources.dhis2_health.credential_env` | Reads DHIS2 `baseUrl`, `username`, and `password` from the `OPENFN_DHIS2_DEMO_CREDENTIAL_JSON` variable. |
...
| `evidence.source_connections.dhis2_openfn.token_env` | Sends the shared sidecar token from the `OPENFN_SIDECAR_TOKEN_RAW` variable. |
