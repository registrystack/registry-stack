...
tutorial_test:
  toolset: evidence
  skip: how-to against the reader's own OpenAPI source; no fixed scenario to replay
---
...
```sh
evidencectl init institution-evidence \
...
schemas/people-parameters.schema.yaml: schema objects must declare bounded properties`. Fill the
stub with the one reviewed parameter this source takes, require it, and give it a value in
`adapterParameters`. The Evidence authoring form reference states that rule and the others a first
build meets under
...
Start the local Evidence Gateway and stock identity provider:
...
`dev start` returns once both services answer on loopback; the retained pair keeps running after
the command exits. Stop it when you are finished:
