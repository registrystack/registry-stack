...
Solmara Lab is a standalone Registry Stack adopter demo for the fictional Republic of Solmara. It
wires six [Registry Relay](../../products/registry-relay/) instances and four
[Registry Notary](../../products/registry-notary/) instances over generated synthetic data into one
topology, alongside a citizen portal and a public Visitor's Center, and it replaces the monorepo's
former `lab` directory.

This tutorial brings the topology up with the `just` recipes the lab provides, verifies it with the
smoke suite, and calls a Notary evaluation route directly so you touch the protected API surface,
not only the portal. If you only want the shortest hosted path before running anything locally,
start with the [Quickstart](../../start/quickstart/) instead. This page is the full local tour.

For a standalone local project generated from your own data, use the `registryctl` tutorials
instead: [run a protected registry API](../publish-spreadsheet-secured-registry-api/) or
[evaluate a claim with Registry Notary](../verify-claim-registry-api/). Clone Solmara Lab only when
you want the full multi-service country demo rather than a generated project.
...
<QuickstartMeta
  ...
  prerequisites={['Docker Compose', 'just', 'uv', 'python3 and openssl', 'pnpm 11.3.0 via corepack', 'git']}
/>
...
Every Relay and Notary configuration in this lab sets `openapi_requires_auth: false`, so each
instance publishes its OpenAPI reference at `/docs` and `/openapi.json` without a credential, the
same route pattern documented in
...
disk, and reproduce the Notary evaluation call above in a single command so it can run without a
shell variable.
