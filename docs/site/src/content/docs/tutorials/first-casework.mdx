...
`tutorial-work/requester.header` and the three header files after it each hold a bearer token that
acts as that client until it expires.
...
It reads its own [terminal feed](../../reference/glossary/#terminal-feed):
...
Now ask for the same inbox with the Staff token, selecting the `supervisor` profile:
