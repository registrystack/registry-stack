...
`CASEWORKCTL_DEV_CASEWORK_PORT` and `CASEWORKCTL_DEV_MINT_PORT` overrides; replace the URLs if the
report differs:

```sh
casework_url="http://127.0.0.1:${CASEWORKCTL_DEV_CASEWORK_PORT:-8092}"
token_endpoint="http://127.0.0.1:${CASEWORKCTL_DEV_MINT_PORT:-8093}/token"
