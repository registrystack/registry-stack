...
Turn an editable Evidence project into one reviewed production candidate, then bind its secrets and
runtime paths on the target host. This tutorial uses released `evidence` and `evidencectl` binaries.
It does not copy `.evidence/dev`, generate a key, register a caller, or deploy a service for you.
...
Verify the retained response with independently prepared production policy and trusted public keys,
