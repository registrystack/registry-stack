...
[`runtime.docker.yaml`](https://github.com/registrystack/registry-stack/blob/v0.26.1/docker/compose/runtime.docker.yaml)
in the Registry Stack repository does; that value permits the wildcard bind for an operator-confined
container network and authorizes no public or direct-TLS serving. The runtime cannot override the
governed service, authentication, authority, source, disclosure, or signing fields.
...
`evidencectl build` takes the binary from `EVIDENCE_BIN` or the first `evidence` on `PATH`, and
`evidencectl fixtures run` accepts `--evidence-bin` for the same handshake. A bundle that declares a
`publication` also has to render the description that advertises it, and a build whose description
comes back empty is refused rather than written, because `the candidate would carry no
catalog.jsonld`. The build does not contact an identity provider, Mint, or a source endpoint. It
...
Make the candidate runtime and bundle non-writable to the Evidence Gateway service identity, for
example `chmod -R a-w bundle && chmod 444 runtime.yaml`. `evidencectl fixtures run` below runs
