...
or npm. They require Cosign and download the first Registry Stack release carrying this client API.
...
case "$(uname -s)-$(uname -m)" in
  Linux-x86_64) PLATFORM="manylinux_2_17_x86_64.manylinux2014_x86_64" ;;
  Linux-aarch64|Linux-arm64) PLATFORM="manylinux_2_17_aarch64.manylinux2014_aarch64" ;;
...
`adult-status` starter, then leave you in that directory. Its implicit tutorial caller remains
unchanged for the authoring follow-ups. The access policy, registered client, and private client key
remain isolated in `adult-status-request`.
