...
This command starts Evidence Gateway and Mint only. The Python registry remains the process you started in
the first terminal.
...
expectations before a response exists. It does not send the request to Evidence Gateway or contact the
...
`curl` called Evidence Gateway, Evidence Gateway called the registry, and the HTTP response is a signed flattened
...
```text
Local Evidence Gateway stopped
```
...
```text
Removed stopped local Evidence Gateway state
```
