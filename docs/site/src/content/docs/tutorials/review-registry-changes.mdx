...
description: Author and inspect a Base Registry Engine (BReg) change request whose review is owned by Registry Casework, then verify that source application remains separately authorized.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
<QuickstartMeta
  ...
  prerequisites={[
    { label: 'bregctl', href: '/tutorials/first-breg/#install-base-registry-engine' },
    { label: 'A Registry Stack checkout at the same release as your bregctl', href: 'https://github.com/registrystack/registry-stack' },
    'A shell and an editor',
  ]}
/>
...
Open `tutorial-work/asset-corrections/registry.yaml`. The request entity declares the effect BReg
will freeze and the logical review authority and policy Casework will evaluate:

```yaml test-excerpt="tutorial-work/asset-corrections/registry.yaml"
changeRequest:
  retention:
    mode: operator_erase
  effects:
...
grants. It has no BReg reviewer action or stage route. The request's entry names the review
authority and how an approved result is applied:

```json test-excerpt="tutorial-work/change-requests.json"
{
