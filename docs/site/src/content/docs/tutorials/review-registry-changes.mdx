...
cp -R products/registry-server/acceptance/asset-site-placement-change-requests tutorial-work/asset-corrections
export CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0
cargo build --locked -p registry-serverctl
target/debug/registry-serverctl check tutorial-work/asset-corrections
```
...
```sh
target/debug/registry-serverctl explain change-requests tutorial-work/asset-corrections \
...
The JSON file also lists the compiled effects, the grants of each role, and the request bounds.
...
```sh
python3 - "$PWD/products/registry-server/quickstart/.run" tutorial-work/test.env <<'PY'
...
{/* Evidence: products/registry-server/quickstart/run.sh;
    ...
    products/registry-server/generated/asset-site-placement-change-requests/generated/openapi.json;
    crates/registry-server/src/fixtures.rs;
    crates/registry-server/src/request_workflow.rs. */}
...
- [Author a registry project](../../configure/registry-server/#change-requests) for every member
  of `changeRequest` and `changeControl`.
- [Registry Server API reference](../../reference/registry-server-api/#change-requests) for the
  seven request states, the action routes, and the `request.actions[]` shape.
- [Deploy and operate a registry](../../operate/registry-server/) to package and activate your
  project, and to erase proposal detail under `operator_erase`.
- Open `products/registry-server/CHANGE_REQUEST_EXAMPLES.md` in your clone for the household
...
| `change-requests` is not a recognized explain subject | Rebuild `registry-serverctl` from this clone; the tutorial does not target a released binary. |
