...
<QuickstartMeta
  ...
  prerequisites={['Docker Compose 2.20+', 'just', 'uv', 'A Registry Lab checkout with submodules']}
/>
...
[`registryctl` protected API tutorial](../publish-spreadsheet-secured-registry-api/)
or the [`registryctl` claim evaluation tutorial](../verify-claim-registry-api/).
`registryctl` can scaffold a standalone Notary project for an existing FHIR
source-adapter sidecar (a small service that runs next to Notary and translates the source's own
routes, auth, and response shape into the Registry Data API-shaped lookup route Notary expects),
but this tutorial uses `registry-lab` directly because it also starts deterministic FHIR fixture
data and the source adapter.
...
Clone the Registry Stack monorepo if you do not already have it; the lab lives in
...
Build the two FHIR profile images (the fixture server runs a stock Python image):
...
Give the Notary two to three minutes after `just fhir-up` before running the
smoke: the fixture server and sidecar are health-gated, but the Notary is not,
and an early run fails with HTTP 500 `claim.rule_evaluation_failed` until it
finishes warming up ([GH#279](https://github.com/registrystack/registry-stack/issues/279)).
...
```text
FHIR smoke passed; artifacts written to <checkout>/lab/output/fhir-smoke
```

The artifact path prints as an absolute path for your checkout.
...
```mermaid
flowchart LR
    caller["Caller (you)<br/>FHIR_EVIDENCE_CLIENT_BEARER"]
    notary["Registry Notary<br/>config/notary/fhir-health-notary.yaml"]
    sidecar["FHIR source adapter sidecar<br/>config/fhir/fhir-source-adapter-sidecar.yaml.template"]
    fhir["FHIR fixture server<br/>FHIR R4 API"]
    caller -- "bearer token" --> notary
    notary -- "sidecar token" --> sidecar
    sidecar -- "fhir engine" --> fhir
```

The diagram restates the chain: your call carries `FHIR_EVIDENCE_CLIENT_BEARER` to Registry
Notary, Notary authenticates to the private source adapter with the `FHIR_SIDECAR_TOKEN_RAW`
sidecar token, and the adapter's built-in `fhir` engine queries the FHIR fixture server.
...
| `just setup` fails with `fatal: cannot change to '<pid>(git rev-parse ...)'` | Escaping bug in the setup recipe on fresh clones ([GH#279](https://github.com/registrystack/registry-stack/issues/279)). | Run the submodule command directly: `git -C "$(git rev-parse --show-toplevel)" submodule update --init --recursive -- lab/vendor/crosswalk lab/vendor/registry-atlas lab/vendor/esignet-relay-authenticator`. |
| `just fhir-smoke` returns HTTP 500 `claim.rule_evaluation_failed` shortly after startup | The Notary is still warming up; it is not health-gated by Compose. | Wait two to three minutes after `just fhir-up`, then rerun the smoke. |
| `. .env` reports `no such file or directory` under zsh | zsh does not search the current directory for `.` targets. | Use `. ./.env` (works in both `sh` and `zsh`). |
...
You ran Registry Notary against local FHIR R4 fixture data through a private source adapter,
without exposing FHIR resources directly to a caller. The scripted smoke evaluated three claim
groups, person health-navigation, provider affiliation, and facility service availability, and
every claim returned `satisfied: true`. You then called Registry Notary manually and received a
Notary claim result, not a FHIR `Evidence` resource: the source adapter performed the
FHIR-specific lookup, and Notary evaluated its configured claim rules against the projected
fields.
...
- [First run with Solmara Lab](../first-run-with-solmara-lab/)
