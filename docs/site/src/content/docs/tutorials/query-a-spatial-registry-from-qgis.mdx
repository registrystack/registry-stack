...
The installer places `breg` and `bregctl` together in `~/.local/bin` after checking both downloads
against the release `SHA256SUMS`. It supports Linux on amd64 and arm64 and macOS on Apple Silicon.
Replace `| bash` with `| less` to read the installer before you run it on a host you operate.
Before these binaries reach a host that serves anyone else, check them against the signed release
chain described in [OpenSSF and release trust](../../security/openssf-evidence/). If the installer
ends by asking you to add its directory to `PATH`, do that in both terminals before you continue.

The quickstart also runs Registry Mint, which the Evidence Gateway installer provides:
...
The quickstart launcher and the synthetic service-site project live in the repository, so clone it
at the tag that matches your installed binaries:
...
This project is pre-1.0, so its default branch may already carry configuration or API changes the
installed binaries do not implement. Pinning the checkout to the installed version keeps the
project and the binaries on one contract. If you already have a checkout, use it instead of
cloning, after confirming its `version` in `Cargo.toml` matches `bregctl --version`.

Run every shell command from the root of that clone. Keep two terminals open: the first runs Base
Registry Engine and Registry Mint, while the second performs the refresh write.
...
- [Deploy and operate a registry](../../operate/breg/) covers running Base Registry Engine
...
| The launcher exits before the banner and names `breg`, `bregctl`, or `mint` as required in `--installed` mode | The installer's directory is not on your `PATH`, or you skipped one of the two installers | Add `~/.local/bin` to `PATH` in this terminal, rerun the installer you skipped, then start the launcher again |
