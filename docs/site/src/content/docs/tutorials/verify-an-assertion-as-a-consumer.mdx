...
accepted = json.loads(Path("verified.json").read_text())
expectation = context["subjectExpectation"]
if expectation["mode"] != "accept-first-use":
    raise SystemExit("expected the first tutorial's first-use context")
subjects = [
    {"role": subject["role"], "binding": subject["binding"]}
    for subject in accepted["subjects"]
]
if sorted(subject["role"] for subject in subjects) != sorted(expectation["roles"]):
    raise SystemExit("the verified subject roles do not match the retained request")
policy = context["verificationPolicy"]
policy["expectedSubjects"] = subjects
...
- `verification-policy.json`, expectations retained from your own request and governance, with the
  first-use binding pinned from the already verified transaction record.

The tutorial extracts the key set and original policy from the private local context because it
recorded them before the response. First use could not contain a prior subject binding, so the script
adds that one opaque value from `verified.json`, which the first tutorial wrote only after local
verification succeeded. A production consumer atomically pins the same receipt in its own
transaction record before acting. Never decode an untrusted response and copy its claims into the
policy used to verify that same response.
...
- the request nonce and expected subjects, concepts, purpose, audience, provider, issuer, and
  configuration revision, including any first-use binding pinned from the verified result;
