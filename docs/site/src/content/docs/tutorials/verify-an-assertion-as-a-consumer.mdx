...
The command then prints the verified Evidence Gateway payload, which carries the same answer the first
tutorial verified:
...
It opens no listener, contacts no issuer, fetches no discovery document, and calls no source. The
named JWKS file is the complete trust set for this verification.
