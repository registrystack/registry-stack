...
`--require-runtime-dependencies` is what turns this into a check of the container rather than of
the files. Without it, the command compiles the bundle, compiles the source plans, refuses a
mounted extract already older than its source allows, and validates the mounted secret material.
With it, the command also opens the configured audit path, signs a self-test
message through the Transit proxy and verifies it against the governed public JWK, and resolves
every configured source credential. It appends no audit event in either form, so a check that fails
