...
[Relay operations runbook](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/ops.md)
...
The current RegistryStack release is `v0.8.3`, so the examples below use that tag.
Replace it with a newer release tag when you intentionally upgrade.
For the image signing policy, see the Relay
[security assurance notes](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/security-assurance.md)
and the [build and release section](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/ops.md#build-and-release)
of the operations runbook.

Pull the version you intend to run:

```sh
docker pull ghcr.io/registrystack/registry-relay:v0.8.3
```
...
[build and release section](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/ops.md#build-and-release)
...
[`config/example.yaml`](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/config/example.yaml)
and the
[configuration guide](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/configuration.md),
...
```sh
docker run --rm ghcr.io/registrystack/registry-relay:v0.8.3 generate-api-key --id program_system
```
...
binary.
The [API keys section](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/configuration.md#api-keys)
...
docker run --rm \
  ...
  ghcr.io/registrystack/registry-relay:v0.8.3
```
...
[readiness and probes section](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/ops.md)
...
```sh
docker pull ghcr.io/registrystack/registry-notary:v0.8.3
```
...
```sh
docker run --rm ghcr.io/registrystack/registry-relay:v0.8.3 generate-api-key --id notary_source
```

Store the emitted `fingerprint` for the Relay container's `NOTARY_SOURCE_API_KEY_HASH`
environment variable, and keep the raw `api_key`: it becomes Notary's source token below.
...
```sh
docker run --rm ghcr.io/registrystack/registry-notary:v0.8.3 hash-api-key --print-secret
```
...
docker run --rm \
  ...
  ghcr.io/registrystack/registry-notary:v0.8.3
```
...
The
[Operating with Registry Notary section](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/ops.md#operating-with-registry-notary)
...
- [Relay configuration guide](https://github.com/registrystack/registry-stack/blob/v0.8.3/crates/registry-relay/docs/configuration.md):
