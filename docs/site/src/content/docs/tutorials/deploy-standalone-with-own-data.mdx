...
[Relay operations runbook](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/ops.md)
...
Registry Relay release images publish to `ghcr.io/registrystack/registry-relay` from stable `vX.Y.Z`
tags.
...
[Relay releases page](https://github.com/registrystack/registry-stack/releases); at the time of writing
the current beta-5 release is `v0.5.0`, so replace `vX.Y.Z` with that (or a newer release) throughout
this page.
For the image signing policy, see the Relay
[security assurance notes](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/security-assurance.md)
and the [build and release section](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/ops.md#build-and-release)
of the operations runbook.

Pull the version you intend to run:

```sh
docker pull ghcr.io/registrystack/registry-relay:vX.Y.Z
```
...
[build and release section](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/ops.md#build-and-release)
...
[`config/example.yaml`](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/config/example.yaml)
and the
[configuration guide](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/configuration.md),
...
```sh
docker run --rm ghcr.io/registrystack/registry-relay:vX.Y.Z generate-api-key --id program_system
```
...
The [API keys section](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/configuration.md#api-keys)
...
docker run --rm \
  ...
  ghcr.io/registrystack/registry-relay:vX.Y.Z
```
...
[readiness and probes section](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/ops.md)
...
Registry Notary release images publish to `ghcr.io/registrystack/registry-notary` from stable `vX.Y.Z`
tags.
As with Relay, consume a version tag or an image digest, not `latest`.
The published versions are listed on the
[Notary releases page](https://github.com/registrystack/registry-stack/releases); at the time of
writing the current beta-5 release is `v0.7.0`, so replace `vX.Y.Z` with that (or a newer release) in
the commands below.

```sh
docker pull ghcr.io/registrystack/registry-notary:vX.Y.Z
```
...
```sh
docker run --rm ghcr.io/registrystack/registry-relay:vX.Y.Z generate-api-key --id notary_source
```
...
```sh
docker run --rm ghcr.io/registrystack/registry-notary:vX.Y.Z hash-api-key --print-secret
```
...
docker run --rm \
  ...
  ghcr.io/registrystack/registry-notary:vX.Y.Z
```
...
The
[Operating with Registry Notary section](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/ops.md#operating-with-registry-notary)
...
- [Relay configuration guide](https://github.com/registrystack/registry-stack/blob/v0.8.1/crates/registry-relay/docs/configuration.md):
