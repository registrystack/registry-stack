...
<QuickstartMeta
  ...
  prerequisites={['A registry-stack source checkout', 'The repository Rust toolchain', 'Bash', 'Python 3', 'Loopback ports 18440 through 18442 available', 'For the optional upstream check: Git, network access, npm, Java 17, Android SDK platform 34 with Build Tools 33.0.1, full Xcode, and an iPhone 15 simulator']}
/>
...
The ordinary receipt proves the checked-in fixture and Registry-side behavior. The separate
upstream receipt recorded on 2026-08-09 proves that the focused public tests from all three pinned
repositories also built and passed in the stated clean environment.
...
| The Android client preflight fails | Java 17, Android SDK platform 34, or Build Tools 33.0.1 is unavailable. | Install a Java 17 JDK, set `JAVA_HOME`, install the named Android packages, and set `ANDROID_HOME` or `ANDROID_SDK_ROOT`. |
| The Swift client preflight fails | The host lacks full Xcode or the iPhone 15 simulator runtime. | Install full Xcode, select it with `xcode-select`, accept its licence, and install an iPhone 15 simulator before rerunning on macOS. |
