...
Publishing, activating, and retiring a key each change the bundle revision you record at approval,
but no requirement's `configurationRevision`. Which keys sign is trust consumers take from the JWKS,
so they update their pinned key set, not the revisions their verification policies pin. Revoking a
key is the exception described below.
...
A key identifier in `signing.revokedKeyIds` changes every requirement's `configurationRevision`, so
give consumers the new revisions through the same emergency channel before they resume decisions.
