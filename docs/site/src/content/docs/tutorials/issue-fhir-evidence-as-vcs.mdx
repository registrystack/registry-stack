...
Token Verifiable Credential (SD-JWT VC) responses. One answer protects a patient by disclosing a
coverage result rather than a FHIR resource. The other demonstrates the same pattern for a
...
This tutorial works with this server as written. Its test-only origin override accepts only a
numeric-loopback HTTP endpoint so Registry Docs can replay the same steps against sanitized local
records. Another FHIR server may use different resource profiles, data, authorization, search
behavior, or JSON shapes. Do not replace only the server URL and assume the same evidence claim
remains valid.
...
numeric loopback under local assurance. Use this small adapter to preserve that boundary while
forwarding each governed read to the real SMART FHIR server. It contains no fixtures or seeded
responses, binds only to loopback, accepts only the two FHIR resource paths used below, denies
redirects, validates the returned resource identity, bounds the response, and suppresses request
logging so selectors do not enter logs.
...
This is a transport adapter, not a mock FHIR server. In the reader path, every Coverage or
Organization response used by Evidence Gateway is fetched live from `https://r4.smarthealthit.org`
at request time. The adapter asks the upstream server for FHIR JSON, validates the resource, and
exposes the same JSON body as `application/json`, which is inside Evidence Gateway's frozen v1
source contract. Both scripts ignore ambient proxy variables so selectors cannot be sent through
an undeclared intermediary.
...
- The credential-free source remained loopback-only and cannot be promoted as production source
  authentication.
