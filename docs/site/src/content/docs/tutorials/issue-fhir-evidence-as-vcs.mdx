...
class FhirReadThrough(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.send_body(200, body, "application/fhir+json")
...
by Evidence Gateway is fetched live from `https://r4.smarthealthit.org` at request time.
...
path per question, asks for `application/fhir+json`, applies response-size and timeout bounds, and
projects the response before the derivation runs. The adapter independently denies redirects and
validates the remote FHIR resource identity before returning it.
