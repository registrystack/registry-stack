...
This is a transport adapter, not a mock FHIR server. Every Coverage or Organization response used
by Evidence Gateway is fetched live from `https://r4.smarthealthit.org` at request time. The
adapter asks the upstream server for FHIR JSON, validates the resource, and exposes the same JSON
body as `application/json`, which is inside Evidence Gateway's frozen v1 source contract.
