...
<QuickstartMeta
  ...
  prerequisites={['registryctl 0.8.2+', 'A Docker Compose provider', 'curl']}
/>
...
Use `registryctl` 0.8.2 or newer.

The `v0.8.2` release publishes binaries for Linux x86_64, Linux arm64, and macOS arm64.
...
```sh
curl -fsSL https://raw.githubusercontent.com/registrystack/registry-stack/v0.8.2/crates/registryctl/install.sh | REGISTRYCTL_VERSION=v0.8.2 sh
```

The installer downloads the pinned release selected by `REGISTRYCTL_VERSION`.
It defaults to `v0.8.2` in the current stack release.
