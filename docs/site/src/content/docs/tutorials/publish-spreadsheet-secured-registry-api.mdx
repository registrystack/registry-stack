...
<QuickstartMeta
  ...
  prerequisites={['registryctl 0.10.0', 'A Docker Compose provider', 'curl']}
/>
...
The v0.10.0 source beta and this local tutorial can be used while hosted credential publication is
...
`SHA256SUMS`. The pinned `v0.10.0` installer installs the binary and matching
...
| Docker reports no `linux/arm64` manifest for a Registry Stack image | The v0.10.0 Relay and Notary images are published for `linux/amd64` only. | Start with `DOCKER_DEFAULT_PLATFORM=linux/amd64 registryctl start`. |
