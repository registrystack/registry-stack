...
<QuickstartMeta
  ...
  prerequisites={['registryctl 0.8.0+', 'A Docker Compose provider', 'curl']}
/>
...
Use `registryctl` 0.8.0 or newer.

The `v0.8.0` release publishes a Linux x86_64 binary.
...
It defaults to `v0.8.0` in the current stack release.
