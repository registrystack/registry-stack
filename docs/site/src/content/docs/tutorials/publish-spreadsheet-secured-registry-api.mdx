...
<QuickstartMeta
  ...
  prerequisites={['registryctl 0.12.1', 'A Docker Compose provider', 'curl']}
/>
...
The v0.12.1 source beta and this local tutorial can be used while hosted credential publication is
...
`SHA256SUMS`. The pinned `v0.12.1` installer installs the binary and matching
...
```sh
curl -fsSL https://raw.githubusercontent.com/registrystack/registry-stack/refs/tags/v0.12.1/crates/registryctl/install.sh | REGISTRYCTL_VERSION=v0.12.1 bash
registryctl --version
```
...
`v0.12.1`, also checksum-verify the matching release image lock and
...
| Docker reports no `linux/arm64` manifest for a Registry Stack image | The v0.12.1 Relay and Notary images are published for `linux/amd64` only. | Start with `DOCKER_DEFAULT_PLATFORM=linux/amd64 registryctl start`. |
