...
<QuickstartMeta
  ...
  prerequisites={['registryctl 0.9.0', 'A Docker Compose provider', 'curl']}
/>
...
`SHA256SUMS`. The pinned `v0.9.0` installer installs the binary and matching
...
- [Hosted credential tour (held)](../../start/credential-tour/): inspect the hosted issuance path;
  it is not the promoted first-run route.
