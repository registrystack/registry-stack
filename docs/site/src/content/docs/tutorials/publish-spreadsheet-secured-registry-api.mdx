...
<QuickstartMeta
  ...
  prerequisites={['registryctl 0.8.1+', 'A Docker Compose provider', 'curl']}
/>
...
Use `registryctl` 0.8.1 or newer.

The `v0.8.1` release publishes a Linux x86_64 binary.
On other platforms (macOS, and Linux arm64), install from source with Rust or run the tutorial
from a Linux x86_64 environment such as a VM or container.
...
It defaults to `v0.8.1` in the current stack release.
