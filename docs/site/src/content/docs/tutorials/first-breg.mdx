...
<QuickstartMeta
  ...
  prerequisites={[
    'Linux amd64 or arm64, or macOS on Apple Silicon',
    'A Bash or zsh shell',
    { label: 'Running Docker', href: 'https://docs.docker.com/get-started/get-docker/' },
    { label: 'curl 7.76 or later', href: 'https://curl.se/download.html' },
    { label: 'Python 3, to format JSON responses', href: 'https://www.python.org/downloads/' },
    'An editor',
  ]}
/>
...
| You opened a fresh terminal | Run the `registry_url` and `authorization_header` assignments from [Get a token](#get-a-token) again. The header file and `tutorial-work` directory are still there. |
