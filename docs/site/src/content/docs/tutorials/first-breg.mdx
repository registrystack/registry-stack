...
| `bregctl dev` refuses over a reported version | The `breg` or `mint` it resolved comes from another release than `bregctl`. The refusal names both versions. Install all three from the same release, or put the matching build first on `PATH`. |
