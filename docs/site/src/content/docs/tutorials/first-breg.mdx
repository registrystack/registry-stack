...
```sh
bregctl dev --project tutorial-work/project \
  --clients-file tutorial-work/project/dev-clients.yaml
```
...
What happened: `dev` started PostgreSQL in a container, started Registry Mint with the two clients
from `dev-clients.yaml` and a fresh private key for each, built a package from the project, replayed
...
```sh
bregctl dev stop --project tutorial-work/project
```
...
```sh
bregctl dev stop --project tutorial-work/project --remove
```

Either way, keep `tutorial-work/`: the next tutorial edits the project inside it, and the keys,
credentials, and package under `.breg/dev/` serve the next start.
