...
The two callers in `dev-clients.yaml`, `operator` and `reader`, are ready to use with no edits, and
...
`dev` started PostgreSQL in a container, registered the two clients from `dev-clients.yaml` with
