...
The report contains `"status": "valid"`. Review the fixed source authority, required caller
scopes, consultation input mapping, output projection, claims, disclosure modes, and required
review classes.
...
The report contains `"status": "built"`. The output has separate Relay and Notary Config Bundle
input directories under `.registry-stack/build/local/private/`. It contains secret references,
not secret values. Package, sign, verify, and activate each product input through that product's
...
- Use [Configure a script source adapter](../configure-project-sandboxed-rhai/) to review the
