...
tutorial_test:
  toolset: evidence
  after: tutorials/return-a-governed-value
---
...
<QuickstartMeta
  outcome="Two applications receive different policies, with generation-bound onboarding, refusal, and revocation."
...
The pinned local issuer authenticates each application from its own key and issues a short-lived access
...
part of the local issuer generation:
...
The local issuer recognizes `age-checker`. Evidence Gateway has both access policies available, even though
...
Stop the current session, then register `service-router` under the existing `service-routing`
policy:
...
The successful token exchange proves that the new issuer generation accepted the client registry.
...
The command updates `access/clients/age-checker.yaml`. The next issuer generation excludes the
revoked client.
...
`evidencectl` rejects an unknown or revoked named client locally, before contacting the issuer
...
The revoked-client preparation remains local and creates no Evidence Gateway audit event because no
...
- Revoking a local client excludes it from the next issuer generation and prevents local request
  preparation.
