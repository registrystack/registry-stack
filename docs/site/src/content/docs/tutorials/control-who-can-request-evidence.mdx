...
You will give two applications different access to that Evidence service, add one while the service
...
selectors, or returned values. The earlier authorization refusal is not retained in the durable
Evidence audit. Evidence currently returns that refusal before appending an access-attempt event.
