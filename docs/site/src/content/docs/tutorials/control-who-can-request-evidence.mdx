...
| --- | --- |
| Add or revoke a client | Registry Mint receives a reload request; Evidence does not restart |
...
new registry. The next token exchange provides that functional proof. Evidence does not restart
...
The question, purpose, and subject are unchanged. Changing only the authenticated application
changes the access decision because `service-router` belongs to the required policy.
...
without restarting Evidence. Revocation cannot erase a stateless access token already issued to
...
selectors, or returned values. The earlier entitlement refusal is not retained in the durable
Evidence audit. Evidence currently returns that refusal before appending an access-audit event.
The revoked-client preparation also has no Evidence audit event because no HTTP request was sent.
...
- A client added while the services run can obtain a token without restarting Evidence or
  Registry Mint.
