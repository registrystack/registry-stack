...
<QuickstartMeta
  ...
  prerequisites={[
    { label: 'The binaries from Create and query your first registry', href: '/tutorials/first-breg/#install-base-registry-engine' },
    { label: 'Running Docker', href: 'https://docs.docker.com/get-started/get-docker/' },
    'An editor',
  ]}
/>
...
Each finding prints a sentence of its own and one path per place it fires. `init` compiled the project before printing its `revision`, the digest of
...
revision e5cfbbc0161e4d2c802d1acd752ec193253f7820
...
```diff lang="yaml" title="household-registry/model/selection.yaml" test-edit
 - concept: Person
   properties:
   - name: given_name
   - name: family_name
...
`marital_status` removed the other one:

```text test-excerpt
finding  access.profile.higher_classification  entities[id=group-membership].accessProfiles[id=operator].readableFields[field=person]
         this field is more sensitive than its entity's classification; verify the
         profile's scope and purpose before disclosing it
```

The `person` field table in `household-registry-2/README.md`
...
| Symptom | Next move |
| --- | --- |
| `bregctl dev` refuses with `dev.failed` over a reported version | The installed `breg` comes from another release than this `bregctl`. The refusal names the file that answered, the version it reported, and the version this `bregctl` reports. Install both from the same release, or put the matching build first on `PATH`. |
