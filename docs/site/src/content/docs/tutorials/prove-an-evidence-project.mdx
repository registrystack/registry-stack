...
fixture the project references must declare at least one case`, because a run that decided nothing
proved nothing. Production and evidence-grade assurance require complete fixture coverage before the
bundle loads.
...
Evidencectl compiles the editable project into a private staging bundle, runs `evidence check`
against it, discovers the fixture files the compiled bundle captured, and drives each one through
`evidence evaluate`. It does not start HTTP, resolve a source credential, call the source, or write
