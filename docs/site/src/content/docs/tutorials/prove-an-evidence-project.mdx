---
title: Prove and promote an Evidence project
description: Add fixtures that describe your own requirement, run them through the production evaluator, then promote the exact reviewed bundle.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
promotion. Do not convert a real failure into an ignored case.
...
## Promote the exact reviewed revision

Evidencectl deliberately has no generic `promote` command. Your deployment system owns approval,
artifact transport, rollback, and environment binding.

Promote only when these facts are recorded together:

1. The fixture suite passed against the candidate bundle.
2. `evidence check` accepted the exact frozen runtime and bundle.
...
5. The deployed bundle digest matches the reviewed candidate.
