---
title: Verify a registered parent with OpenCRVS
description: >-
  Connect Evidence to the public OpenCRVS demo and verify a registered-parent relationship
  without disclosing civil-registration identifiers.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
Connect Evidence to the public OpenCRVS Farajaland demo and answer, "Is this candidate registered
as a parent of this child?" The request identifies both people, but the signed assertion contains
only a boolean answer and two pseudonymous subject bindings.

<QuickstartMeta
  outcome="A verified registered-parent assertion derived from a synthetic OpenCRVS birth."
...
The Evidence question has two distinct subject roles:

- `child`, selected by the child's national ID
- `candidate-parent`, selected by the candidate's national ID

The source searches for one registered birth by `child.nid`, reads the bounded `mother.nid` and
`father.nid` fields when present, and compares them with the candidate. It does not return either
national ID to the requester.
...
fn extract(source_response, parameters) {
    ...
    if declaration.contains(mother_field) {
        ...
        parent_ids.push(declaration[mother_field]);
    }
    ...
    if declaration.contains(father_field) {
        ...
        parent_ids.push(declaration[father_field]);
    }
...
These facts are the complete input to the relationship logic. Names, dates, addresses, and other
birth fields cannot reach the derivation.
...
fn answer(facts, selectors, context) {
    ...
    let confirmed = false;
    for parent_id in facts.registered_parent_national_ids {
        if parent_id == candidate { confirmed = true; }
    }
    #{relationship_confirmed: confirmed}
}
```

The first check binds the returned birth to the requested child. The loop compares the candidate
with at most two authenticated parent identifiers, then returns only the declared boolean.
...
The audit identifies the authorized question, purpose, requester pseudonym, decision, and
disclosed concept. It does not record either selector, the OpenCRVS token, response, or boolean.
