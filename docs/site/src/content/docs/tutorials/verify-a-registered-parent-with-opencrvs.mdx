...
description: >-
  Connect Evidence to the public OpenCRVS demo, verify a registered-parent relationship, and
...
Connect Evidence to the public OpenCRVS Farajaland demo and answer two questions from the same
registered birth. First verify, "Is this candidate registered as a parent of this child?" Then
return the recorded mother and father national IDs for a caller authorized to receive them.
...
Create an editable Evidence project and disposable local keys:

```sh
evidencectl new project \
...
`record-transformed` states that OpenCRVS returns a bounded birth record and Evidence reduces it
...
Evidence resolves these logical secret references beneath that directory. It does not read the
...
fn extract(source_response, parameters) {
    ...
    if declaration.contains(mother_field) {
        ...
        parent_ids["mother_national_id"] = declaration[mother_field];
    }
    ...
    if declaration.contains(father_field) {
        ...
        if parent_ids.contains("mother_national_id") &&
           parent_ids["mother_national_id"] == father_id {
            throw("source_protocol_error");
        }
        parent_ids["father_national_id"] = father_id;
    }

    if parent_ids.len == 0 || parent_ids.len > 2 {
        throw("source_protocol_error");
    }
    ...
    #{
        ...
        facts: #{
            ...
            registered_parent_national_ids: parent_ids
        }
    }
}
```
...
type: object
additionalProperties: false
required: [child_national_id, registered_parent_national_ids]
properties:
  child_national_id:
    type: string
    minLength: 1
...
fn answer(facts, selectors, context) {
    ...
    let parents = facts.registered_parent_national_ids;
    let confirmed =
        (parents.contains("mother_national_id") &&
         parents["mother_national_id"] == candidate) ||
        (parents.contains("father_national_id") &&
         parents["father_national_id"] == candidate);
...
This closed schema preserves each parent's role and rejects extra fields. Either identifier may
be absent from a registration, but Evidence will not issue the assertion unless the source
establishes at least one authenticated parent identifier.
...
fn answer(facts, selectors, context) {
    ...
    #{
        registered_parent_national_ids: #{
            ...
            fields: facts.registered_parent_national_ids
        }
    }
}
```

Evidence validates the returned object against the reviewed schema before signing it. The
derivation cannot add names, dates, addresses, or any other birth-registration field.

## Start the project

Compile the editable artifacts into a private local generation and start Evidence with Registry
Mint:
...
`dev` runs the real Evidence configuration check before either service becomes ready. Unresolved
draft markers, invalid schemas, inconsistent roles, or missing credentials stop the start.

## Send a real Evidence request
...
Use this only with the public synthetic record. In a real integration, selectors need an input
...
Evidence exchanges its client credentials for an OpenCRVS token, performs the fixed child search,
...
For a synthetic registration with two authenticated parent identifiers, `fields` contains both
`mother_national_id` and `father_national_id`. You can create such a registration in the demo and
prepare the same question with that child's national ID.
...
Delete the Record Search client in OpenCRVS when you finish. Remove the two local credential files
when you no longer need the project.

## Next

- [Issue a birth certificate VC from OpenCRVS](../issue-a-birth-certificate-vc-from-opencrvs/)
