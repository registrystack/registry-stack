---
title: See Evidence refuse unsafe changes
...
A useful evidence service must fail safely, not only succeed. Reuse the age-bracket project to
observe two boundaries: Evidence denies a purpose the provider did not authorize, and the verifier
rejects a response changed after signing.
...
<QuickstartMeta
  ...
  prerequisites={[
    'The adult-status project after completing the governed age-bracket tutorial',
    'The registry.py source from the first Evidence tutorial',
    'Python 3 and curl',
  ]}
/>
...
The provider authorized `service-path-selection`. Preparation also obtained short-lived local
authorization for that exact governed path.
