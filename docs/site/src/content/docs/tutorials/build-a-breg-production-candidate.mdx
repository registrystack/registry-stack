...
tutorial_test:
  toolset: breg
  skip: needs a reader-supplied signing key and a production database; product CI builds the candidate
---
...
The runtime configuration in this source-preview tutorial uses the unreleased shared audit writer.
Use the [matching source-built tool pair](../../operate/breg/#obtain-the-runtime) before continuing;
the released installer from the earlier tutorial does not supply this contract.
...
The report repeats, its lead sentence now reading `Sealed and published a deployment package.`, with `state` at `published` and `provided signatures` at `1`. The package is in
