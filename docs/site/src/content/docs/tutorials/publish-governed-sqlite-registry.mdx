...
It reports the files it created. Every `relayctl` command prints its own name on the first
line and then a JSON report; the blocks in this tutorial show the report:
...
The report lists every table, view, index, and column, and opens with a fingerprint of the whole
schema. `objects` is abridged here; your run prints the full list:
...
```json
{
  ...
  "details": {
    ...
    "fingerprint": "sha256:<your-schema-fingerprint>",
    "objects": [ ... ]
  }
}
```
...
run, so a value that differs from one printed in a tutorial means the schema text differs, not
...
exist, and reports the revision of what it compiled. The real report also carries a
`configuration_key_paths` listing of every key the two documents accept, which is left out here:
...
The starter project is refused, and the diagnostics say exactly why. Each one also carries a
`severity`, dropped here for width; all five below are errors:
  ...
  "details": {
    ...
    "manifest": {
      ...
      "sourceSchemaFingerprints": {
        "registry": "sha256:<your-schema-fingerprint>"
      }
    }
  }
}
...
That manifest is abridged: the real one also carries the package format version and the full
observed schema of every source. The package holds the compiled contract, the generated
artifacts, and the governed files, with a digest for each. It does not hold the database. Packaging recompiles under the production
