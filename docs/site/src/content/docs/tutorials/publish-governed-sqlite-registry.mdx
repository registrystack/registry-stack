...
Authoring check passed.
  contract revision   sha256:00c95e6d12a344b5baba27f47c44d084c541ae7f8e62181b8931a7c8c63fccba
  registry key paths  121
  runtime key paths   19
```
...
Production check passed.
  contract revision   sha256:b6d31cf523c849c88dc2a413f75207382ef675c074138a7907ed74307fe0479c
  registry key paths  121
  runtime key paths   19
```

The contract revision is not the one the first check reported. The revision covers the governed
...
```text
Sealed a deployment package. 17 artifacts, 25 files.
  package version    relay.registrystack.org/package/v1alpha3
  package revision   sha256:bdf7adaf89e3ce07cda35a76ef9670a3d9de70a70641ce7448bf22d79ce9ae16
  contract revision  sha256:b6d31cf523c849c88dc2a413f75207382ef675c074138a7907ed74307fe0479c
  artifact bindings  1
...
That report summarizes `package/relay-package.json`, which names all 25 files and 17 artifacts
with a digest for each and records the full observed schema of every source. The package holds
the compiled contract, the generated artifacts, and the governed files. It does not hold the
database. Packaging recompiles under the production profile, so a package cannot be produced
from a revision that would fail `check --production`.
...
```json
{
  ...
  "meta": {
    ...
    "contractRevision": "sha256:b6d31cf523c849c88dc2a413f75207382ef675c074138a7907ed74307fe0479c",
