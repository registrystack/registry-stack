...
<QuickstartMeta
  ...
  prerequisites={['The sqlite3 command line', 'A shell with curl', 'An editor', 'Linux or macOS']}
/>
...
Install the runtime and the authoring tool:
...
The installer provides `relayctl`, which you use to author and check a project, and `relay`,
which serves a sealed package. Both land in `~/.local/bin` unless you set `RELAY_INSTALL_DIR`.
If the version commands are not found, add that directory to your `PATH`.
...
Read what is not there. The line records that an anonymous caller was answered with the
properties `legalName` and `legalForm` under the `consultation` processing description. It does
not record what the values were. `prev_hash` is `null` on the first line and links each
subsequent line to the one before it, which is what the integrity key protects.
