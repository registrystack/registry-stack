...
keys because in a real deployment they often are not the same body, and the audit log is only
...
resources:
  - id: registered-business
    datasetIdentifier: businesses
    entityTypeIdentifier: business
...
`datasetIdentifier` and `entityTypeIdentifier` are not inferred from `id`, so a cursor stays scoped
...
Its identifier is what the audit log records against each released answer, so a released field
...
`method: manual` is the honest description of what you did: a person read the inventory and
...
Create the audit directory, then start the service:
...
to start when any group or other bit is set on it. Only one Relay process may write
`var/audit.jsonl` at a time: a second one started against the same file refuses to start.
...
The full `record` carries more than is shown: the registry identifier, the operation identifier
repeated as `operationId`, the revision of the access rules that were applied, the operation
surface and wire format, the trace identifier shared with the log line, and the handling levels the
...
Read the phase too. The envelope's `request` phase and the record's `attempt` phase are written
before Relay reads the source, so this line proves the request was accepted and scoped, not that
it was answered. The next line with the same `correlation` is a `response` entry whose record
phase is `terminal`, carrying the outcome; a refusal is written as `refusal` instead. The lines are
not chained or signed, so a deployment ships them to append-only storage to make them
tamper-evident.
...
- An audit log that records which properties were released, to whom, under which contract
...
| `the required audit destination is not ready` at startup | `var/` is not owner-only, or another Relay process already writes `var/audit.jsonl` | `chmod 700 var`, and stop the other process. |
