...
`changesRequested` review result; the submitter follows the request's `revise_request` action,
patches the draft, and submits it again, and Casework opens a fresh review for the new proposal
version.
