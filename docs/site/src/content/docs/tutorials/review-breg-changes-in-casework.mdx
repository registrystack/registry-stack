...
`revise_request` action with the `rebase` value it carries, so BReg records the new draft as a
revision, patches the draft, and submits it again, and Casework opens a fresh review for the new
proposal version.
