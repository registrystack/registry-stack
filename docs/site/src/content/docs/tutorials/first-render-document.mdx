...
```text
render 0.32.0-dev (typst 0.15.1)
```
...
```sh
target/release/render init render-work/bundle
```
...
fonts with their license, a vendored QR package, and a fixture payload. The template is plain
Typst: open `render-work/bundle/templates/letter.typ` in any Typst-aware editor and the same file
renders in both.

## Render offline

```sh
target/release/render compile \
...
```sh
target/release/render seal --bundle render-work/bundle
target/release/render check --bundle render-work/bundle
```

`render seal` writes a per-file sha256 for every governed file into the bundle's manifest, and
`render check` verifies structure, hashes, label script coverage, and label key sets. You know both
worked when check prints the document line and the bundle line:
...
```sh
target/release/render serve --runtime render-work/deploy/runtime.yaml &
...
ledger. Verification covers sealed segments, and the writer holds the active one, so verify after
shutdown:

```sh
target/release/render audit-verify --runtime render-work/deploy/runtime.yaml
```
