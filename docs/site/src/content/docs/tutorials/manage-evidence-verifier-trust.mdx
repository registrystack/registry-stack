...
tutorial_test:
  toolset: evidence
  skip: how-to against the reader's own deployment; no fixed scenario to replay
---
...
A provider's planned rotation or emergency key revocation leaves every requirement's
`configurationRevision` unchanged, so a revision that moves during either is a separate change to
review, not part of the key change.
