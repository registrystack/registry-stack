...
A planned provider rotation leaves every requirement's `configurationRevision` unchanged, so a
revision that moves during one is a separate change to review, not part of the rotation.
