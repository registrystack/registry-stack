...
description: Bind a fixed API-key header or query credential to a country bounded HTTP integration without placing its value in authored or generated files.
status: draft
owner: registry-docs
source_repos:
  - registry-stack
...
Use this guide when a reviewed source API requires a fixed API key for one country bounded HTTP
integration.
...
{/* Evidence: crates/registryctl/src/country_authoring.rs,
    crates/registryctl/tests/country_authoring.rs, and
...
- A country project that passes the ordinary
  [bounded HTTP authoring journey](../author-country-integration/).
...
In the integration's `capability.bounded_http` block, replace the credential interface:
...
Bind the private value and review class in the environment:
...
The first report contains `"status": "passed"`.
The second contains `"status": "valid"` and lists `operator_security` among the required reviews
for a query credential.
...
| `operator_security` is missing | The query credential has no explicit review declaration. | Add the review declaration and rerun `check --explain`. |
...
- Return to [Author a bounded HTTP country integration](../author-country-integration/) to finish
  fixture, review, and build steps.
- Use [Configure a FHIR R4 integration](../configure-country-fhir-r4/) for a fixed FHIR search
