...
Answer, "Is this candidate registered as a parent of this child?" This question differs from the
first two tutorials because one assertion binds two people in distinct roles. Evidence must not
let a caller omit a role, repeat one role, or swap its selector field.
...
<QuickstartMeta
  ...
  prerequisites={['Evidence toolset', 'Python 3', 'A shell with curl', 'Linux or macOS']}
/>
...
the one governed answer. Evidence still validates the source response, answer form, disclosure,
...
- [Verify a registered parent with OpenCRVS](../verify-a-registered-parent-with-opencrvs/)
