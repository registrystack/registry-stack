...
```python title="registry.py" test-file
import json
...
```sh test-background="http://127.0.0.1:8002/openapi.json"
python3 registry.py
```
...
```sh
evidencectl init parent-relationship \
...
```yaml title="questions/parent-relationship.yaml" test-file
...
```rhai title="derivations/parent-relationship.rhai" test-file
...
ACCESS AUTHORIZED parent-relationship relationship-check requester=<pseudonym>
...
This tutorial needs three loopback ports: `8002` for `registry.py`, plus the Evidence Gateway and
stock identity-provider ports. If another tutorial's services are still running,
`evidencectl dev start .` cannot bind. Stop them with `evidencectl dev stop` in that project, or
start this one on two unused ports with `--evidence-port` and `--issuer-port`.

Port `8002` appears twice inside `registry.py`, in the OpenAPI `servers` URL and in the listener,
and again in the `--openapi` argument. Change all three before running `evidencectl init`, because
the project retains the server URL it was created from.
