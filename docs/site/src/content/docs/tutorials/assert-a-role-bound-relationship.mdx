...
This tutorial needs three loopback ports: `8002` for `registry.py`, and `8080` and `8081` for
Evidence Gateway and Registry Mint. If another tutorial's services are still running,
`evidencectl dev start .` cannot bind. Stop them with `evidencectl dev stop` in that project, or
start this one on two unused ports with `--evidence-port` and `--mint-port`.
