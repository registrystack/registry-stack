---
title: Issue a birth certificate VC from OpenCRVS
description: >-
  Map a synthetic OpenCRVS birth into a governed structured value, request it as SD-JWT VC, and
  verify the credential across the Evidence HTTP boundary.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
Extend the OpenCRVS project with a birth-record question and request the answer as a Selective
Disclosure JWT Verifiable Credential (SD-JWT VC). The credential exposes `givenName`, `familyName`,
`dateOfBirth`, and `placeOfBirth` as four independently disclosable fields.
...
Evidence produces a governed extract from a registered birth:
...
certificate. Evidence proves which provider signed the configured assertion. It does not grant
...
Continue in the `project` directory created by
...
`birthCertificate` is a configured JSON claim name. Evidence has no built-in birth-certificate
...
Evidence validates the complete value against the schema before mapping any field to an SD-JWT
...
Send the retained request across the real Evidence boundary:
...
Evidence authenticates the caller, authorizes the exact question and purpose, obtains an OpenCRVS
...
Inspect the verified Evidence payload:
...
Evidence's V1 verifier checks the complete issued credential. Wallet presentation, omission of
...
- [Request another Evidence assertion as SD-JWT VC](../request-evidence-as-sd-jwt-vc/)
