...
installed binaries, and you want to know what leaves a registry when a record changes.
...
<QuickstartMeta
  ...
  prerequisites={['The binaries from Create and query your first registry', 'git', 'Running Docker', 'Python 3', 'curl 7.76 or later', 'Two terminals']}
/>
...
The demo launcher and its receiver live in the repository, so clone it at the version you installed:
...
Create a directory inside the checkout for the files this tutorial keeps; the checkout's `.gitignore`
covers it:
...
In the first terminal, at the root of the `breg-tutorial` checkout and with the registry from the first
...
After `Explained the compiled inventory.` and the project's findings, the report has one delivery,
