...
DHIS2 stores an immunization reading on its event when a health worker enters the reading, and
keeps a Child Programme event `ACTIVE` until the whole visit is closed.
Reading only `COMPLETED` events would discard most of the readings the public demo holds, so the
source adapter accepts Birth and Baby Postnatal events in both statuses.
        ...
        if enrollment["program"] == "IpHINAT79UW" {
            ...
            if !is_missing(events) {
                for event in events {
                    ...
                    // DHIS2 stores a reading on its event when a health worker
                    // enters the reading, and keeps the event ACTIVE until the
                    // whole visit is closed. Both statuses carry a recorded
                    // reading, so the extractor reads both.
