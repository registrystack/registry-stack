...
In the public demo, the Child Programme's immunization readings sit almost entirely on `ACTIVE`
events, so a `COMPLETED`-only rule returns no assertion for effectively every child.
