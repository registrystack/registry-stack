...
The v0.9.0 source beta and this local tutorial can be used while hosted credential publication is
...
- [Hosted credential tour (held)](../../start/credential-tour/): inspect credential issuance and
  cross-authority flows; this is not the promoted first-run route.
...
| Docker reports no `linux/arm64` manifest for a Registry Stack image | The v0.9.0 Relay and Notary images are published for `linux/amd64` only. | Start with `DOCKER_DEFAULT_PLATFORM=linux/amd64 registryctl start`. |
