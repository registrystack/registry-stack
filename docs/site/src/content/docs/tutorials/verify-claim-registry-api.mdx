...
<QuickstartMeta
  ...
  prerequisites={['The unmodified my-first-api project from the first tutorial', 'Registryctl 0.15.2 with its matching image lock', 'Linux amd64', 'Docker Engine with Compose v2', 'curl', 'Ports 4242 and 4255 available on loopback'] }
/>
...
| --- | --- | --- |
| `registryctl add notary` reports an unsupported shape | The project differs from the maintained spreadsheet starter or already has a conflicting Notary integration. | Start from the unmodified `my-first-api` project created by Registryctl 0.15.2. |
