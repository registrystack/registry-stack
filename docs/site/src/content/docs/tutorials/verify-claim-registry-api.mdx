...
PASS notary evaluator can verify starter claim
```
...
The `request_id` is a server-minted ULID that differs on every request; match the other
fields exactly.

This distinction matters when you integrate: a claim result answers the configured question about a
...
- [Registry Notary operator configuration reference](../../products/registry-notary/operator-config-reference/):
  define more policy for this claim, including context constraints, denial reasons, and audit
  inspection.
