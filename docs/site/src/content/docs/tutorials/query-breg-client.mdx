...
<QuickstartMeta
  ...
  prerequisites={['Python 3.10 or later, or Node 22.12 or later', 'The registry from Create and query your first registry, still running', 'The breg and mint binaries on your path']}
/>
...
The first tutorial wrote the token into a header file for `curl`; the client wants the bare token.
It lasts five minutes, and the install takes longer than that, so mint one now, as the `operator`
client, and again whenever a call fails with status `401`:

```sh
credentials="$PWD/tutorial-work/project/.breg/dev/credentials/operator"
umask 077
...
Success prints nothing. The file is readable only by your user; the programs read it and never print it.
