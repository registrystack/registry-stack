...
```yaml title="questions/age-bracket.yaml" test-file
id: age-bracket
...
```rhai title="derivations/age-bracket.rhai" test-file
...
`evidencectl dev start .` refuses a project that still holds a local session, reporting
...
```sh test-skip="recovers a session the first tutorial left running; the replay stopped it"
evidencectl dev stop
evidencectl dev clean
```

If ports `8080` or `8081` belong to something else, pass `--evidence-port` and `--issuer-port` with
