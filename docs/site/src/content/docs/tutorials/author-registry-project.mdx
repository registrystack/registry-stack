...
the exact starter embedded in the current registryctl release:
...
output, claim results, and derived security cases. The report starts with:
...
Relay, Registry Notary, or a source registry. Add `--format json` when another program needs the
...
The build report identifies
`registry-project/.registry-stack/build/local/artifact-manifest.json`. Treat that strict
`registry.project.artifact_manifest.v1` document as the complete tree and classification source.
It records every reviewable, private, operations, secret-consumer, approval-state, Relay, and
Notary artifact plus its digest, edit policy, publication boundary, consumers, and regeneration
command. Do not maintain or automate against a copied directory listing.
