...
{/* Evidence: crates/registryctl/src/main.rs, crates/registryctl/src/project_authoring.rs,
...
| `integrations/person-record/integration.yaml` | `source.versions.unverified` | `[project-fixture-v1]` | `[tutorial-contract-v2]` |
| `integrations/person-record/integration.yaml` | the `person` request `path` | `/v1/person-records` | `/v2/citizens/status` |
| `environments/local.yaml` | `source_version` | `project-fixture-v1` | `tutorial-contract-v2` |
...
2. In `integrations/person-record/integration.yaml`, define the typed subject input, fixed
   operation, closed response schema, output mapping, cardinality, bytes, calls, deadline, and
   concurrency.
3. In `integrations/person-record/fixtures/`, replace the synthetic source observations and
   expected outputs and claims. Keep positive and denial fixtures.
...
Review the fixed operations, input mappings, caller scopes, purpose, outputs, claims, disclosure
...
- [Configure Sandboxed Rhai orchestration](../configure-project-sandboxed-rhai/) only when the
