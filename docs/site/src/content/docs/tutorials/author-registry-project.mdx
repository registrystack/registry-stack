...
This tutorial uses Registryctl 0.15.2 and its matching release material. Do not
mix the page with an older CLI, image lock, or starter.

One project defines one registry trust domain. A deployment can use Relay only
for governed materialization and records APIs, Notary without configured
claims, or both products for consultations and claims derived from Relay outputs. Independent
...
<QuickstartMeta
  ...
  prerequisites={['registryctl 0.15.2 and its matching image lock', 'A shell and a text editor']}
/>
...
Initialized Registry Stack project "fictional-citizen-registry".
  ...
  Starter: http (Registry Stack 0.15.2)
...
the exact starter embedded in the installed release:
