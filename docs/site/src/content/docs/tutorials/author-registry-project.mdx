...
The authored fixture set covers meaningful synthetic match, no-match, ambiguity, and source-shape
variations. The harness derives malformed decoding, byte and structural limits, timeout,
authorization-before-source, and output-minimization cases from the same integration.
...
- [Configure a script source adapter](../configure-project-sandboxed-rhai/) only when the reviewed
