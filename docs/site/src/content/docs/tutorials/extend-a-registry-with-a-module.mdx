...
| `bregctl dev` refuses to start the edited project | Expected: `dev` keeps the package it started with, and refuses inputs that differ from it. To run the edited project, copy `tutorial-work/project` without its `.breg/` directory to a new path and start that copy with its own `dev-clients.yaml`. |
