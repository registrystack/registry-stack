...
the README, the findings the example keeps on purpose, and the placeholder `canonicalBaseIri` that has
...
The command prints `check succeeded`, a new `revision`, and the findings.
...
`check` then prints `check succeeded` with the findings and nothing else.
