...
paths above: no shipper-acknowledgement cursor file, no server-side ingest cache, no
configuration-signing or anti-rollback state, and no separate consultation database. Relay V1 had
all four; the [Relay recovery state](#relay-recovery-state) section explains why none of them carry
forward.
...
the product-level expiry checks above have run.
