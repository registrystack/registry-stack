...
Relay writes chained JSONL audit records through the same segmented sink Evidence Gateway uses, sealing full segments at a fixed `64 MiB` and never deleting a sealed segment
itself.
...
or `relayctl` command that replays the chain: unlike Evidence Gateway's `evidence verify-audit`, Relay V2 has no offline chain-integrity command. Verify
