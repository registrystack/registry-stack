...
- Treat local audit files as tamper-evidence for the retained records, not as a completeness
  ...
  Use off-host shipping when completeness matters, and use append-only storage or retained
  tail-hash anchors to make retained local records harder to rewrite unnoticed.
...
The chain detects edits, deletions, insertions, and reordering within the retained records.
It does not prove that older rotated-away records still exist.
