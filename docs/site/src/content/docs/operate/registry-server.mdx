...
is already complete, when the registry is not ready, when a retained revision is not indexed by a
commit, and when a live row has no retained journal head that reproduces it. The result and the
audit record carry counts and positions, not values.
...
| A fresh snapshot reference is refused after an erasure | Coverage ends before the erased commit. Run `history rebaseline` to cover the current state again; earlier references stay unavailable. |
