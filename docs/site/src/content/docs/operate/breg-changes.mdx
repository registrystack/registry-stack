...
field or action input uses (`field_vocabulary_codes_added`, `action_vocabulary_codes_added`), needs
no further evidence. An added action input code is still marked for review in the diff, because the
action accepts it with no further check. A widened grant is an access change: the planner accepts
it as metadata-only, but it still needs a reviewed migration descriptor and rehearsal evidence, as
the metadata-only case below describes.
