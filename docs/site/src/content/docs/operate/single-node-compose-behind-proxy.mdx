...
consultation result. Notary consumes at least one compiler-pinned Relay
consultation for every configured claim. A Notary-only deployment may start
without claims, but cannot evaluate evidence. Credential issuance requires a combined deployment and a
fresh exact Relay-backed evaluation for every selected claim.
...
```sh
PRODUCT_INPUT=registry-project/.registry-stack/build/staging/private/relay
SIGNING_KEY=operator-inputs/approved-signing-key.jwk
SIGNED_BUNDLE=operator-inputs/signed-relay-bundle
TRUST_ANCHOR=operator-inputs/relay-trust-anchor.json
registryctl bundle sign \
  ...
  --sequence 42 \
  --bundle-id staging-relay-42 \
  --out "$SIGNED_BUNDLE"
registryctl bundle verify \
  --bundle-dir "$SIGNED_BUNDLE" \
