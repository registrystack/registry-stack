...
```sh
backup_items="registryctl.yaml compose.yaml relay state data"
[ -d notary ] && backup_items="$backup_items notary"
...
volume.
Capture that volume after asking Redis to flush a snapshot:
