...
and any caller with the access token and the source file continues it. A run stores no source
...
patch operation, the profile, the package revision and schema fingerprint, the input length, and
...
Every submission rechecks the profile against the entity's batch route, so a run id alone grants
nothing.

A run is `open` while chunks are due, `complete` when the last chunk commits, `cancelled` after
an explicit cancel, and `blocked` once the active package no longer matches its binding. Each
failure has one recovery:
...
| The package or schema changed | The run reports `blocked` with reason `active_package_changed` and stays inspectable. Start a successor run under the new binding. |
...
| A durable ingestion run reports `blocked` | The active package no longer matches the binding the run was created under. Read the run, then start a successor run under the new package; the blocked run stays inspectable. |
