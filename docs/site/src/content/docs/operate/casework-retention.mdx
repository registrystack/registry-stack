...
credential are readable. Each one loads the authored project directory, resolves the operator file
(`operator.yaml` inside that directory unless you pass `--operator`), and refuses when the operator
file's `project` key does not resolve to that directory's `casework.yaml`. Each connects with the
migration credential, not the runtime credential.

Both commands preview by default and write nothing until you repeat the exact invocation with
`--apply`. A successful run prints one JSON object on standard output and exits 0. A refusal prints
`{"ok": false}` with a single `caseworkctl.refused` diagnostic on standard error and exits 1, so a
wrapper script can tell the two apart without parsing prose.
...
```sh
caseworkctl retention erase /etc/registry-casework/package \
...
control character. `operator configuration is bound to a different Casework project` means the
project directory and the operator file disagree; name the directory whose `casework.yaml` the
operator file points at.
...
```sh
caseworkctl attempt settle /etc/registry-casework/package \
  --operator /etc/registry-casework/operator.yaml \
...
The chain is keyed with the audit secret named by the operator file's `audit.secretRef`, which
authenticates the retained file against a writer that cannot read the deployment secret. Provide at
least 32 bytes of NUL-free text through a secret provider, never in the operator file:
...
| Either command refuses with `operator configuration is bound to a different Casework project` | The project directory and the operator file's `project` key name different projects. Pass the directory holding the `casework.yaml` that the operator file points at. |
...
- [Configure policy and authority](../../configure/casework/) for the hosted kinds that declare
