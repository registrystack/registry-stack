...
| --- | --- | --- |
| Hosted item payload, the decision result and its result constraints, requester notes, and staff hosted history | `terminalDays` from the moment the item completed or was cancelled | The runtime retention pass |
| Requester terminal feed events, including a completed item's recorded result | The same `terminalDays` from the terminal time | The runtime retention pass |
...
and display together with its result constraints, and deletes the protected accountability record
at its own later expiry. A failed pass logs a warning and the next tick retries.

A structured decision result is decision data, not source record content, but it follows the same
clock as the display payload it was decided on: the result a terminal event carries is deleted with
the event at `terminalDays`, and the constraints the Requester supplied at create time are nulled
beside the display. What outlives them is the accountability record's sha256 digest of the result,
