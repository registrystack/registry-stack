...
| Review clock occurrences: every activity clock, and every subject clock the settlement completed or cancelled | The same `terminalDays` from terminal settlement | The runtime retention pass |
| A subject clock paused by a changes-requested settlement, or left running by a superseded one | Until a later round of the same review kind continues it, otherwise until the accountability deadline | The runtime retention pass |
...
request context, results, notes, drafts, tasks, history, clock occurrences no later round
continues, and replay responses, then deletes the
...
file](../casework/#upgrade-from-a-release-that-rotated-the-audit-file) gives the steps.

{/* Evidence: crates/registry-casework/src/runtime.rs, serve_from_path() and
