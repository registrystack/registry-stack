...
| The request's producer and initiator identities | Raw until `terminalDays`, then a keyed sha256 tombstone until the accountability deadline, so the same producer or initiator still learns the result expired | The runtime retention pass |
