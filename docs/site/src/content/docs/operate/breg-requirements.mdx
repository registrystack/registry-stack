...
- **PostgreSQL 17 or newer**, with two login roles: a migration role that owns the schema and
  applies packages, and a runtime role the serving process uses. A registry with spatial fields
  also needs PostGIS 3.5 or newer. Your administrator installs the extensions the
...
Records, their history, pending change requests, and the outbox of undelivered events live in
PostgreSQL. The default audit destination needs a writable local directory per process; alternatively,
collect the best-effort `stdout` stream through your platform. Webhook delivery
...
trust anchor, and secrets mount read-only into it. File audit needs a separate writable volume.
...
**The audit journal.** Each audited request writes minimized entries through a file or stdout writer.
You ship the streams from every process to append-only storage and verify delivery before local
retention expires. The product does not provide tamper evidence.
...
{/* Evidence: crates/registry-breg/src/runtime_config.rs, AuditConfig, CursorConfig, DatabaseConfig,
    ...
    crates/registry-breg/src/migration_reconcile.rs; crates/registry-breg/src/audit.rs;
