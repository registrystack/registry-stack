...
binaries, with an installer that verifies both against the release checksums and installs both or
neither. Keep the pair on one release: both compute the schema fingerprint that a test receipt
binds. Each release also publishes a container image built on a distroless base with no shell,
