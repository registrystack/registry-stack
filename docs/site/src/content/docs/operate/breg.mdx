...
before anything reaches the install directory, and installs `breg`, `bregctl`, and `mint` together
or not at all. Registry Mint comes along because a local registry uses it when no identity provider
is at hand; a production deployment with its own OpenID Connect issuer can ignore it. It does not verify
...
| `evidenceProviders` | One binding per Evidence provider the package declares, keyed by provider ID: `baseUrl`, `trustBindingId`, secret references for the token and trusted JWKS, `revokedKeyIds`, and an optional `caBundleRef`. The script cannot replace them; see [Governed registry actions](../../explanation/governed-registry-actions/). |
...
When the identity provider rotates its signing keys — or regenerates them after state loss —
every token it issues fails verification against the pinned document, and callers all see the
same value-free `authentication.refused`. The server log names the operational cause on the
`registry_breg::auth` target: the refused token's signing key is not in the configured JWKS.
Recover by re-pinning, and treat it as a deliberate trust change, because the new document
becomes the anchor for bearer verification:
...
3. Restart `breg`; the static document is read only at startup.
4. Verify with a fresh provider login that authentication succeeds again, and that tokens
...
{/* Evidence: crates/registry-breg/src/runtime_config.rs, RuntimeConfig;
    ...
    static_jwks_rotation_is_diagnosed_and_repinning_restores_authentication;
...
| --- | --- |
| `alg`, `typ`, `kid` headers | `alg` equals `allowedAlgorithm`; `typ` equals `accessTokenType`, and when `accessTokenType` names the RFC 9068 access-token media type (`at+jwt` or `application/at+jwt`), either spelling of that one type is accepted; `kid` is present in the JWKS and absent from `deniedKids`. |
| `iss` | Equals `issuer`. |
