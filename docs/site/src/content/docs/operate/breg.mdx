...
Many operator paths are resolved through held directory descriptors rather than by name: audit
export output, data export output and checkpoint, data import state and checkpoint, test receipt
...
{/* Evidence: crates/registry-bregctl/src/audit_lifecycle.rs, create_export_file();
    crates/registry-bregctl/src/data_lifecycle.rs, resolve_write_destination();
...
platform rather than guessing. It verifies every downloaded binary against the release `SHA256SUMS`
before anything reaches the install directory, and installs `breg` and `bregctl` together or not at all. It does not verify
...
prints it, and the installer keeps the set together, so reinstalling replaces all three. The release
...
including the idle bound
([PgBouncer configuration](https://www.pgbouncer.org/config.html#ignore_startup_parameters)). When
a pooler drops the startup options, set the bound on the runtime role, where it then also applies
...
Base Registry Engine's catalog checks accept the extension, and the
`postgres.pg_stat_statements.unavailable` advisory stops. Only superusers and roles with the
privileges of `pg_read_all_stats` see the statement text of other roles.
...
{/* Evidence: crates/registry-breg/src/postgres/baseline.rs, advise() and connection_budget(),
    ...
    prepared_server_sessions_are_named_bounded_and_advised_with_pg_stat_statements. */}
...
this server runs the default connection settings with `database.pool.maxSize: 8`:
...
- [Retain, erase, and audit](../breg-retention/) for the maintenance commands that erase history,
  erase expired Evidence uses, and prove the audit journal.
