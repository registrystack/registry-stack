...
{/* Evidence: crates/registry-bregctl/src/data_lifecycle.rs, resolve_write_destination();
    ...
    crates/registry-breg/src/runtime_config.rs, read_bounded_runtime_config();
...
This source-preview page uses the unreleased shared audit writer. Use `breg` and `bregctl` built
from the same reviewed checkout as this configuration. The older published binaries do not
implement this audit contract; for a released deployment, use the documentation archive matching
its release.

With the checkout's pinned Rust toolchain and Python 3 installed, build both binaries from its root
using the [maintained build helper](https://github.com/registrystack/registry-stack/blob/main/scripts/cargo-runtime-library-path.sh).
The helper also sets the dynamic-library search path needed to run these binaries directly on macOS:
...
Keep using this shell and put that checkout's `target/debug` directory on your `PATH` for the
commands on this page.
Testing, packaging, activation, and serving must use this same build. Record the checkout revision
alongside the candidate: an unreleased binary's version string alone does not identify its source.
The [source quickstart prerequisites](https://github.com/registrystack/registry-stack/blob/main/products/breg/quickstart/README.md)
also describe the local smoke journey. Release installation and authenticity checks belong to the
...
For containers, obtain a matching candidate image from the maintained
[release build procedure](https://github.com/registrystack/registry-stack/blob/main/release/OPERATIONS.md).
Use its recorded image digest as `BREG_IMAGE` in [Serve the container image](#serve-the-container-image);
an older public image tag cannot serve this configuration. The release image is built on
distroless nonroot for `linux/amd64`; see
...
`--config /etc/breg/runtime.yaml`, and it exposes port 8080. The image carries no shell, no writable
directory, no healthcheck subcommand, and no `bregctl`. Mount a writable volume for the file audit
destination; mount the runtime configuration and its package and secret files read-only under
`/etc/breg`. The orchestrator probes `GET /health` or `GET /healthz` for liveness and `GET /ready`
for readiness. Activation runs on a separate operator host with the matching `bregctl`.
...
accepts, bound to the same identity. The loader requires the file's bytes to already be RFC 8785
canonical JSON: sorted object keys and no inserted whitespace. A pretty-printed copy of the same
values is refused as an integrity failure, so write it compact, for example with `jq -cS`:
...
| `submission-recovery-expired` | Reconcile the Casework request by the recorded idempotency and digest bindings before starting a new proposal. |
...
| `result-poll-attempts-exhausted` | The result lookup failed, or answered an empty `404`, until the attempt budget ran out; a review the authority keeps answering as pending is never failed. Reconcile the accepted Casework request by its retained binding. If the authority still holds a result, repair the result endpoint and start a new proposal; the retained binding identifies the review to close. |
...
| `executor-denied` | Restore the executor's current application grant, then use the advertised authorized manual apply action for the same approved proposal. |
...
with one rebaseline, then writes one audit record that carries counts, not values.
...
user, and publish the listener port. Set `BREG_IMAGE` to the digest of your matching candidate image
...
{/* Evidence: crates/registry-breg/src/package.rs, rederive(), inspect_package(), and load_predecessor_package();
    crates/registry-bregctl/src/package_inspection.rs, inspect_runtime_package() and
    inspect_runtime_predecessor_package();
    crates/registry-bregctl/src/doctor.rs, startup_diagnostic();
