...
`authentication.refused`. On the `registry_breg::auth` target, the server warns that a token's
key identifier is unknown or disallowed, at most once per minute per authenticator. This warning
...
1. Fetch the provider's current JWKS document from its published `jwks_uri` over TLS, the same
   ...
   not announced.
2. Replace the secret that `documentRef` names. For `secret:file/<jwks-document>`, replace the
