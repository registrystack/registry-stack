...
Base Registry Engine stores only the wrapped key and offers no recovery, so escrow the Transit key
before enabling. The local file provider is development custody: startup refuses it unless the
