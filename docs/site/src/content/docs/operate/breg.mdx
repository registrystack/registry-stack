...
files, the reviewed migrations tree, project migrate writes, and authoring sources. `bregctl`
resolves each component of these paths against the directory descriptor that holds it, then opens,
creates, renames, and publishes through those held descriptors, so a directory replaced by a
symbolic link after the path is validated cannot redirect the command. `--runtime-config` and
`--package` are resolved by pathname in `registry-breg` instead, as is every `bregctl dev` path,
and the local file `apply --backup <binding>=<path>` names, which `registry-breg` opens with
`O_NOFOLLOW` on its final component only.
...
{/* Evidence: crates/registry-bregctl/src/audit_lifecycle.rs, create_export_file();
    ...
    read_bounded_source_file(), and path_diagnostic();
