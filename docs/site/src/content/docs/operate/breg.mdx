...
No path an operator command reads or writes may traverse a symbolic link. `bregctl` resolves each
component against the directory descriptor that holds it, then opens, creates, renames, and
publishes through those held descriptors, so a directory replaced by a symbolic link after the
path is validated redirects the pathname without redirecting the command. Name the real
directory: on macOS, `/tmp` and `/var` are reached through symbolic links, so pass `/private/tmp`
and `/private/var`.

The refusal is fail-closed. The Linux and macOS builds resolve every operator path this way. A
build for a platform that offers no equivalent kernel-enforced resolution refuses each such path
with `path.no_symlink_unsupported` rather than falling back to resolution by pathname.

{/* Evidence: crates/registry-bregctl/src/lib.rs, TestArgs, PackageArgs, ApplyArgs, DoctorArgs, and path_diagnostic();
    crates/registry-bregctl/src/safe_path.rs, SafeDir and SafeEntry;
    products/breg/scripts/test-adopter-workflow.sh. */}

## Obtain the runtime

Install `breg` and `bregctl` from the release assets:
...
Testing, packaging, activation, and serving must use one release: the `bregctl` that packages and
the `breg` that serves carry the same tag, and `--version` on each prints it. The installer keeps
the pair together, so reinstalling replaces both.

The release publishes the installer as `breg-v0.26.1-install.sh`, with `breg-install.sh` as the
movable alias, and the binaries as `breg-v0.26.1-linux-amd64`, `breg-v0.26.1-linux-arm64`,
`breg-v0.26.1-macos-arm64`, `bregctl-v0.26.1-linux-amd64`, `bregctl-v0.26.1-linux-arm64`, and
`bregctl-v0.26.1-macos-arm64`.
...
[Activate and serve](#activate-and-serve) records. The release workflow records each promoted
image digest in the release manifest; pin that digest instead of the movable tag.
...
length, digest, record count, and server cursor in its checkpoint. A rerun streams the checkpointed
prefix of the existing output, matches it against that checkpoint, and only then requests the next
server-validated cursor. A run stopped between the two writes leaves one page the checkpoint never
recorded; the rerun discards that page and fetches it again. It refuses when the output is shorter
than its checkpoint, when the checkpointed prefix no longer matches, when more than one page follows
the checkpoint, or when anything follows a checkpoint that already reports the export complete. Do
not edit either file. `--max-chunks` and `--max-pages` bound one operator run.
...
| `audit verify`, `audit export`, or `audit prune` reports `audit.envelope.invalid` | A reachable journal record holds an envelope no reader accepts, or names a link no record follows. Nothing is pruned. Keep the database as it is and treat it as an integrity incident, like a broken chain. |
