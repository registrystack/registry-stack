...
| `fieldEncryption` | Key custody for encrypted fields: the Transit binding over a Unix socket path, with its mount, key name, and unwrap timeout, or the local file provider, which is for local assurance only. Declare it when the project encrypts a field; the model and its limits are in [Field encryption for restricted fields](../../explanation/breg-field-encryption/). |
...
and only Transit unwraps. If the key is lost, or the `fieldEncryption` binding cannot reach it,
entities with encrypted fields fail closed: reads and writes answer HTTP `503` with the
`runtime.field_encryption.unavailable` problem, and entities without encrypted fields keep
serving. Base Registry Engine holds no unwrapped copy and offers no recovery, so escrow the Transit
key before enabling. The local file provider is local-only, and `bregctl check --production`
refuses a candidate that names it.
:::

{/* TODO[evidence]: the fieldEncryption binding, the fail-closed 503 problem, and the
    production refusal of the local-file DEK are Phase 1 behavior of registry-stack issue
    #1108, whose implementation is not yet in this tree. When the milestone lands, anchor them
    against crates/registry-breg/src/runtime_config.rs, the DEK provider, and the production
    check, and fold them into the Evidence block below. */}
