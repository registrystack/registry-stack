...
entry with a response whose outcome is `unfinished`. Only a crash, or a destination that already
stopped accepting entries, can leave a request entry without a response.
...
which states how many objects still wait for deletion. If the erasure committed but its response
...
event delivery's attempt is recorded before its request leaves, and its outcome only once the
delivery's state has committed, so the journal never records a delivery that rolled back; an
operator replay is recorded as a request before the reset and a response once it commits or is
refused.
...
{/* Evidence: crates/registry-breg/src/audit.rs;
    ...
    crates/registry-breg/src/request_retention.rs, ErasureUnaudited;
