...
audit record and a report that carries counts rather than values. Four of the commands are
irreversible: `history erase`, `request-retention erase`, `evidence-retention erase-expired`, and
`audit prune` without `--dry-run` remove bytes that nothing restores, and the next section says
