...
You have a sealed template bundle and callers that need PDFs, and you want `render serve`
answering them. At the end of this page one `render` process serves your bundle on a private
...
- **One sealed bundle directory, mounted read-only.** `render seal` writes per-file sha256 hashes
  into the bundle's `manifest.yaml`, and serve verifies the seal at startup and again in the
  worker on every request, so a bundle that drifts while the service runs is refused, not
  rendered. Treat the directory as content the deployment does not write.
...
target/release/render --version
```

The version line names the release and the Typst pin, for example `render <version> (typst
...
```sh
target/release/render serve --runtime /etc/registry-render/runtime.yaml &
...
if the sink cannot answer, readiness fails. Both are value-free. `render healthcheck --runtime
<file>` performs the same probe for a supervisor script.
...
problems with JSON pointers into the data, and `render validate` dry-runs that check without
rendering.
...
```sh
target/release/render audit-verify --runtime /etc/registry-render/runtime.yaml
```
