...
with your stop timeout, not a decorative field.
...
reconcine the running service against what you deployed. `GET /ready` includes the audit ledger:
...
ledger cannot be written, the response fails closed.
...
active segment, and verification covers what has been sealed. Then:
...
{/* Evidence: crates/registry-render/src/audit.rs, verify_chain;
    crates/registry-render/src/server.rs, RenderAuditEvent. */}
