...
order, selected fields, authorization context, optional bbox, response format, GeoJSON profile,
and expiry. Treat cursors as opaque continuation tokens even though they contain no plaintext
filter, order, or bbox values.
...
Bounded bbox consultation is a list operation, so it uses a reviewed snapshot source in this
profile. GeoJSON and JSON-FG do not change the source, access, audit, or cache rules. Their exact
...
| A spatial request returns `406 representation.unsupported` | The selected representation does not disclose a primary geometry or the `Accept` value is unsupported | Select an entitled geometry-bearing representation or request JSON or JSON-LD. |
