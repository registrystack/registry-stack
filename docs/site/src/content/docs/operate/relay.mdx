...
`authentication.issuer: null` is valid only when every compiled access profile is public.
A package with a protected access profile needs the configured issuer at startup.
...
order, selected fields, authorization context, optional bbox, wire format, format profile,
...
Audit binds the Registry, resource, operation, access profile, disclosure profile, selected
