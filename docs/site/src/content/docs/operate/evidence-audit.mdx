---
title: Verify and interpret the Evidence audit chain
description: Prove the integrity of retained Evidence audit history, understand what each phase establishes, and keep the subject-correlation boundary explicit.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
Evidence writes a keyed, append-only audit chain. The chain is useful for proving which governed
...
If Evidence is still running, the writer keeps the active segment locked. Verification proves the
...
| `denial` or `transient-failure` | An admitted operation later failed in one closed, value-free class. | The protected selector, source value, or credential that triggered it. |

Failures that happen before Evidence has enough privacy-safe context, such as authentication
failure or an invalid selector, do not fabricate a native subject event. Do not expect every HTTP
request to produce two audit records.

Native records can carry the governed requirement, purpose, bundle revision, requester pseudonym,
subject role and selector-profile pseudonym, source and adapter identifiers, response protection,
decision, safe failure class, disclosed concept IDs, evidence ID, and signing key ID. They never
carry the disclosed value, raw selector, source response, access token, or credential.
...
This command verifies the whole retained local chain, then renders only the last operation. It is
...
- Allow one Evidence writer per audit path.
