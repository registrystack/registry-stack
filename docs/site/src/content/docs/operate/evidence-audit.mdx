...
retained local audit files in order, checks that every entry is well formed and that each access
attempt pairs with a coherent terminal record, then renders the operation of the physically last
entry. Concurrent requests can complete in a different order from their admission order, so "last"
