...
The `relay` binary has a published installer. The published platform is Linux
amd64:
...
It does not verify release authenticity.
...
`relayctl` has no installer. Operators who need it take the
`relayctl-<tag>-linux-amd64`, `relayctl-<tag>-linux-arm64`, or
`relayctl-<tag>-macos-arm64` release binary, or build it from source.
