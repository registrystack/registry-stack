...
One installer publishes both binaries, `relay` and `relayctl`, for Linux amd64,
...
The installer verifies each downloaded binary against the release `SHA256SUMS`
and installs both or neither. It does not verify release authenticity.
For a higher-assurance installation, follow
[release verification](https://github.com/registrystack/registry-stack/blob/v0.19.0/release/VERIFY.md)
...
The container image is `ghcr.io/registrystack/relay:v0.19.0`, built on
...
Operators who prefer to place the binaries themselves take the
`relay-<tag>-<platform>` and `relayctl-<tag>-<platform>` release assets
directly, or build them from source.
