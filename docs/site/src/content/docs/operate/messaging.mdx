...
{/* messaging-s6b: document the retention sweep (payloadDays, recordDays, submissionReceiptDays erasure) and messagingctl retention erase-expired once they land. */}
...
`GET /health` answers `200` with an empty body while the process serves, and `GET /ready` answers
`200` once the database carries every expected migration, `503 service.unavailable` otherwise.
Neither takes a token, and both carry `Cache-Control: no-store`.
...
| --- | --- | --- |
| `messaging_http_requests_total` | `route`, `method`, `status` | Answered HTTP requests |
| `messaging_authentication_refusals_total` | `reason` | Bearer credentials refused before a caller was resolved |
| `messaging_provider_callbacks_total` | `outcome` | Provider callbacks: `unverified`, `unreadable`, `ignored`, `applied`, `unchanged`, `unmatched`, `ambiguous`, or `unavailable` |

A rising `unverified` count usually means a callback secret or URL differs between the provider and
the runtime file.

{/* Evidence: crates/registry-messaging/src/metrics.rs; products/messaging/RUNTIME-CONFIG.md,
    Provider callbacks. */}

{/* messaging-s6b: replace this table with the final metrics names, including dispatch and quota series, once they land. */}
