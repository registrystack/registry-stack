...
default. `${VAR}`, `${VAR:-default}`, and `${VAR:?message}` expand over the document's raw text
before it is parsed as YAML, so an expression in a comment or a key is expanded too and must
resolve; a value that fills a whole member is quoted as one string, and one embedded in longer text
is refused if it could change the document's shape. A member whose name ends in `Ref` names a
secret and refuses `${` anywhere at or below it, before expansion runs. The expanded document is
capped at one mebibyte. The full key reference is `products/messaging/RUNTIME-CONFIG.md` in the
Registry Stack repository.

{/* Evidence: products/messaging/RUNTIME-CONFIG.md; products/messaging/examples/starter/runtime.example.yaml;
    products/messaging/generated/runtime/runtime.schema.json; crates/registry-messaging/src/config.rs,
    parse_with_environment(); crates/registry-platform-config/src/lib.rs,
    expand_config_env_vars_with() and reject_config_env_expressions_in(). */}
