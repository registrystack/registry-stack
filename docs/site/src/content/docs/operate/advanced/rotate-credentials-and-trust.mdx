...
description: Rotate Registry Relay and Evidence Gateway credentials, signing keys, and trust material without exposing secret material or widening authority.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
path or an Evidence Gateway signing key, without
...
- Preserve a verified recovery set for the current Relay product configuration, and keep the
  prior Evidence Gateway bundle revision each rotation replaces.
