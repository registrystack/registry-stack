...
trust anchor, an Evidence Gateway signing key, or a Registry Mint signing key or client key, without
...
Evidence Gateway owns its signing key, audit hash secret, and subject-binding secret, each resolved
...
own: Evidence Gateway validates its governed bundle and runtime file with `evidence check`, and Registry
...
3. Decide whether the change remains in the secret plane or changes a governed reference.
   ...
   product bundle through `registryctl`. For Evidence Gateway and Registry Mint, a governed change edits
...
Do not copy source certificates, keys, or destinations into the Evidence Gateway bundle: Evidence Gateway sources
...
restart the process.
Evidence Gateway does not need reconfiguring for this: it fetches Registry Mint's JWKS from the configured
...
[Rotate Evidence Gateway signing keys](../../../tutorials/move-evidence-to-production-signing/#stage-a-new-keypair)
...
cannot overlap safely, a certificate changes source identity, an old Evidence Gateway public key cannot
...
- [Move Evidence Gateway to production signing](../../../tutorials/move-evidence-to-production-signing/)
