...
The unified review schema arrives in a single migration that drops the experimental hosted-item
tables from the earlier Casework release. No data is carried over, and this release has
no in-place legacy conversion command. Do not delete rows, edit the migration ledger, or run
migration SQL by hand. For a disposable local session, use `caseworkctl dev stop PROJECT --remove`
and start it again. For an operated database, retain the old deployment long enough to export
required work, take the normal backup, and provision a fresh Casework database for the unified
review package.
...
rollback is a configuration change rather than a rebuild. A rollback to an earlier package or source
binding does not reopen the work items that binding left behind: they stay superseded, and the
source's next observation opens fresh work items beside them.
