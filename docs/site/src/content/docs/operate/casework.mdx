...
the cause appears or changes, and logs again when reads recover.

Between a source change and the reconciliation that applies it, the affected item stays in lists
and its view, history, and clocks stay readable with the binding Casework last applied, but it
offers no actions, and a claim, draft, or decision on it is refused with
...
{/* Evidence: crates/registry-casework/src/runtime.rs, migrate_from_path(), serve_from_path(),
    ...
    crates/registry-casework-breg/src/lib.rs, BregAdapter::read_result(). */}
