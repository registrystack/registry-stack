...
tables from the earlier Casework release. No data is carried over. Do not delete rows, edit the
migration ledger, or run migration SQL by hand. For a disposable local session, use `caseworkctl
dev stop PROJECT --remove` and start it again. For an operated database, retain the old deployment
long enough to export required work, take the normal backup, and provision a fresh Casework
database for the unified review package.
