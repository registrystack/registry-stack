...
description: Install the Registry Casework runtime, write its operator file, verify the reviewed policy package, migrate PostgreSQL, serve behind your TLS proxy, and establish directory authority.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
[configure policy and authority](../../configure/casework/) covers that side. This page starts from
...
- **Secrets the process can read.** Every credential in the operator file is a reference:
...
its callers create live in Casework's own database, and the source bindings in the operator file
...
`--config /etc/registry-casework/operator.yaml serve`, and it exposes port 8091. It carries the
...
audit chain; mount the operator file, the policy package, and the secret root read-only. The
...
The operator file binds one policy package to one database, one issuer, one listener, and one
...
```yaml
# The casework.yaml inside the installed policy package, not the authoring project.
project: /etc/registry-casework/package/casework.yaml
# A private or loopback address. The proxy in front of it terminates TLS.
listen: 10.42.0.7:8091
# operator-controlled-upstream for production; development-loopback only for a loopback listener.
tlsTermination: operator-controlled-upstream
# private-address accepts loopback and private addresses; container-private also accepts a wildcard bind.
networkExposure: private-address
secretProviders:
  file:
    # Owner-only files, one per reference. An absolute path avoids resolution against this file.
...
authentication:
  oidc:
    ...
    # Non-empty; each access profile in the package names the claim its own callers carry.
    principalClaim: sub
...
| --- | --- |
| `project` | The `casework.yaml` the runtime serves, with its manifest and exact source descriptions beside it. |
| `listen`, `tlsTermination`, `networkExposure` | The listener address and the transport boundary the deployment declares for it. |
| `secretProviders` | The root directory of the file secret provider. The environment provider is always available for `secret:env/<NAME>`. |
...
Relative `project`, secret root, and audit paths resolve against the directory holding the operator
file, so pass an absolute path to `--config` or keep every path in the file absolute. A relative
`--config` leaves the secret root relative, and the resolver, which requires an absolute root,
refuses the configuration before anything else runs.
...
`scopeClaim` defaults to `registry_scopes` and carries every required scope for the selected
profile. `humanIdentity` defaults to the claim `registry_actor_kind` with the value `human`, must
...
`trustedRootCertificatesRef` for a private certificate authority in front of the source, and
`requestTimeoutMilliseconds` and `connectTimeoutMilliseconds`, which default to 30000 and 10000,
accept at most 300000, and are refused when the connect timeout is the larger of the two.
...
{/* Evidence: crates/registry-casework/src/config.rs, RuntimeConfig, TlsTermination,
    ...
    crates/registry-casework-breg/src/config.rs, BregBinding, DEFAULT_EVENT_TYPE,
...
The runtime verifies the package every time it loads the operator file, before it opens the
...
casework: the Casework secret-provider configuration is invalid
```
...
operator file itself: unparseable YAML, and a configuration the checks refuse, which covers an
...
resolver cannot use, most often a relative root left by a relative `--config`.
...
`migrate` loads the same operator file, resolves `migrationUrlRef`, takes a PostgreSQL advisory
...
and a source reconciliation pass every 60 seconds per bound source. A source outage stays visible
...
prints a JSON report naming each check and each source it contacted. The project argument is the
directory holding the `casework.yaml` the operator file points at, and `doctor` refuses an operator
file bound to a different project.
...
new directory beside the current one, point `project` at the new directory's `casework.yaml`, and
...
| `the Casework secret-provider configuration is invalid` | The file provider root is relative or unusable. Pass an absolute path to `--config`, or write an absolute `root`. |
...
- [Configure policy and authority](../../configure/casework/) for the package this deployment
