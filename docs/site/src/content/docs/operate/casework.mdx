...
The unified review migration accepts a fresh database and the schema produced by the immediately
preceding Casework release. It refuses a database containing incompatible legacy casework state
with one generic migration error. This release has no in-place legacy conversion command. Do not
delete rows, edit the migration ledger, or run migration SQL by hand. For a disposable local
session, use `caseworkctl dev stop PROJECT --remove` and start it again. For an operated database,
retain the old deployment long enough to finish or export required work, take the normal backup,
and provision a fresh Casework database for the unified review package.
