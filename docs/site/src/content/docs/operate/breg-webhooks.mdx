...
Lifecycle events whose transition and state filters permit rejection or a request for revision
have an `internal` classification floor: their system request envelope may contain the reviewer's
explanation. Filters excluding both outcomes retain the event's ordinary derived classification.
...
{/* Evidence: crates/registry-breg/src/compiler.rs request_event_may_include_review_reason and compile_event_delivery_inventory; crates/registry-breg/src/request_events.rs. */}
