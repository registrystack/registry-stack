...
The two interact: rolling back the software does not permit rolling back the applied config
...
- You know where your deployment keeps its persistent state. At minimum: the audit log
  ...
  Registry Notary, and your config files and secrets.
...
4. Back up state that a failed upgrade must not lose: the audit log directory (copy, never
   truncate or edit; the hash chain makes hand-edited files unverifiable), the anti-rollback
   state file, and your config and secrets as currently deployed.
...
6. Verify the new deployment:
   ...
   - `/healthz` returns 200 and `/ready` returns 200. A 503 from `/ready` with a
     `deployment/not_ready` problem body means a profile gate is failing; check the admin
     posture surface for the finding.
...
2. Respect the constraints that survive a software roll-back:
   ...
   - Anti-rollback state is monotonic. If you applied a new signed config bundle after
     upgrading, the previous software with the previous config still loads, but you cannot
     re-apply an older bundle over a newer sequence number. Roll back the software, not the
     applied config sequence.
...
- `/ready` stays 503 after upgrade: read the problem body. `deployment/not_ready` is a
  profile gate, inspect the admin posture surface; `schema/resource_unavailable` on Registry
  Relay means a configured resource failed ingest.
- Config bundle apply rejected with a rollback outcome after reverting: expected. The
  anti-rollback store saw a newer sequence. Issue a new bundle with a higher sequence
  instead of re-applying the old one.
