...
| `GET /ready`, journal not writable | 503 `{"status":"unavailable"}` | 503 `{"status":"not-ready"}` |
...
The Base Registry Engine release set includes a `breg-mcp` image and a `breg-review` image beside
the `breg` image. Each carries its binary alone, on a distroless base with no shell. The release manifest
records the promoted digest of each image; pin that digest instead of the movable tag.

{/* Reconcile: image names with release wiring */}
...
{/* Reconcile: image names with release wiring; default command, configuration path,
    journal directory, and image user from release/docker/Dockerfile.breg-mcp and
    Dockerfile.breg-review. */}
...
Set `audit.path` to a file under the mounted journal directory, such as
`/var/lib/breg-mcp/audit/audit.jsonl`. Run `check` in the same image before you serve:
