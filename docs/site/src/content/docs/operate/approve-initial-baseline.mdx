...
description: Create independent lane anchors, sign the first two product bundles, verify them, and assemble an approved baseline set.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
Registry Notary is retired and no longer has a lane in this procedure. Evidence and Registry Mint
...
`evidence check` (or the `evidencectl` project scaffold and fixture commands that drive it) and
`mint check`. See [Configure Evidence](../../configure/evidence/) and
[Configure Registry Mint](../../configure/mint/) for that procedure.
...
Current registryctl refuses an approved set issued before the Registry Notary retirement, whether
it declares schema version `1.0`, carries a `notary` lane, or binds cross-lane interface digests.
It reports that the document predates the retirement and names re-approval as the remedy.
