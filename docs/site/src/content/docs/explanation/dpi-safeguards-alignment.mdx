...
It does not create lawful basis, public mandate, inclusion practice, grievance handling, or independent
oversight.
...
Each cluster groups one or more of the framework's 18 principles. The IDs in parentheses
...
| Autonomy and agency (F6) | Notary credentials support holder binding through `did:jwk`, so only the key holder can present a credential; claim results and credentials can avoid exposing more than the configured disclosure mode allows. | The stack does not provide a complete wallet, consent journey, opt-out path, appeal process, or assisted service channel. |
...
It helps reviewers answer what is exposed, who may call it, what policy gates exist, what evidence can
be returned, and which project owns the behavior.
