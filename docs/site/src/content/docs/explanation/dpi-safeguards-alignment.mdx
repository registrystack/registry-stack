...
| Who can read protected data? | Relay authenticates protected source routes. Notary authenticates claim routes and authorizes registry-backed claims before calling Relay. | Deployment policy decides who receives scopes and workload authority. |
...
| Can the exchange be reviewed later? | Relay and Notary audit events, Policy Decision Point (PDP) provenance, package and content digests, and Selective Disclosure JWT Verifiable Credentials (SD-JWT VC) provide review evidence. | Audit records support accountability. They are not accountability by themselves. |
...
| [Registry Notary](../../products/registry-notary/) | Evaluates configured claims from compiler-pinned Relay consultations, authorizes subject access, applies disclosure policy, renders claim results, issues SD-JWT VC credentials only from stored exact Relay-backed dependency-closure provenance under the configured ceremony, and supports static-peer delegated evaluation. | Notary does not accept caller-provided evidence, access registry sources directly, decide eligibility, run a wallet, certify source truth, or provide open federation. See the [Registry Notary protocol](../../spec/rs-pr-notary/). |
...
| Transparency and accountability (F4) | Portable metadata, runtime metadata, OpenAPI references, stable error codes, audit records, Notary federation signed responses, credentials, and the [standards register](../../reference/standards/) make behavior reviewable. | Public governance, remedies, procurement discipline, and independent accountability mechanisms are institutional controls. |
