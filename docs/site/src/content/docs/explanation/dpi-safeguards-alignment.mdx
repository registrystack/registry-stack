...
that boundary in place. Treat the table below as a reviewed argument you can check against the
...
{/* Evidence: PurposeConstraint { claim, allowed } and AuthorityRowBinding over a token claim or the
     token principal, crates/registry-relay-v2/src/contract.rs:966-976, enforced in
     RelayAuthenticator::authorize(), crates/registry-relay-v2/src/auth.rs:204-257; the closed
     26-variant ProblemCode set and its PROBLEM_BASE constant,
     crates/registry-relay-v2/src/problem.rs:14-45; the two fixed transforms are partial-string and
...
| [Registry Relay](../../products/registry-relay/) | Compiles a reviewed registry contract into a fixed route set over one read-only SQLite source: record reads and lists, declared lookups and searches, an aggregate-data read where the contract declares statistical datasets, metadata under a declared visibility, and sealed artifacts. Every served request is audited. | Relay does not mutate source data, expose arbitrary SQL, evaluate external policy, issue credentials, sign anything, host issuer DID documents, or offer an administrative surface of any kind. Its route set is fixed at 14 routes with exactly one `POST`. See the [Registry Relay protocol](../../spec/rs-pr-relay/). |
...
{/* Evidence: the fixed 14-route table with exactly one POST, crates/registry-relay-v2/src/server.rs
     ...
     crates/registry-platform-pdp is depended on only by the retired crates/registry-relay and by
     itself, and crates/registry-relay-v2 contains no PDP or policy-evaluation path, so the
