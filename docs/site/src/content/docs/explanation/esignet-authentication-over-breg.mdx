...
1. **Challenge first.** The provider verifies the citizen's one-time code or other challenge before
   any registry call. A wrong code never reaches BReg or the token issuer, so the registry is not an oracle for
...
The provider configuration fixes the token resource and scopes for every exchange. */}
...
The pages on [BReg access profiles](../../configure/breg-access/) and
...
credential that reaches the registry is the provider's own access token.
...
subject secret and the token client key are the two the deployment must protect and rotate on its
own schedule.
