...
adopter-facing account of version 0.3.0 of
[registrystack/esignet-relay-authenticator](https://github.com/registrystack/esignet-relay-authenticator).
...
repository's README, docs/esignet-configuration.md, and docs/breg-lookup-contract.md when it
changes. */}
...
Mint issues tokens the BReg access profile trusts, and the provider caches each one for at most
five minutes.
...
Registry Stack's own OIDC verifier already accounts for eSignet's token shape. Evidence Gateway and
BReg accept eSignet-issued ID Tokens and UserInfo JWTs that omit the `typ` header, which is how
eSignet emits them, without relaxing any other check.

{/* Evidence: crates/registry-platform-oidc/src/lib.rs, enforce_optional_typ: the shared OIDC
verifier accepts JWTs without a typ header because eSignet omits it on ID Token and UserInfo JWTs,
while still enforcing every other header and claim check. */}
...
- [Run Registry Mint](../../configure/mint/) and register the provider as a `private_key_jwt`
  client.
- [Request an access token](../../configure/request-an-access-token/) to test the grant before
  wiring eSignet.
