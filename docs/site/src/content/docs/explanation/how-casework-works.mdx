...
The identity behind one accountability event is reachable only by a Supervisor with current authority over the task's queue, only while the accountability period lasts, and every such read writes its own audit record in the same transaction as the read.
