...
Casework coordinates source-owned work items and unified review requests. A project that declares neither a source nor a review kind is refused because it describes no work.

| What differs | Source-backed item | Unified review request |
| ------ | ------ | ------ |
| Where it starts | A governed [change request](../../reference/glossary/#change-request) that a [Base Registry Engine](../../reference/glossary/#base-registry-engine) already holds | An admitted producer creating one under a declared review kind |
| What Casework holds | Assignment, routing, clocks, private drafts, attempts, and recovery | Immutable subject and policy bindings, context, tasks, decisions, result, and accountability |
| What stays with the source | Eligibility, current visibility, and the record change itself | The producer's business object and correlation reference |
| How it finishes | An accountable attempt against the source, made by a person | An approved, rejected, changes-requested, answered, cancelled, or superseded result |
...
An admitted producer creates a unified review request with an immutable subject binding and either a submitted snapshot or a source-context reference.
The review kind fixes the ordered stages, thresholds, reviewer exclusions, context strategy, schemas, outcomes, clocks, and retention.
Casework pins the policy identity and canonical submission digest at creation, so publishing a changed policy later shapes new requests and never rewrites existing ones.
The starter project ships one answer policy and admitted producer, which is enough to run without a registry.

{/* Evidence: crates/registry-casework-core/src/review.rs, ReviewKindPolicy and ReviewKindPolicySnapshot;
    crates/registry-casework/src/review.rs, create_review_request and decide_review_task. */}

A review decision may carry a structured result beside a configured rejected, changes-requested, or answer outcome.
The kind's `resultSchema` types that result, and the producer's `resultConstraints` narrow fields to a subset the schema already admits.
Casework validates the result against the pinned schema first and the narrowing second, then returns it through polling, the result feed, or an optional authenticated completion signal.
An approved decision remains payload-free.

{/* Evidence: crates/registry-casework-core/src/review.rs, validate_result_constraints and
    validate_outcome; crates/registry-casework/src/review.rs, decide_review_task. */}
...
| Supervisor | Work those queues, assign visible work, maintain absence records for staff they supervise, move a caseload, and read protected review accountability. |
| Administrator | Maintain the directory and publish holiday revisions while the service runs. |
| Requester | Act as an admitted producer, read its own requests and results, page its result feed, and cancel an active request. It cannot claim or decide. |
...
The reason the person records with the decision travels with it: every promoted decision action and the apply action carry the bounded remark to the source, which records it beside the action under its own disclosure and retention rules.
...
A review kind sets two periods, and the second is at least as long as the first.
The terminal period is how long a producer can read the result and associated request state.
The accountability period is how long the protected record of who decided, when, and why survives.
When both expire, Casework deletes accountability and its payload-free replay tombstone.

{/* Evidence: crates/registry-casework-core/src/review.rs, ReviewRetentionPolicy and ReviewAccountabilityRecord;
    crates/registry-casework/src/review.rs, erase_expired_reviews_at. */}

The result a producer reads names no person and carries no private reason.
The identity behind one accountability event is reachable only by a Supervisor with current authority over the task's queue, only while the accountability period lasts, and every such read returns the identity only after the audit destination accepts the entry recording it.
Accountability is therefore a deliberate, recorded act rather than a field on a response.

{/* Evidence: crates/registry-casework-core/src/review.rs, ReviewResult and ReviewAccountabilityRecord;
    crates/registry-casework/src/review.rs, review_accountability. */}

Idempotency outlives the payload it protected.
When a review request is erased, Casework keeps a payload-free tombstone holding the original request hash and hashed binding metadata, which is enough to recognise a replayed key and refuse a changed body under the same key.
...
- [Author a Casework policy](../../configure/casework/): review kinds, producers, sources, routing rules, calendars, and clocks.
