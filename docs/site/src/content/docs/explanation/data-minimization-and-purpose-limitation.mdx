...
Evidence Gateway also performs no presentation-side verification, no key-binding JWT validation, and no
wallet interaction, and the profile excludes OID4VCI in every part. A caller may supply a holder
public key that Evidence Gateway echoes into the `cnf` claim for a relying party to check itself.
