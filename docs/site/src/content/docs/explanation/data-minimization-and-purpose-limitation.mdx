...
compiler-pinned consultation, or the configured authenticated context for subject access.
...
committed fields and the digest. A delegated evaluation without the
representative ceremony or a legacy evaluation cannot cross the
