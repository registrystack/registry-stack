...
{/* TODO[evidence]: the boundary statements above are approved Phase 1 behavior of
     registry-stack issue #1108, whose implementation is not yet in this tree, so no anchor
     resolves today. When the field-encryption milestone lands, anchor the envelope and AAD
     against the sealing path in crates/registry-breg/, the Transit wrapping against the DEK
     provider and crates/registry-breg/src/runtime_config.rs, the 503 problem and per-entity
     scope against the read path, the blind-index leakage against the blind-index module, and
     the fips feature selection against Cargo.toml and Cargo.lock. Keep the upstream
     validation sentence cited to aws-lc-fips-sys's own documentation; do not strengthen it. */}
