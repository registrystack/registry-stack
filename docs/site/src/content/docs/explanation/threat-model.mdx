...
request id, checked scopes, and `Data-Purpose`) and a deployment can run audit fail-closed,
so an unrecordable request does not return success. Two caveats for the auditor. First,
this is a capability a deployment can turn on, not a guarantee that every route in a given
build has been individually audited; treat per-route audit coverage as something to verify
in your deployment. Second, Notary records hashed principal and correlation identifiers, so
