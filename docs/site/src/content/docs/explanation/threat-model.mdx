...
<figure>
  <img src="../../images/registry-trust-boundaries.svg"
       alt="The Registry Stack trust boundaries in one map. The portable metadata layer
            ...
            Notary consumes compiler-pinned Relay consultations for every claim. Subject
            access may authorize and bind a request but never provides evidence. Federation with a peer
            ...
            Relay consultation or claim evaluation. Delegated subject access is a distinct trust context from peer
...
**Delegated subject access.** Delegated subject access is a distinct trust context from
...
- Caller-provided data crossing an evidence trust gate: Subject access and delegated
  subject access derive subject, requester, and relationship from the authenticated
...
- Authorization context promoted to claim evidence: Configuration accepts only
  registry-backed claims. Caller identity and authorization details cannot
  become claim values. Delegated evaluations without the representative
  ceremony, legacy, missing, stale, or modified evaluation provenance fails
...
- A full credential issuer: The OID4VCI surface is a registry-backed, issuer-initiated
  ...
  authorization code, a public nonce endpoint, response next nonce, issuance from caller-provided evidence,
