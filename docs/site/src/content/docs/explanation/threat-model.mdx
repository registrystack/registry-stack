...
not return success. Two caveats for the auditor. First, the recorded scope list is not a
...
correlation identifiers, so operator-side investigation depends on retaining the hashing
secret and request context.
