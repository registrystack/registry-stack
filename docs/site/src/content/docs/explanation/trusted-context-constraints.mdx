...
Registry Notary evaluate with the shared policy layer provided by Registry Platform. The product
still owns source identity, route identity, lookup semantics, and adapter contracts. The shared
report shape makes the constraints visible without pretending every product reads the same source
in the same way.
...
For OpenCRVS DCI (Digital Convergence Initiative) starter projects, `registryctl` maps the
freshness field from the DCI response envelope:
