...
Earlier versions of this page described a shared policy vocabulary in `registry-platform-pdp`,
...
point, evaluates no external policy, and reads none of that vocabulary. The crate has no current
runtime consumer.

{/* Evidence: crates/registry-platform-pdp is listed as a dependency only by
     crates/registry-relay (the retired runtime) and by itself; crates/registry-relay-v2 contains
...
its audit log. The processing entries described above are the deployment's declaration of legal
