...
authorized caller: a governed, audited, paginated read bounded by the caller's `dataset:rows`
scope and the dataset's configured filters and limits. Registry Notary returns the answer a
