...
Four questions carry equal weight in the Evidence Gateway contract, and none of them is the
...
signs it. It contacts its own configured authoritative HTTP source rather than calling Relay.
...
owns source access and adaptation policy. Evidence Gateway owns evidence authorization and the
...
- Correctness depends on the source: Relay reports what the reviewed source consultation says,
  and Evidence Gateway derives its answer from what the source returned. Neither product
