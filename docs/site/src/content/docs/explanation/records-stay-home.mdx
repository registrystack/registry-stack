...
boundary. By default a record that does not resolve collapses to a single not-available
reason rather than a `false`, so the lookup cannot be used as an existence oracle. To check eligibility without exposing an income figure, derive the decision with
...
- [Disclosure and minimization](../disclosure-and-minimization/), in depth *(explanation)*
