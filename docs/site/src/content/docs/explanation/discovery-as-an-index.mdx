...
const results = await discovery.searchServices({
  serviceKind: ['evidence'],
  evidenceType: ['urn:example:evidence-type'],
});
const selection = discovery.selectExact(results, {
...
results = discovery.search_services({
    "serviceKind": ["relay"],
    "semanticClass": ["urn:example:registered-business"],
})
selection = discovery.select_exact(results, {
    "recordId": results["items"][0]["recordId"],
    "matchedCapability": {
        "kind": "semantic-class",
        "id": "urn:example:registered-business",
    },
})
```

The libraries validate the server response and exact capability match before returning the selection.
They deliberately do not decide whether the selected origin, issuer, operator, endpoint, or capability
is trusted. Keep that decision in the application's native Evidence or Relay trust configuration.
...
{/* Evidence: `crates/registry-discovery-client-node/src/lib.rs` and
    ...
    `registry-discovery-client`. Their language-level loopback tests exercise search, resolution,
    exact selection, and serialized selection persistence. `crates/registry-discovery-client/src/selection.rs`,
