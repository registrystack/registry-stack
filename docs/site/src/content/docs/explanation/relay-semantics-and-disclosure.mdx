...
operation-explanation, and contextual-finding reports, plus a classification-review starter.
...
Point only for audiences that can receive the selected access profile. The JSON-LD context types
the bounded GeoJSON value as RDF JSON. Relay does not claim GeoSPARQL inference, geometry
conversion, or an OGC API Features service. Generated schemas and the synthetic HTTP journey pin
those limits.
...
codelist schemas, capability metadata, and operation-specific response artifacts.
