...
such as DHIS2 are reached through the built-in source-adapter engines (`http_json` or `http_flow`),
and social protection, agriculture, education, and civil
registration systems can keep their own authoritative stores.
Registry Notary instances return configured claims to consuming systems such as a social
protection management information system without turning those consumers into raw-record readers.
...
<figure>
  <img src="../../images/registry-country-evidence-mesh.svg"
       alt="Country evidence mesh: civil registration, clinical health over FHIR API,
            public health through DHIS2 via built-in http_json or http_flow source adapter, social protection, agriculture, and
