...
A Registry Stack project can deploy Relay only, Notary only, or both products.
Relay-only projects expose governed source capabilities. Notary-only projects evaluate source-free
or self-attested claims, but those claims are not issuable. In a combined project, Relay keeps source credentials and returns only
...
The supported local first-run combined path extends the canonical spreadsheet sample in place with
Notary and a private consultation Relay, then runs them as one local project. The snapshot starter
is a separate offline authoring path. Its synthetic fixture results do not establish that Relay or
Notary is running.
...
{/* Evidence: crates/registryctl/tests/project_authoring.rs,
    ...
    crates/registryctl/src/lib.rs, add_notary_to_canonical_project. */}
...
<figure>
  <img src="../../images/registry-country-evidence-mesh.svg"
       alt="Country evidence mesh: civil registration, clinical health over FHIR API,
            ...
            only, evaluation-only source-free Notary, or both. Relay returns governed typed outputs,
