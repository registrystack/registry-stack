...
A Registry Stack project can deploy Relay only, Notary without configured
claims, or both products. Relay-only projects expose governed source
capabilities. In a combined project, Relay keeps source credentials and returns only
reviewed typed consultation outputs for Notary to turn into configured claims. Consuming systems
such as a social-protection management information system do not become raw-record readers.

The supported local first run starts from the public HTTP template.
The project declares the public Relay, consultation Relay, and Notary path
before Registryctl creates a disposable development environment.
Spreadsheet uses that same development plan as an ordinary source provider.
...
{/* Evidence: crates/registryctl/tests/project_authoring.rs,
    ...
    relay_only_and_notary_only_projects_emit_only_selected_products; and
    crates/registryctl/tests/dev_runtime_core.rs. */}
...
<figure>
  <img src="../../images/registry-country-evidence-mesh.svg"
       alt="Country evidence mesh: civil registration, clinical health over FHIR API,
            public health through DHIS2 via a reviewed Rhai script, social protection, agriculture,
            and education systems keep their source data. Registry Stack projects can deploy Relay
            only, an empty Notary control plane, or both. Relay returns governed typed outputs,
            combined projects turn consultation outputs into minimized claims and credentials, and the social protection MIS
            owns the eligibility decision and reads the registry it owns directly." />
</figure>

## Evidence-to-action boundary

Every combined integration follows the same ownership boundary. Registry Relay owns
source-specific acquisition, source-access policy, and typed normalization. Registry Notary owns
atomic evidence claims plus evidence authorization and disclosure policy. The evidence consumer
determines how the evidence is used, and the decision owner remains accountable for requirements,
decisions, workflow, and action.

The maintained DHIS2 health-evidence conformance workspace makes that boundary
visible.
...
Registry Notary may attest a decision that DHIS2 or another authoritative source already made only
when the claim identifier and its review documentation identify it as a source-owned decision.
This exception does not make Notary a general-purpose eligibility or workflow engine.
...
- A workflow step needs an authoritative evidence fact (status, predicate, selected value) as an
