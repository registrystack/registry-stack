---
title: Accessibility
description: Accessibility targets for the Registry Docs documentation site.
status: current
owner: registry-docs
source_repos: []
...
Contrast values are calculated from design tokens; an external audit has not been performed (see [Known limitations](#known-limitations)).
...
The design decisions that back these targets are in `design-registry-docs.md`, the visual design document for this site (maintained alongside the repository, not published separately).
