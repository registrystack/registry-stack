...
list. Configure the deployment's OAuth issuer to issue an access token that carries a scope such as
`registry:business:premises-search-registrar`.
...
- Configure the deployment's OAuth issuer to issue tokens for the scopes declared in this
  project's access profiles.
