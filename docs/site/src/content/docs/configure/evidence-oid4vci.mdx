...
When retaining Registry Mint as the token issuer, leave `resource` or `scopes` out if that
installation does not accept those OAuth request parameters. `check`, `inspect`, and `serve` accept
...
- [Configure Registry Mint](../mint/) if you retain Mint to issue the access tokens used for
  Evidence Gateway and the adapter's `POST /offers` endpoint.
