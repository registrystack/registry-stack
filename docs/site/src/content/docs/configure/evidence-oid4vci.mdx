...
description: Deploy and configure evidence-oid4vci, the front end that delivers holder-bound Evidence Gateway credentials to a wallet over OID4VCI 1.0 Final.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
Configure `evidence-oid4vci` when a deployment needs to deliver a holder-bound Evidence Gateway
credential into a stock wallet, which accepts a credential only over OID4VCI, never over an
arbitrary HTTP response.

## When to use evidence-oid4vci

`evidence-oid4vci` is a supporting service, not a product pattern of its own, in the same sense
Registry Mint is. It delivers Evidence Gateway credentials to a wallet over OID4VCI 1.0 Final, the
...
(`credentialIssuer`, `evidence.baseUrl`, `mint.tokenEndpoint`, and the client assertion audience) to
be an absolute `https` URL with a host and no embedded credentials, query, or fragment. Set it to
`supervised-local-development` only for a disposable local developer environment; that mode admits
one supervised process group on loopback, requiring the listener to match the credential issuer's
origin exactly and every other endpoint to be a canonical `http://127.0.0.1:<port>` origin.
...
| Endpoint | Auth | Notes |
| --- | --- | --- |
| `GET /.well-known/openid-credential-issuer` | none | Publishes `credential_issuer`, `credential_endpoint`, `nonce_endpoint`, `authorization_servers`, and `credential_configurations_supported`. The token endpoint is not published here. |
| `GET /.well-known/oauth-authorization-server` | none | The issuer names itself as its own authorization server, so it publishes the RFC 8414 document a wallet needs to find `token_endpoint`. Publishes only `issuer`, `token_endpoint`, the single supported grant type, an empty `response_types_supported`, and `none` for token endpoint authentication. |
...
| `POST /nonce` | none required | Empty body in, `{"c_nonce": ...}` out, `Cache-Control: no-store`. A bearer token is read when one is presented, solely to bind the nonce to that token; without one the nonce is well-formed and no credential request can use it. |
...
- [Request a holder-bound credential](../../tutorials/request-a-holder-bound-credential/) to
  configure the `holder-bound` subject binding mode on Evidence Gateway itself.
