...
describe a credential Evidence Gateway would refuse to issue. The advertised
`batch_credential_issuance.batch_size` also comes from authenticated Evidence discovery. The
adapter bounds the value by its own request ceiling and records the effective value with each
offer, so a later metadata change cannot widen an outstanding exchange.
...
with open("oid4vci-metadata.json", encoding="utf-8") as source:
    document = json.load(source)
print(document["profile"])
print(document["deployment"])
PY
```

```text
registry.evidence.oid4vci-profile/v1
{'metricsEnabled': True, 'replicas': 1, 'restartInvalidatesOutstandingExchanges': True}
```

The document also contains `credentialIssuerMetadata` and `authorizationServerMetadata` exactly as
the service derives them. `inspect` is an explicit diagnostic command, so it may print raw issuer
and audience values to stdout. Keep the output owner-only and do not send it to operational logs,
metrics, or a shared build artifact. Operational logging omits those raw values.
...
| `evidence_oid4vci_outcomes_total` | Counter | Offer, code, token, nonce, proof, issuance, upstream Evidence, saturation, and cleanup outcomes |
| `evidence_oid4vci_store_entries` and `evidence_oid4vci_store_capacity` | Gauges | Current process-local state pressure and its aggregate ceiling |
...
`evidence_unavailable`, and `cleanup_expired`. These labels contain no request-derived or
...
Size `store.maximumOffers` for the maximum live offer window. Saturation refuses a new offer
