...
token to present to a resource server such as Evidence. If you only need a token at a terminal,
...
| The resource server rejects a token Registry Mint issued | The two deployments name different claims for the same authority field. See [How Evidence verifies these tokens](../../reference/mint/#how-evidence-verifies-these-tokens). |
