...
A token selects one profile per request, and that profile decides everything the request may do: which operations, fields, rows, lookups, and paths it may touch.
Profiles are never merged, and naming a profile in a request grants nothing the profile does not already allow.

Before the server looks at any record, it admits the request against the profile: the token's principal claim must be present, every scope in `requiredScopes` must be present, the token's purpose must match one of `requiredPurposes`, and every row boundary must find its claim.
...
| `requiredScopes`, `requiredPurposes` | Every listed scope, and one of the listed purposes, must be present in the token. An empty `requiredPurposes` imposes no purpose restriction. |
...
Rows outside the boundary do not exist for the caller, on reads and on writes: a get returns 404, and a list omits them.
...
Put them on entities whose data must never leave a jurisdiction or a purpose, so a later profile cannot widen access by accident.
...
The report lists, per entity and profile, what was compiled.
...
Read it the way a reviewer would: every field a profile can read, every row it can reach, and every operation it holds are in that block, and nothing outside it applies.
