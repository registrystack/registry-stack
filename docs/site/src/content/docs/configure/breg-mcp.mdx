...
applications it answers `not-permitted`. An `update_application` identical to one that already
applied returns the draft as it now stands, even though its revision has moved on; any other update
from an earlier revision answers `stale-application` and changes nothing.
...
{/* Evidence: crates/registry-breg-mcp/src/tools.rs, start_input_schema(), update_input_schema();
    ...
    crates/registry-breg-mcp/src/gateway.rs, Gateway::update(), UNMATCHED_ETAG;
