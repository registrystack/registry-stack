...
| `application.targetField` | The API name of the field referencing the citizen's record |
| `application.ownerField` | The API name of the field recording the citizen's principal |
...
API name the registry's metadata publishes, such as `newAddressLine` for a field declared as
`new-address-line`.
...
`registry.targetField` takes the field identifier as the project declares it, in lowercase with
hyphens, while the gateway's `application.targetField` takes the API name.
