...
and replaces the output only after the complete index validates. Run it again only when you intend
to publish a new index revision.
...
{/* Evidence: `crates/registry-discoveryctl/src/build.rs`, `fetch_origins()` and `atomic_replace()`;
    ...
    `failed_origin_fetch_leaves_the_previous_output_untouched`. */}
...
{/* Evidence: `crates/registry-discovery/src/startup.rs`, `RuntimeConfig`, `load_runtime()`, and
    ...
    `crates/registry-discovery/src/server.rs`, `READY_ROUTE`. */}
