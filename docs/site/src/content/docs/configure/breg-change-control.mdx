...
| `review.stages` | Stages run in order. Each needs `approvals` distinct approvers, and `excludeSubmitter` keeps the submitter out of that stage. A request that needs no review declares `review.mode: none` instead of stages; submitting it moves it straight to `approved`. |
