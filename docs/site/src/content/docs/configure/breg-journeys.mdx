...
An error is a declaration the compiler cannot compile, printed as an `error` row carrying its code, its document path, and its message, and the command exits with status 1.
A finding is an advisory attached to a document path: the project compiles, but a reviewer must read it.
In JSON output, the `findings[]` array carries that classification and each element omits the
constant `severity` field. Refusal reports use `diagnostics[]` and retain `severity`, including when
`--deny-findings` promotes findings to a refusal.
The project `init` wrote reports both findings on purpose: `access.profile.unrestricted_collection`, because its `operator` profile lists every record, and `access.profile.unrestricted_rows`, because its `evidence-source` profile looks up any record by code.
...
`--deny-findings` makes every finding fail the command, so the first one prints `bregctl check refused.` above the same two findings and exits with status 1 until they are resolved or the profiles are changed.
`--production` applies the production closure rules: package identity, a lock entry with a pinned digest for every module, a loaded source for every lock, and a declared handler for every event must all be present, and a project missing one fails with `package.identity.required`, `module.lock.missing`, `module.lock.digest_required`, `module.source.required`, or `event.delivery.required`.
...
`access` prints the compiled permissions per profile in a human-readable form and, with `--scenario`, previews admission; [control access per profile](../breg-access/) covers it.
...
| `access.profile.row_boundary_not_writable` | The profile grants `create` and a row boundary whose field it cannot write, so the row policy refuses every create. | Add the boundary field to the writable fields, or remove `create` from the permission. |
...
| `access.profile.unrestricted_rows` | The profile's granted operations reach every row and no claim-bound boundary narrows them. | Add a claim-bound row boundary, or accept a registry-wide role knowingly. |
| `access.profile.writable_row_boundary` | The profile can write a field a row boundary depends on. | With `create`, keep it writable: the row policy pins it to the caller's claim on insert. With `patch` alone, remove it from the writable fields unless moving records is intended. |
| `entity.list.unindexed_filter` | No index leads with a filterable list field, so a filtered list scans every row the caller may read. | For an entity that will hold more than a few thousand rows, declare an index that leads with the field under `entities[].indexes`. |
| `entity.list.unindexed_sort` | No index leads with a sortable list field, so a sorted list orders every row the caller may read. | For an entity that will hold more than a few thousand rows, declare an index that leads with the field under `entities[].indexes`. |
...
Under `--production`, the `module.*` and `package.*` findings become errors; `manifest_projection.missing` and the `entity.list.*` findings are reported only in authoring mode, so resolve them before you leave this page.
...
| `request.operation` | `create`, `get`, `list`, `query`, `lookup`, `read_path`, `patch`, `batch`, `target_conditions`, `invoke`, or a source-owned change-request action: `submit_request`, `revise_request`, `cancel_request`, or `apply_request`. External review decisions are tested in Casework, not as BReg fixture operations. |
...
| A journey step fails with 404 where you expected data. | The profile's row boundary or purpose excluded the caller. Compare the step's `claims` with the permission, or preview them with `explain access --scenario` ([control access per profile](../breg-access/)). |
