...
QGIS's OAuth2 request hook detects the expiring token, discards a Client Credentials session that
has no refresh token, and performs the Client Credentials exchange again before applying
authentication to the next layer request.
