...
| `casework.source-add.row-boundary-claim-unsupported` | The selected request's review or apply access profile declares `rowBoundaries`, and the local dev-client export does not add the claim(s) it depends on. | Add the named claim(s) to the local Casework reviewer dev clients that need the profile, matching the values BReg's `rowBoundaries` expects. |
