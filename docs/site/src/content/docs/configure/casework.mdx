...
BReg names an initiator only for a human caller, so a request a service or agent submits for an `excludeInitiator` kind is refused when Casework admits it.
