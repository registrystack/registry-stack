...
The result travels back to the Requester on the terminal item and survives as long as the display payload does; [Retain, erase, and settle](../../operate/casework-retention/) covers the clocks.

The Requester that creates an item may narrow those result fields for that one item by sending `resultConstraints` beside the display.
A constraint names one top-level property of `resultSchema` and admits only values the schema already admits, so a Requester can pick among the operator's choices or tighten a bound, and can never introduce a field, add a keyword, or widen a range.
...
Constraints are stored verbatim and folded into the create idempotency hash, so a replay under the same key with different constraints is a conflict rather than a silent change.
...
{/* Evidence: crates/registry-casework-core/src/hosted.rs, HostedKindPolicy,
    HostedRetentionPolicy, HostedOutcomePolicy, check_closed_object_schema(),
    HostedCreateRequest.check(), and HostedDecisionRequest.check(). */}
...
Without `--apply` it previews the six places it would update in the BReg project: the lifecycle
event, the `casework-reader` access profile, its local reader client, and the three Casework clients.
