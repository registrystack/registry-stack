...
Every `caseworkctl` command prints one JSON report on standard output, and a refused command prints a `caseworkctl.refused` diagnostic and exits non-zero.
...
`init` refuses a destination that already exists, with `casework command refused: destination already exists; init never overwrites a project`.
Point it at a new directory, or edit the project you already have.
`casework.yaml` is the policy a deployment serves: `operator.example.yaml` is a starting point for the deployment's own file, `dev-clients.yaml` describes local callers, `fixtures/` holds the synthetic cases `caseworkctl test` runs, and `sources/` waits for the imported description of any source you connect.
...
The report repeats the digest and states `"secretsIncluded": false` and `"operatorConfigurationIncluded": false`, because the operator file and its secrets stay outside the package and outside review.
Packaging refuses an existing output directory with `casework command refused: policy package output already exists`, so each candidate lands in its own new directory.
...
When the register carries no entity by that name, the connection refuses with `casework command refused: declared request entity is absent from BReg compiled metadata`.
...
Package the connected project and hand it to an operator, who adds the matching source binding to the operator file.
...
| `operator.example.yaml` | The starting point for the deployment's own file |
...
The operator file, the database credentials, the identity provider, the audit key, and the source bindings are theirs, and none of them belongs in your project or your package.
