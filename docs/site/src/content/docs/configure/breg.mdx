...
`pattern` stays allowed on an encrypted field, and it keeps its meaning: the rule is enforced in
Rust against the plaintext before the value is sealed, so a value the pattern refuses never reaches
the database.
...
{/* TODO[evidence]: the encrypted member, the allowed types and classification, the closed
    normalization vocabulary in declared order, the unique index, the pre-seal pattern
    enforcement, and the refusal summary are Phase 1 behavior of registry-stack issue #1108,
    whose implementation is not yet in this tree, so no anchor resolves today. When the
    field-encryption milestone lands, anchor this section against the field contract in
    crates/registry-breg/src/contract.rs, the compiler refusals in
    crates/registry-breg/src/compiler.rs, and the authoring schema under
    products/breg/generated/authoring/. Until then the claims in this section are written at
    the strength of the approved contract, not of code this checkout holds. */}
