...
| `handler.kind` | `url`. An entity hook delivers to a bound destination; the engine refuses a local handler kind. |
| `handler.destinationId` | The key the runtime configuration binds to a receiver URL and signing key. A production compilation requires every hook to name one. |
