...
refuses startup or reload when that response could exceed the shared private-key-JWT provider's
16 KiB read ceiling.
...
Registry Mint fixes these values in the server-side registration. The token request has no
`scope` or authority form member, and Mint does not copy authority from the client assertion.
