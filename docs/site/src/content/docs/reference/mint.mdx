...
| `principal` | string (<=512 bytes) | required | |
| `evidenceAudience` | string | required | |
| `requesterTags` | list of strings (<=32 entries) | required | |
...
| `keys` | list of public JWKs (<=8 entries) | required | A document carrying a private key member is rejected. |
