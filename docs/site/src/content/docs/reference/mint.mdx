...
| `hashKeyFile` | path | required | At least 32 bytes in a regular, owner-only, single-link file. Must differ from signing and audit paths. |
...
and its parent directory for each append. It has no automatic audit rotation or deletion, so the
operator owns capacity, backup, and retention for `audit.path`. Verify retained records with:
...
| `GET /health` | Liveness. |
| `GET /ready` | Readiness. Returns `503` while no client is registered or after an audit write failure poisons the writer. |
...
source, the acceptance definition, or the assertion. The remaining exchanges are Evidence's, and
Evidence's only knowledge of Registry Mint is a URL and a key set: it reads the token by the claim
names in its own
...
[How Evidence verifies these tokens](#how-evidence-verifies-these-tokens) for the claim names.
...
Evidence from verifying a token: with no key set ever retrieved, every request is rejected until
one can be, and with a key set already held, that held set keeps being accepted only until its
allowance runs out. Evidence names the outage in either case rather than failing silently.
