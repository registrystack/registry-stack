...
`crates/registry-mint/src/error.rs`. Readiness has the separate response described below.

| Code | Status | When |
| --- | --- | --- |
| `invalid_request` | `400` | The content type is wrong; the bounded request body is unreadable, oversized, or timed out; a recognized form field is missing or duplicated; authentication methods are mixed; or `client_assertion_type` is not the exact `jwt-bearer` URN. Unknown form members are ignored. A missing `grant_type` lands here, not below. |
