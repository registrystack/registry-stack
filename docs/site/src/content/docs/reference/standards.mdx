...
are tracked at
[`products/relay-v2/acceptance/labour-statistics`](https://github.com/registrystack/registry-stack/tree/v0.19.0/products/relay-v2/acceptance/labour-statistics).
...
- OGC API Features and OGC API EDR were feature-gated routes on the retired Relay V1 runtime, and no
  ...
  surface is the CRS84 Point profile above, which makes no OGC API conformance claim. OGC API Records
...
- ODRL is still emitted by Registry Manifest as policy metadata, and that metadata is descriptive
  ...
  evaluation path belonged to Relay V1, and `registry-platform-pdp` keeps the enforcement profile
  vocabulary with no maintained consumer.
- The Verifiable Credentials Data Model (W3C VC) is listed as `aligns_with` (`referenced`).
  ...
  W3C VC Data Model envelope. Relay does not emit or host VC artifacts either.
