...
The initial profile is one classified Point in CRS84, with an exact bounded bounding-box query on
a separately named `consultation.search` operation. The operation's access profile remains the
authorization and maximum-disclosure boundary. This does not claim OGC API Features support and
does not inherit the separate Relay 1.0 adapter described in the roster.
