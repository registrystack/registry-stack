...
## Relay V2 spatial representation profile

Relay V2 uses GeoJSON and JSON-FG as optional representations of the same
governed Registry Record. The initial profile is one classified Point in
CRS84, with an exact bounded bounding-box query on an existing consultation
route. This does not claim OGC API Features support and does not inherit the
separate Relay 1.0 adapter described in the roster.
