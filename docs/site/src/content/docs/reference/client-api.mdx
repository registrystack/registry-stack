...
Installing `@types/node` is not sufficient on its own: if `tsconfig.json` sets `compilerOptions.types`
to an explicit list, `Buffer` still reports `TS2591` unless that list includes `"node"`:
