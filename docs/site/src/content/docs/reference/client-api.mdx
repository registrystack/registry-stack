...
`createRecordJson`, `patchRecordJson`, and `executeLifecycleActionJson` return a `JsonOutcome`: the
validated response as JSON text in `valueJson`, with the same trace, ETag, location, and
continuation semantics as their object counterparts. `lifecycleActionsJson` takes a complete record
