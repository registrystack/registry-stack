...
description: Construction, operations, outcomes, and errors for the Registry Discovery, Evidence Gateway, Registry Relay, Base Registry Engine, and Registry Casework namespaces of the unified Registry Stack client.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
{/* Evidence: crates/registry-breg-client/src/webhook.rs, verify_webhook_delivery();
    ...
    crates/registry-platform-crypto/src/breg_webhook.rs, verify_v1(). */}
...
{/* Evidence: crates/registry-stack-client-node/breg/client.d.ts,
    ...
    crates/registry-breg-client-node/client.d.ts, JsonOutcome, valueJson, bodyJson, reviewJson,
...
The Casework client owns Casework routes, its work item, hosted item, and directory types, its closed
...
page = client.list_hosted_work_items(token, "staff", {"view": "mine", "limit": 25})
```
...
third. `assignWorkItem`, `delegateWorkItem`, `previewCaseloadMove`, and `applyCaseloadMove` take the
source profile as a trailing optional argument, so omitting it addresses hosted work. In Rust the
...
- __Service description.__ `description` returns the queues, sources, calendars, clocks, and hosted
  kinds the selected profile may see.
- __Requester.__ `create_hosted_item`, `get_hosted_item`, `add_hosted_note`,
  `requester_hosted_notes`, `cancel_hosted_item`, and `hosted_terminal_items` cover the hosted items
  one Requester owns.
- __Staff on hosted work.__ `list_hosted_work_items`, `get_hosted_work_item`,
  `hosted_work_item_history`, `claim_hosted_work_item`, `release_hosted_work_item`, and
  `decide_hosted_work_item` work the hosted inbox without a source profile.
...
- __Supervisor.__ `holdings`, `assign_work_item`, `delegate_work_item`, `preview_caseload_move`,
  `apply_caseload_move`, and `hosted_accountability_record` for a retained accountability record.
...
attempt identifier when the refusal named one, and the hosted validation detail when the service
...
| `invalid_request` | An input was refused before any exchange | none |
| `transport` | The exchange failed below HTTP | `transport_kind` |
| `problem` | Casework answered with an accepted problem | `status`, `code`, `detail`, `trace_id`, plus `original_attempt_id` on a recoverable attempt and `validation` on a refused hosted submission |
...
`maximum_depth_exceeded`, `schema_mismatch`, `outcome_not_declared`, `reason_required`, and
`text_invalid`. Neither member repeats a submitted value.
