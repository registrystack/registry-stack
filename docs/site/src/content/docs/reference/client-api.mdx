...
The Messaging client covers four routes: liveness, readiness, the submission of one message, and
the read of one message's status. It carries no route for cancelling a message or previewing a
template, and no operator route. The client never reaches the Messaging runtime or its operator
tooling; it speaks the public HTTP contract any caller uses.
...
{/* Evidence: crates/registry-messaging-client/src/client.rs, MessagingClient, submit(),
    message(), is_idempotency_key(), and is_message_id();
...
them still answers a refusal the caller can match. The error carries no `Retry-After` value, so a
`rate-limit.exceeded` or `quota.exceeded` refusal names the limit but not the wait.

Python raises `messaging.MessagingClientError`, an `Exception` whose attributes are `kind`, `code`,
`title`, `detail`, `status`, `trace_id`, `transport_kind`, and `protocol_failure`; an absent member
is `None`. Node rejects with `messaging.MessagingClientError`, an `Error` with `kind` and the
optional `code`, `title`, `detail`, `status`, `traceId`, `transportKind`, and `protocolFailure`; an
absent member is undefined.
...
| `invalid_request` | An input was refused before any exchange | none |
| `transport` | The exchange failed below HTTP | `transport_kind` |
| `problem` | Messaging answered with an accepted problem | `status`, `code`, `title`, `detail`, and `trace_id` |
| `protocol` | The response violated the contract | `status`, `protocol_failure`, and `trace_id` when one was validated |
...
{/* Evidence: crates/registry-messaging-client/src/error.rs, MessagingClientError and
    ...
    crates/registry-messaging-client-py/python/registry_messaging_client/__init__.pyi,
    MessagingErrorKind; crates/registry-messaging-client-node/client.d.ts, MessagingClientError */}
