...
the base URL, the pinned key set, and the credential configuration in one file. The optional second
...
```python
client = evidence.EvidenceClient.from_profile("evidence-client.yaml")
```

```js
const client = evidence.EvidenceClient.fromProfile('evidence-client.yaml');
```
