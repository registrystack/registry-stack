...
`required_for_submit`, `maximum_bytes`, `content_types`, `can_download`, `can_upload`, `can_remove`,
and `accepts_content_type(content_type)`, camel case in Node. The served routes stay opaque, so only
the client can spend a slot.
