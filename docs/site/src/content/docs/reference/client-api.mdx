...
The five result reasons govern a hosted item's structured result. `result_not_declared` refuses a
result or a result constraint when the item's kind declares no `resultSchema`. `result_required`
