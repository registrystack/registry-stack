...
Engine. Each namespace keeps its own routes, authentication, errors, and verification rules, because
those contracts differ. Relay and BReg share only the neutral `registry-record` types they decode
Registry Record responses into.
...
A TypeScript consumer also needs the Node type definitions. The published declarations name
`Buffer`, a Node.js global, so a project compiling against them without `@types/node` gets a
`TS2580` error on every use of it:
...
Every method performs at most one service exchange. A configured private-key-JWT provider can make a
separate token-endpoint exchange when it needs to acquire or refresh a bearer. No client follows
redirects, reads ambient proxy configuration, retries an exchange, fetches referenced schemas, or
advances pagination on its own.
...
and verifies every response against a pinned key set. Verification is the point of the client: the
deployment's own published key set is never fetched to judge a response it also produced.
...
the base URL, the pinned key set, and the credential configuration in one file. The profile is the
owner-only JSON document `evidencectl client profile create --output` writes. The optional second
argument is a private JWK held in memory, for an application that reads its key from a secret
manager rather than from disk. It is never retained by the wrapper, and profile paths and
file-backed secrets are redacted from every error.
...
{/* Evidence: crates/registry-stack-client-node/evidence/client.d.ts,
    ...
    crates/registry-evidence-client/src/profile.rs, EvidenceClientProfile::from_file */}
