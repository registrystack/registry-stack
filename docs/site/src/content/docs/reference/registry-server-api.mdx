...
proposal; a proposal changes its targets only when an applier runs `apply`. The record moves
through seven server states:
...
{/* Evidence: crates/registry-server/src/request_workflow.rs, RequestState;
    ...
    crates/registry-server/src/artifacts.rs, request_action_response_schema();
