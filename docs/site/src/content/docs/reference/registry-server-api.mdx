...
Lookup reference for the HTTP API a compiled Registry Server package serves and for the events
it emits. Every registry serves the same route shapes; the entity routes, fields, profiles, and
actions come from your project. The contract is a source preview, not a frozen compatibility
promise.

For the authoring grammar, use [author a registry project](../../configure/registry-server/).
For deployment, receivers, and recovery, use
[deploy and operate a registry](../../operate/registry-server/).
For every YAML key, use the [configuration reference](../registry-server-configuration/).
...
`iss`, `aud`, and `exp`, and its scopes come from the claim named by `scopeClaim` (default
`scope`, space separated). The principal and purpose claims are the ones named under
`authorityClaims`.
...
{/* Evidence: crates/registry-server/src/auth.rs;
    ...
    crates/registry-server/src/query.rs, access_profile;
    crates/registry-server/src/contract.rs, ProjectAccessProfileSource and AccessGrantSource. */}
...
{/* Evidence: crates/registry-server/src/api/mod.rs, router(), registry_metadata(), entity_schema(), revision_dispatch(), and tombstone_dispatch();
    crates/registry-server/src/compiler.rs, route_shape() and query_kind_id();
    crates/registry-server/src/model.rs, MAX_REVISION_HISTORY_RECORDS. */}
...
| `ETag` | `"rs-hmac-sha256:<64 hex>"`, a strong validator of the current revision. Send it back in `If-Match`. |
...
{/* Evidence: crates/registry-server/src/artifacts.rs, record_member_schema() and field_schema();
    crates/registry-server/src/api/mod.rs, exact_read() and exact_mutation();
    crates/registry-server/src/compiler.rs, reserved_logical_name();
    products/registry-server/acceptance/registry-record-conformance/registry.yaml. */}

## Collections and paging

A collection read returns the matching envelopes as `items`, a `pageInfo` object, and the same
`meta` as a single record:

```json
{
  "items": [{"data": {"recordIdentifier": "…", "revisionIdentifier": "1", "domainData": {}}}],
...
other query option except `accessProfile`. The cursor is signed with the runtime `cursor.secret`,
binds the whole original query, and expires after `cursor.maxAgeSeconds`; a cursor that fails
any of those checks returns `400 query.cursor_invalid`. With `$count`, the response carries the
matching total beside the page.

{/* Evidence: crates/registry-server/src/query.rs, ParsedReadQueryMode;
    crates/registry-server/src/api/mod.rs, cursor_invalid();
    crates/registry-server/src/runtime_config.rs, CursorConfig. */}
...
{/* Evidence: crates/registry-server/src/query.rs, parse_filter(), MAX_SELECTED_FIELDS, MAX_IN_VALUES, MAX_FILTER_PREDICATES, MAX_FILTER_DEPTH, MAX_FILTER_NODES, MAX_LITERAL_BYTES, MAX_OPAQUE_VALUE_BYTES, MAX_QUERY_PAYLOAD_BYTES, and MAX_BBOX_PARAMETER_BYTES;
    crates/registry-server/src/api/mod.rs, MAX_TOP. */}
...
```json
{
  "items": [
    ...
    {"operation": "patch", "id": "2d7e5e0c-…", "ifMatch": "\"rs-hmac-sha256:…\"", "patch": [{"op": "replace", "path": "/data/label", "value": "West depot"}]}
  ],
  "changeContext": {
    "kind": "correction",
...
{/* Evidence: crates/registry-server/src/api/mod.rs, patch_dispatch(), tombstone_dispatch(), and precondition_required();
    crates/registry-server/src/mutation/request.rs;
    crates/registry-server/src/postgres/mutation.rs;
    crates/registry-server/src/artifacts.rs, batch_response_schema;
...
{/* Evidence: crates/registry-server/src/api/mod.rs, revision_dispatch();
    crates/registry-server/src/query.rs, ParsedSnapshotQuery;
    crates/registry-server/src/postgres/history_read.rs;
    crates/registry-server/src/postgres/revision_read.rs;
    crates/registry-server/src/history_erasure.rs;
...
draft ──submit──▶ submitted ──approve (every stage)──▶ approved ──apply──▶ applied
  ...
  └── cancel from any state except applied and canceled ──▶ canceled
```
...
```json
{
  ...
  "actions": [
    {"operation": "approve_request", "stage": "review", "href": "/v1/records/placement-corrections/2d7e…/actions/stages/review/approve", "ifMatch": "\"rs-hmac-sha256:…\""}
  ]
}
```
...
plan adds `queueReason` with its `code` and `label`.
...
{/* Evidence: crates/registry-server/src/request_workflow.rs, RequestState;
    crates/registry-server/src/api/mod.rs, request_action_dispatch();
    crates/registry-server/src/artifacts.rs, request_action_response_schema() and request_capability_metadata();
    crates/registry-server/src/fixtures.rs, assert_request_action_shape();
    crates/registry-server/src/postgres/request_read.rs;
    products/registry-server/acceptance/asset-site-placement-change-requests/registry.yaml;
    products/registry-server/acceptance/person-name-change-rhai/registry.yaml. */}
...
{
  "input": {"assetCode": "AST-0042", "siteCode": "SITE-001"},
  "preconditions": {"asset": {"ifMatch": "\"rs-hmac-sha256:…\""}}
}
```
...
{/* Evidence: crates/registry-server/src/api/mod.rs, router();
    crates/registry-server/src/contract.rs, ActionSource;
    crates/registry-server/src/fixtures.rs;
    products/registry-server/generated/household-contact-actions/generated/openapi.json. */}
...
```json
{
  "type": "urn:registry-server:problem:precondition.failed",
...
`traceId` is a 32-character hexadecimal value that also appears in the server's audit record for
the request, so an operator can find the refusal without the client sending any record value.
`fieldPath` names the offending body member when the refusal is about one.

<ApiReferenceTable id="problem-codes" />

{/* Evidence: crates/registry-server/src/api/mod.rs, fixed_problem(), lookup_unresolved(), cursor_invalid(), and precondition_required();
    crates/registry-server/src/auth.rs;
    crates/registry-server/src/artifacts.rs, problem_schema. */}
...
{/* Evidence: crates/registry-server/src/compiler.rs, validate_event_condition();
    ...
    crates/registry-server/src/request_events.rs, lifecycle_condition_matches() and insert_request_lifecycle_events();
...
{/* Evidence: crates/registry-server/src/webhook.rs, reload_and_send();
    ...
    crates/registry-server/src/request_events.rs, insert_request_lifecycle_events();
...
by `hmacSha256KeyRef`, at least 32 bytes, neither trimmed nor hex-decoded. This is a Registry
Server format, not a CloudEvents signing standard.

The signing input starts with the ASCII bytes `registry-server-webhook-signature-v1`. Each later
...
before it acts. The accepted skew is receiver policy, not a Server limit.
...
{/* Evidence: crates/registry-server/src/webhook.rs, claim(), finalize(), replay(), webhook_idempotency_key(), verify_retained_bindings(), WebhookDeliveryService, and WebhookDeliveryStatusKind;
...
Command prefix `registry-serverctl`; add `--format json` for a machine-readable report.

<EventReferenceTable id="commands" />

`list` and `replay` connect directly to PostgreSQL through the runtime configuration. The public
...
`registry-server-gis-bounded-crs84-point-bbox-v1`. It does not declare OGC API Features
conformance, compute collection extents, or add a query engine of its own.

{/* Evidence: crates/registry-server/src/api/gis.rs;
    products/registry-server/acceptance/spatial-service-sites/README.md. */}
...
{/* Route, query, problem, and request tables generated from docs/site/src/data/server-api.yaml,
    protocol tables from docs/site/src/data/server-events.yaml, by
    docs/site/scripts/generate-data.mjs. Run npm run generate from docs/site. */}

## Next

- [Deploy and operate a registry](../../operate/registry-server/) for runtime configuration, receivers, replay, and erasure.
- [Author a registry project](../../configure/registry-server/) for entities, profiles, change requests, and actions.
- [Configuration reference](../registry-server-configuration/) for every project, module, and runtime key.
- [Create and query your first registry](../../tutorials/first-registry-server/) to see the envelope and headers in a running server.
