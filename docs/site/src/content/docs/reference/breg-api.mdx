...
Approve, reject, request-revision, and apply bodies may also include an optional `reason` string of up
to 4096 Unicode characters: the approver's or applier's recorded remark, not only an explanation of a
refusal. The server preserves whitespace and Unicode exactly, accepts an empty string, and refuses
null, other types, NUL, or longer text with `400 request.invalid`. Include the same reason when
replaying an action with its original idempotency key; changing it is a different request body.

Read the current proposal's decisions in `data.request.decisions`. Each decision has `stageId`,
`kind`, `decidedAt`, and `reasonPresent`; `reason` appears only when retained and permitted by the
selected profile. Retained proposal history exposes the same decision shape under
`data.request.history.proposals[].decisions`. A request grant defaults to
`readableRequestFields: [reason]`; set `readableRequestFields: []` to hide explanation text while
keeping the decision facts and reason-presence flag. Anonymous profiles never receive reason
text. Request-detail erasure removes the text while retaining reason presence and the decision
facts. The applied request's `data.request.application` carries the same disclosure for the applier's
remark: `reasonPresent` always, and `reason` under the same `readableRequestFields` rule, with the
text removed but the presence retained by request-detail erasure.

`readableRequestFields` also accepts `review_state`, which discloses `review` and `reviewTiming`
on the request record, and `actor_reference`, which discloses `submitterReference` and
...
{/* Evidence: crates/registry-breg/src/artifacts.rs openapi_request_action_input_schema; crates/registry-breg/src/postgres/request_read.rs decisions_value; crates/registry-breg/src/contract.rs readable_request_fields; crates/registry-breg/src/request_retention.rs; crates/registry-breg/tests/postgres_request_upgrade_retention.rs. */}
...
without splitting a proposal's decisions. To continue, send the returned
...
URL. Owner, stage reviewer, and applier access use their current compiled request
read authority, including request entries in `reviewStages[].targets[]` and
`applyTargets`. Field projection also applies: a slot reaches a caller only
