...
An event is declared as a hook on an entity or a module extension and delivered to a webhook
destination bound in the runtime configuration. Field paths in the next table are relative to one
...
- An entity hook declares `phase: after` and `handler.kind: url`; the compiler refuses a phase or
  a handler kind it cannot run.
