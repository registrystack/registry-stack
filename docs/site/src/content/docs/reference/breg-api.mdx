...
unapplied reports `review.application.state` `expired` beside the result's `availableUntil`; the
other application states are `awaitingReview`, `ready`, `queued`, `applying`, `applied`, and
`blocked`.
...
{/* Evidence: crates/registry-breg/src/postgres/request_read.rs, action_is_available and revise_rebase_available;
    ...
    expired_approval_hides_apply_and_is_answered_by_a_revision, and
