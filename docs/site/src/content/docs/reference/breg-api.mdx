...
every send, and the terminal audit entry is accepted before the state transition and payload
erasure commit together.
...
{/* Evidence: crates/registry-platform-hooks/src/delivery/service.rs, claim(), finalize(), replay(), delivery_idempotency_key(), and verify_retained_bindings();
