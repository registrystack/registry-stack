...
{/* Evidence: crates/registry-breg/src/auth.rs;
    ...
    crates/registry-breg/src/contract.rs, ProjectAccessProfileSource and AccessPermissionSource. */}
...
An ingestion run is the durable account of one bulk import that the client drives one bounded
chunk at a time. The client keeps the source file; the run keeps the checkpoint. A run binds the
active package revision, schema fingerprint, entity, selected access profile, create-or-patch
operation, input digest, chunking algorithm, and the expected item and chunk counts. No server
worker advances a run: only a chunk submission moves the checkpoint, and every operation rechecks
the caller's authority against the compiled batch route. A run identifier locates a run and
grants nothing. Runs are visible to the caller that created them, and another caller's run and
an unknown run are the same concealed 404.

A run response carries operational metadata and bounded failure classifications only: never
source rows, committed record values, or chunk bodies.

{/* Evidence: crates/registry-breg/src/ingestion_store.rs, IngestionRunStatus and IngestionRefusal;
    crates/registry-breg/src/data.rs, ingestion_batch_route() and ingestion_chunk_idempotency_key();
    crates/registry-breg/src/api/mod.rs, router(). */}

### Create a run

`POST /v1/records/{route}/ingestion-runs` with the binding:

```json
{
  "operation": "create",
  "profileId": "operator",
  "packageRevision": "sha256:9f2c…",
  "schemaFingerprint": "sha256:1a7b…",
  "inputDigest": "5e8d…",
  "inputLength": 48211,
  "itemCount": 12000,
  "chunkCount": 13,
...
The response is `201 Created` with the run document. `inputDigest` is the SHA-256 of the whole
...
Every run response is one document:
...
{/* Evidence: crates/registry-breg/src/ingestion_store.rs, IngestionRunRecord and IngestionAttemptOutcome;
    crates/registry-breg/src/data.rs, RUN_CHUNK_ALGORITHM_VERSION. */}

### Submit chunks

`POST /v1/records/{route}/ingestion-runs/{runId}/chunks` submits the chunk the run's
...
{/* Evidence: crates/registry-breg/src/ingestion_store.rs, IngestionChunkBinding and IngestionChunkCommit;
    crates/registry-breg/src/data.rs, canonical_chunk_body() and ingestion_chunk_idempotency_key(). */}

### List and read runs

`GET /v1/records/{route}/ingestion-runs` lists the caller's runs: `limit` (default 25, at most
...
{/* Evidence: crates/registry-breg/src/ingestion_store.rs, MAX_RUN_PAGE_SIZE and DEFAULT_RUN_PAGE_SIZE. */}

### Cancel and recover

`POST /v1/records/{route}/ingestion-runs/{runId}/cancel` cancels an open or blocked run. The
...
{/* Evidence: crates/registry-breg/src/ingestion_store.rs, StoredChunkReceipt and IngestionBlockedReason;
    crates/registry-breg/src/data.rs, ingestion_batch_route(). */}
...
A run drives the same compiled batch route one chunk at a time, so an existing direct batch
client keeps its contract unchanged.

{/* Evidence: crates/registry-breg/src/data.rs, ingestion_batch_route(). */}
...
```text
draft ──submit──▶ submitted ──apply with exact approved evidence──▶ applied
  ▲                   │
  └────revise─────────┘
draft or submitted ──cancel by owner──▶ cancelled
```

`submit`, `revise`, and `cancel` belong to the principal that owns the request. Another principal
...
```json
{
  ...
  "proposal": {"review": {"authority": "casework", "policyId": "asset-placement-correction"}},
  "review": {
    "submission": {"state": "accepted", "authority": "casework", "requestId": "…", "submissionDigest": "sha256:…", "policy": {"id": "asset-placement-correction", "version": "1", "digest": "sha256:…"}},
    "result": {"state": "approved", "resultId": "…", "completedAt": "2026-09-20T00:00:00.000000Z", "availableUntil": "2026-10-20T00:00:00.000000Z"},
    "delivery": {"state": "reconciled", "eventId": "…", "receivedAt": "2026-09-20T00:00:01.000000Z"},
    "application": {"mode": "manual", "state": "ready"},
    "recovery": {"state": "none"}
  },
  "editable": false,
  "actions": [
    {"operation": "apply_request", "href": "/v1/records/placement-corrections/2d7e…/actions/apply?accessProfile=correction-applier", "ifMatch": "\"breg-hmac-sha256:…\"", "proposalVersion": 2, "effectDigest": "sha256:9f2c…"}
  ]
}
```
...
```text
$filter=bregState eq 'submitted' and reviewOutcome eq 'approved'
$filter=bregState eq 'submitted' and reviewOutcome eq 'approvedExpired'
```

A profile that does not disclose `review_state` is refused the filter with `400 query.invalid`, the
...
{/* Evidence: crates/registry-breg/src/postgres/request_read.rs, action_is_available and revise_rebase_available;
    crates/registry-breg/src/mutation/request.rs, the settled review outcome check;
    crates/registry-breg/src/review_store.rs, settled_outcome, read_projection, and approved_evidence;
    crates/registry-breg/src/review_integration.rs, from_approved;
    crates/registry-breg/src/postgres/read.rs, REQUEST_REVIEW_OUTCOME_SQL;
    crates/registry-breg/src/compiler.rs, query_operation;
    crates/registry-breg/tests/postgres_change_requests.rs, rejected_review_offers_only_cancel_and_refuses_revise_and_apply,
    changes_requested_review_offers_revision_and_cancel_and_refuses_rebase_and_apply,
    revision_after_changes_requested_records_revision_not_rebase,
    answered_review_hides_apply_and_keeps_revise_and_cancel,
    unexpired_approval_offers_apply_and_the_apply_succeeds,
    expired_approval_hides_apply_and_is_answered_by_a_revision,
    expired_automatic_approval_projects_expired_until_an_apply_is_in_flight, and
    request_list_filters_on_the_settled_review_outcome. */}
...
{/* Evidence: crates/registry-breg/src/artifacts.rs openapi_request_action_input_schema; crates/registry-breg/src/postgres/request_read.rs request_proposal_metadata and may_disclose_review_state; crates/registry-breg/src/review_store.rs read_projection; crates/registry-breg/src/contract.rs readable_request_fields; crates/registry-breg/src/request_retention.rs. */}
...
{/* Evidence: crates/registry-breg/src/request_retention.rs load_retained_history; crates/registry-breg/src/postgres/request_read.rs retained_history and bound_request_metadata; crates/registry-breg/src/api/mod.rs requestHistoryAfterProposalVersion. */}
...
fields it `set`s with where each value comes from, and the fields it `clear`s. A `rhai` planner
...
{/* Evidence: crates/registry-breg/src/request_workflow.rs, RequestState;
    ...
    crates/registry-breg/src/api/mod.rs, registry_metadata();
...
A request entity can declare up to eight `attachments[]` slots. Each slot holds
...
{/* Evidence: crates/registry-breg/src/contract.rs, AttachmentSlotSource;
    crates/registry-breg/src/artifacts.rs, attachment_metadata_schema() and openapi_attachment_operation();
    crates/registry-breg/src/api/metadata.rs, attachment_request();
    crates/registry-breg/src/attachment_store.rs;
    crates/registry-breg/src/request_workflow.rs, AttachmentManifestEntry. */}
...
An event is declared as a hook on an entity or a module extension. A `url` handler delivers to a
...
- An entity hook declares `phase: after`; the compiler refuses any other phase. `handler.kind` is
  `url`, `rhai`, or `wasm`; a local handler names its reviewed `script` or `module` path.
- Production compilation requires every hook to declare a handler, either a bound destination or a
  reviewed local program.
...
{/* Evidence: crates/registry-breg/src/contract.rs, HookSource and HookHandlerSource;
    crates/registry-breg/src/compiler.rs, validate_hooks() and compile_event_delivery_inventory();
    ...
    from products/breg/generated/authoring/registry-module.schema.json through
...
The body is one shared hook envelope, the same envelope every Registry Stack product delivers.
Its eight members are required and nothing else appears beside them. The envelope restates the
event identity the CloudEvents headers carry, so a receiver that reads only the body still knows
which event it holds; a body and its headers can never describe two different events. Delivery
attributes stay on the transport: attempt, generation, delivery time, idempotency key, and
signature are headers, never envelope members.

<EventReferenceTable id="envelope" />
...
```json
{
  "causation": {"hop": 0, "root": "<event-uuid>"},
  "data": {
    "entity": "record",
  ...
  "dataschema": "urn:breg:event-schema:<registry-id>:record:record-patched-v1:sha256:<digest>",
  "id": "<event-uuid>",
  "source": "urn:registrystack:registry:<registry-id>:instance:<instance-id>",
  "subject": {"recordReference": "hmac-sha256:<64-hex>", "recordRevision": 2},
  "time": "2026-01-01T00:00:00.123Z",
  "type": "record-patched-v1"
}
```

Members appear in canonical JSON order, the order the bytes on the wire use.

{/* Evidence: crates/registry-platform-hooks/src/envelope.rs, HookEnvelope, EventSubject, and Causation;
    crates/registry-platform-hooks/src/delivery/service.rs, reload_and_send() and accept_stored_envelope();
    ...
    crates/registry-breg/src/outbox.rs, insert_configured_events() and capture_envelope();
...
[webhook delivery verifier](../client-api/#verify-a-webhook-delivery) instead of constructing this
...
{/* Evidence: crates/registry-platform-crypto/src/delivery_signature.rs, SignatureFields, sign_v1(),
    and v1_binds_every_field_and_exact_body();
...
{/* Evidence: crates/registry-platform-hooks/src/delivery/service.rs, replay(), delivery_idempotency_key(), and verify_retained_bindings();
    crates/registry-platform-hooks/src/delivery/store.rs, finished_columns() and after_finished();
    crates/registry-platform-dispatch/src/postgres/dispatcher.rs, claim(), finish(), and replay();
    crates/registry-breg/src/webhook.rs, WebhookDeliveryService and WebhookDeliveryStatusKind;
