...
draft; a submitted proposal changes its targets only when it is applied. BReg owns five source
states:
...
an older proposal replaced by a newer one ──▶ superseded
```
...
cannot read is never given as a source. `possibleWriteCount` still counts every declared write.
Fields are named by their field id, as in `readableFields`. The description cannot grant write
