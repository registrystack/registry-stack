...
| Registry Notary HTTP API, CLI, config schema, and released image | None. Registry Notary is retired: it is not a surface an adopter can start on, and no compatibility promise applies to it or to the `ghcr.io/registrystack/registry-notary` image name | None. The retirement is not a deprecation with a migration window inside the promise; the product is gone |
...
Registry Notary is retired, so a reader arriving with a Notary integration should stop here rather
than plan an upgrade: its HTTP API, CLI, config schema, and image name are no longer covered
surfaces, and no future release of this stack restores them. Nothing in this promise, including
the deprecation policy, gives that surface a migration window, because the promise never took
effect before the product was retired. The stack's evidence-shaped answer is now Evidence, which
answers one bounded question about one subject with a signed assertion and shares neither Notary's
API nor its credential lifecycle.
...
The reverse is not promised. Registry Relay, Evidence, and Registry Mint all parse config with
...
Evidence and Registry Mint have no migrated state to upgrade. Evidence's only persistence is the
append-only audit log named by `auditStorage` in its runtime file, and Registry Mint keeps its
client registry on disk and its client-assertion replay cache in memory, so both start clean
...
- Adopter tooling that wraps a covered surface rather than defining one, including `evidencectl`,
  ...
  every Evidence semantic decision.
...
Evidence's two versioned routes, `POST /v1/evidence` and `GET /v1/evidence-definitions`, are the
...
`/openapi.json`, `/.well-known/evidence/jwks.json`, and `/.well-known/jwt-vc-issuer`. Evidence
...
- Evidence: root CI's `evidence-contracts` job regenerates every contract artifact from the code
...
- Config: Registry Relay reproduces its committed schema from its typed config graph, and
  Evidence's config parser is tested against the frozen contract schemas rather than a schema
