...
block in its runtime file. Relay's sealed package is an input to verify rather than state to migrate. `relay` reads the
...
- Forward state path: the release upgrade rehearsal workflow runs
  ...
  retained tables lose rows. BReg rebuilds and applies a successor before serving; its
...
{/* Evidence: .github/workflows/release-upgrade-rehearsal.yml; release/scripts/rehearse-upgrade.py, fetch_release(), breg_view_differences(), view_differences(), row_count_losses(), and check_forward_path(). */}
