...
and digest-verification interface remain covered release artifacts. What Registry Mint does hold
steady is structural rather than versioned: the `client_credentials` grant with registered
authentication, `private_key_jwt` (RFC 7523) by default or an explicitly selected standard
client-secret method, the collapse of every client authentication failure to `invalid_client`,
and authority written from the client registry rather than from the caller's request. The
[Registry Mint reference](../mint/) defines these surfaces.
