...
| Release artifacts and verification interface | Released binary asset names, the released `ghcr.io/registrystack/breg`, `ghcr.io/registrystack/casework`, `ghcr.io/registrystack/discovery`, `ghcr.io/registrystack/evidence`, and `ghcr.io/registrystack/relay` container images, and the signature and provenance layout in `release/VERIFY.md` | Release workflow; signed assets and exact image digests verified as documented in [SECURITY.md](https://github.com/registrystack/registry-stack/blob/v0.21.0/SECURITY.md) |
...
audit log named by `auditStorage` in its runtime file. Relay's sealed package is an input to verify rather than state to migrate. `relay` reads the
...
its migration, `casework migrate` applies Casework's migrations, and `scheduling migrate` applies
Scheduling's migrations. Before 1.0, an upgrade between releases of any of the three can require the
...
- Forward state path: the release upgrade rehearsal workflow runs
  ...
  describes, and writes records, review work, and an audit chain with them against disposable
  PostgreSQL. It then runs the upgrade steps with the binaries built from the pull request and
  fails when any captured view is served differently or any table holds fewer rows. It refuses to
  start from a release before `v0.33.0`. Scheduling state is not rehearsed.

{/* Evidence: .github/workflows/release-upgrade-rehearsal.yml; release/scripts/rehearse-upgrade.py, fetch_release(), view_differences(), row_count_losses(), and check_forward_path(). */}
