...
- Adopter tooling that wraps a covered surface rather than defining one, including `evidencectl`,
  which generates key material and project scaffolds, delegates runtime decisions to the
  `evidence` binary, and reuses the Evidence client and portable verifier for relying-party
  request preparation and offline response verification.
