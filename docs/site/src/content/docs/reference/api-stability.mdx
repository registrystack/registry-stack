...
| Signed config bundle format and verification semantics | `registry.platform.config_bundle.v1` manifests, `registry.platform.config_bundle_signatures.v1` signature envelopes, trust-anchor binding, file-closure checks, product/environment/stream/instance binding, and anti-rollback sequence checks | Shared verifier tests in `registry-platform-config`; product CLI coverage in `crates/registry-relay/tests/config_verify_bundle_cli.rs` and `crates/registry-notary/tests/config_verify_bundle_cli.rs` |
