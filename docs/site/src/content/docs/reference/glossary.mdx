...
<dl class="glossary-dl">
  ...
  <dd>An atomic, precise, reusable evidence statement that Registry Notary evaluates from Relay outputs or permitted self-attestation. Notary owns the claim's evidence semantics, authorization, and disclosure policy. A claim does not import a consuming programme's eligibility or action rules.</dd>
  ...
  <dt>Relay output</dt>
  <dd>A declared typed value returned by Registry Relay after source-specific acquisition and normalization on a matching consultation. A Relay output carries source evidence, not a consuming programme consequence. `true`, `false`, `null`, `no_match`, `ambiguous`, and source failure retain distinct meanings under the reviewed integration contract.</dd>

  <dt>claim</dt>
  <dd>Short form for a Notary evidence claim: a Registry Notary-owned evidence value or predicate derived from a consultation outcome and typed Relay outputs, or from permitted self-attestation. Purpose-bound authorization limits use of the claim without turning the claim into programme policy.</dd>
  ...
  <dt>programme decision</dt>
  <dd>An eligibility, prioritization, workflow, or action decision made by a consuming programme from evidence and its own rules. Registry Relay and Registry Notary do not own this policy or decision.</dd>
