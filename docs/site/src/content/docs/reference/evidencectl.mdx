...
| `evidencectl package <project> --target <target> --output <new-candidate>` | Editable project and one explicit production target | Creates a new candidate with `runtime.yaml` and closed `bundle/` | Overwrite output, contact source services or OIDC, create production secrets, or start a listener |
...
registered for this Evidence service. The default resource is
...
{/* Evidence: crates/registry-evidencectl/src/dev.rs, DevArgs, select_resource;
