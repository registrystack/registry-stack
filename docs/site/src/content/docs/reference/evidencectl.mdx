...
`build` is create-only. It rejects an existing output directory, unauthenticated or non-HTTPS
...
outside the allowed project directories. A failed build publishes no candidate.
