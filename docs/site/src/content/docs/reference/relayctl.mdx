...
description: Reference for the Relay adopter command line, covering project initialization, schema inspection, validation, artifact generation, fixtures, change classification, and sealed packaging.
status: current
owner: registry-docs
source_repos:
  - registry-stack
...
fixture cases, classifies changes between two project revisions, and builds a sealed deployment
package. It serves no request, starts no listener, and performs no deployment action.
...
This page is written by hand from the command definitions in `crates/registry-relayctl/src/lib.rs`
and the boundary tests in `crates/registry-relayctl/tests/cli_contract.rs`, at release v0.19.0. The
command line is not generated, so the source files win on any disagreement.

## Commands

Seven commands, with no subcommand nesting.
...
- All seven commands print help successfully with an empty standard error, so the adopter workflow
  is exposed by one binary.
...
[environment variable reference](../environment-variables/) for the variables the runtime does read.
