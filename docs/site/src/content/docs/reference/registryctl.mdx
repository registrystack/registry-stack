...
The source distribution also contains Registry Stack extensions for VS Code and Zed. Both launch
the same semantic server, either as `registry-language-server` or through
`registryctl authoring language-server`. The extensions add cross-file definitions, references,
workspace and document symbols, and missing, duplicate, or ambiguous reference diagnostics. They
run alongside the YAML language server, which continues to own syntax, schemas, completion, hover,
and formatting.
