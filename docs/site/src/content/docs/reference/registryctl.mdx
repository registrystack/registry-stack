...
This policy covers `init`, non-watch `test`, `check`, `preflight`, `capabilities`,
...
canonical protected spreadsheet project. `registryctl init --from snapshot`
creates the canonical offline Relay-and-Notary claim project. Both use
`registry-stack.yaml`, explicit entity and environment files, and generated
product inputs under `.registry-stack/`.

The human-readable result identifies the authored project and next validation
command. The JSON result contains no progress text, so another program can
parse standard output directly. Automation must read the project root from
`output` and supported authored entry points from `artifacts` in the
`registryctl.init.v1` report. Generated layout remains a private implementation
detail.
...
Registryctl 0.14.0 retires the pre-1.0 `init relay` scaffold and
`add notary` mutation. Either command returns a precise reinitialization path
and does not modify the target directory. Registryctl does not silently
translate a direct project or preserve two public project models.
