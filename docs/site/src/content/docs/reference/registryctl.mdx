...
This page describes `registryctl 0.15.2`. Use the binary, installer, image
...
One project defines one registry trust domain. Its deployment topology can be
Relay only, Notary without configured claims, or Relay and Notary together;
independent registries use separate projects. Every configured Notary claim
requires the combined topology and a compiler-pinned Relay consultation.
Credential issuance additionally requires exact Relay-backed evaluation
provenance.
...
generator. Every configured Notary claim consumes at least one compiler-pinned
Relay consultation. A Notary-only project may contain no claims.

### Retired direct projects

Registryctl 0.15.2 keeps the pre-1.0 `init relay` scaffold retired. The
...
This reference describes Registryctl 0.15.2. The
