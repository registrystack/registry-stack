...
In registryctl `v0.12.0`, this generation command requires the strict
...
Registryctl `v0.12.0` includes `registryctl add notary`. From a generated `benefits` spreadsheet
...
The released command tree comes from the registryctl source at `v0.12.0`.
For its canonical definitions, read the
[registryctl command tree in `src/main.rs`](https://github.com/registrystack/registry-stack/blob/v0.12.0/crates/registryctl/src/main.rs).
