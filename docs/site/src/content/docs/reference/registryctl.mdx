...
This page lists commands in Main source (unreleased).
The source has not been designated as a release candidate.
The last released `registryctl` is `v0.13.0`.
That artifact does not contain `preflight`, `capabilities`, `compare`, `promote`, `migrate`,
`authoring reference`, or `project diagnostics`; use a current source build for those commands.
The [current-source test procedure](../../start/test-current-source-revision/) pins the checkout,
builds the CLI, and distinguishes the temporary source-test artifacts from release evidence.
...
They do not establish a release candidate and do not describe the `v0.13.0` artifact.
Use [Test one current source revision](../../start/test-current-source-revision/) to build and
identify the exact source used for a review.
For the last released command tree, read the
[registryctl `v0.13.0` command tree](https://github.com/registrystack/registry-stack/blob/v0.13.0/crates/registryctl/src/main.rs).
