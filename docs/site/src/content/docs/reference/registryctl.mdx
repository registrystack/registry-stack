...
runtime files. This legacy sidecar contract is separate from Registry Stack project authoring and
its product-neutral `http`, `script`, and `snapshot` capabilities. The flags below apply to both
`openfn import` and `openfn convert` unless noted.
