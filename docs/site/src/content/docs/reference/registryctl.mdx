...
VS Code and Zed semantic navigation are optional, source-installable developer previews for this
beta. They are not marketplace extensions and are not available as release assets.
Build and install them from the Registry Stack source tree only when you want cross-file
definitions, references, workspace and document symbols, and missing, duplicate, or ambiguous
reference diagnostics.
Both previews launch the same semantic server, either as `registry-language-server` or through
