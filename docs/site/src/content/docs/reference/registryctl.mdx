...
This page lists commands released in registryctl `v0.9.0` and the pre-1.0 country-authoring
...
## Country configuration authoring

Country-authoring commands work on a project containing `registry-stack.yaml`, integration
definitions and fixtures, and explicit environment bindings.
The [country integration authoring guide](../../tutorials/author-country-integration/) covers the
complete workflow and generated trust boundary.

| Subcommand or flag | Purpose |
| --- | --- |
| `init --from <starter>` | Copy `bounded-http`, `dhis2-tracker`, `opencrvs`, or `openspp` into a local workspace. |
| `init --country-dir <dir>` | Destination for `init --from`. Defaults to the current directory. |
| `test --project <dir>` | Compile and run every integration fixture through the product Relay decoder and Notary evaluator offline. Defaults to the current directory. |
...
| `check --project <dir> --environment <name>` | Validate fixtures and generated Relay and Notary configuration without writing build output. |
| `check --explain` | Include the redacted acquisition, authorization, fact, claim, and disclosure plan. |
...
| `build --project <dir> --environment <name>` | Emit deterministic unsigned Relay and Notary Config Bundle input directories. |
...
Their `semantic_changes` entries identify changed claim, integration, country-policy, and
operator-security dimensions with previous and current digests; `required_reviews` remains the
separate external approval set.
Generated files are written under `.registry-stack/build/<environment>/` only by `build`.

### Country authoring schemas
...
| --- | --- | --- |
| `registry-stack.yaml` | `crates/registryctl/schemas/country-authoring/project.schema.json` | `https://registrystack.example/schemas/country-authoring/project.v1.json` |
| `integrations/*/integration.yaml` | `crates/registryctl/schemas/country-authoring/integration.schema.json` | `https://registrystack.example/schemas/country-authoring/integration.v1.json` |
| `integrations/*/fixtures/*.yaml` | `crates/registryctl/schemas/country-authoring/fixture.schema.json` | `https://registrystack.example/schemas/country-authoring/fixture.v1.json` |
| `environments/*.yaml` | `crates/registryctl/schemas/country-authoring/environment.schema.json` | `https://registrystack.example/schemas/country-authoring/environment.v1.json` |
| `records/*.yaml` | `crates/registryctl/schemas/country-authoring/records.schema.json` | `https://registrystack.example/schemas/country-authoring/records.v1.json` |
...
{
  "yaml.schemas": {
    "./crates/registryctl/schemas/country-authoring/project.schema.json": [
      "**/registry-stack.yaml"
    ]
  }
}
...
`snapshot_exact` is supported through authored integration, records definition, and private
...
See the guide's exact-snapshot section for the logical fact contract and private physical mapping.
...
source-backed provenance. Do not put registry or source credentials in the request or country
project.
...
Country `build` output is unsigned. Package and sign each product directory with `bundle sign`,
then verify the resulting bundle against its product trust anchor with `bundle verify`.
...
The pre-1.0 country-authoring table tracks the current source command enum and will receive a
