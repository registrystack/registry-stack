...
| `check --format json` | Emit the machine-readable check report instead of the default human-readable review report. |
...
`init`, non-watch `test`, and `build` print JSON reports. `check` prints a human-readable review
report by default and accepts `--format json` for automation. Watch mode prints one concise
