...
Registry Relay issues signed responses as verifiable credentials whose `credentialSubject` shape is described by published JSON Schemas, alongside a JSON-LD context for the credential terms. A running Relay gateway serves them unauthenticated under its `/schemas/{type}/{version}` and `/contexts/{vocab}/{version}` routes:
