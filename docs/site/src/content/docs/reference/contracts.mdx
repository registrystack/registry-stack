...
Registry Relay issues signed responses as verifiable credentials whose `credentialSubject` shape is described by published JSON Schemas, alongside a JSON-LD context for the credential terms. When provenance is enabled, a running Relay gateway serves them unauthenticated under its `/schemas/{type}/{version}` and `/contexts/{vocab}/{version}` routes:
...
These three are production contracts: they are embedded in issued credentials, served by the gateway, and content-pinned in the relay resource manifest. Their canonical identifiers resolve under `id.registrystack.org/schemas/registry-relay/...` and `id.registrystack.org/contexts/registry-relay/...`.
