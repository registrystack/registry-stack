...
Each contract has one owning project. When a contract changes, the owning repo is the source of truth. Shared crate APIs belong to Registry Platform; product routes and configuration behavior belong to the consuming service.
...
- The Registry contract grammar, `relay.registrystack.org/v2alpha1`, covering `registry.yaml`
  ...
  unrecognized key is a parse failure rather than an ignored field, so a document that compiles
  today cannot silently change meaning under a later runtime.
...
describes is generated per registry. Two deployments therefore publish different contexts for the
same route shape, which is the point: the semantics belong to the authority that runs the registry.
