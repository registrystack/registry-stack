...
These three are production contracts: they are embedded in issued credentials, served by the gateway, and content-pinned in the relay resource manifest. The emitted `$id` values resolve under `id.registrystack.org/schemas/registry-relay/...`; the provenance context terms resolve under `id.registrystack.org/ns/registry-relay/provenance/v1#`.
