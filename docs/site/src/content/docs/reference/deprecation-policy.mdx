...
- The release note for the announcing release, under `release/notes/`, gains a `Deprecated`
  ...
  same entry there.
- An HTTP route or field that a committed OpenAPI document for a covered surface describes is
  marked `deprecated: true` in that document. Evidence Gateway currently has the only such document.
  The committed OID4VCI supporting-service document remains exempt until that service enters the
  covered-surfaces table. Relay publishes no product-level document, so a Relay route deprecation is
  announced in the release note and marked on the reference page for the route inventory instead.
