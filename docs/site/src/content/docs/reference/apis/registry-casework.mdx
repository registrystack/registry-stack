...
events and never sees private reasons, drafts, or raw reviewer identities. Source-context task lists,
...
- Requester is admitted as a producer only through an exact `reviewProducers` identity binding. It
  creates and follows only requests inside that binding and cannot claim or decide work.
