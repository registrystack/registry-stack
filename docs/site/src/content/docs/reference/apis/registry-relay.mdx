...
- **Evidence-offering discovery.** Relay publishes evidence-offering metadata at
  ...
  The routes still exist, but runtime binding accepts only `access.kind: registry-notary`, an
  access kind with nothing maintained behind it, so a new deployment should declare no evidence
  offering. Relay never
  executed the verification behind an offering; an offering was always a pointer somewhere else.
