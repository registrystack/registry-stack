...
- **Feature-gated surfaces.** The live OGC API Features, Records, and EDR adapters, SP DCI sync
  routes, standards-CEL mapping, and attribute release are experimental and feature-frozen.
  They are outside the 1.0 compatibility promise and excluded from the canonical release binary.
  Their source and all-feature tests remain available. Relay does not issue response credentials
  or host issuer DID documents.
