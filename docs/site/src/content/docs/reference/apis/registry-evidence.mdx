...
- `application/dc+sd-jwt`: the same assertion serialized as an SD-JWT VC under the frozen profile
  in `products/evidence/contracts/sd-jwt-vc-profile.yaml`, with one disclosure per supported
  value. Enabling it adds a serialization, not a credential lifecycle.
