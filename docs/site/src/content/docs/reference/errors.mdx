...
description: RFC 9457 problem codes returned by Registry Relay, with their meaning and usual cause, and where the Evidence and Registry Mint error contracts are documented.
status: current
owner: registry-docs
source_repos:
  - registry-relay
...
Registry Notary is retired, and the codes it returned (its evidence-evaluation taxonomy and its
federation, credential-status, admin, readiness, and request-handling codes) are no longer part of
this registry.

## Evidence

Evidence has its own closed problem set, documented at
[Evidence problem types](../evidence-problems/). That page is the single place the set is written
...
Two structural differences are worth knowing before reading the Evidence page. Evidence problem
type URIs are `https://registrystack.org/problems/evidence/<code>` rather than the
`https://id.registrystack.org/problems/...` form used elsewhere in the stack. And an Evidence
...
Evidence answers with one of that closed set for every failure, including transport-level ones: a
...
| `runtime.binding.unsupported_evidence_offering` | runtime evidence offering kind is unsupported | The offering's `access.kind` is not the one value the startup validator accepts, `registry-notary`. Registry Notary is retired, so a new deployment should not declare an evidence offering at all. |
...
(Evidence) and `crates/registry-mint/src/error.rs` (Registry Mint).
