...
description: The closed RFC 9457 problem set Relay returns, with each code's status, title, and detail, and where the Evidence Gateway error contract is documented.
status: current
owner: registry-docs
source_repos:
  - registry-relay
...
full and points to the Evidence Gateway and OID4VCI supporting-service contracts.
