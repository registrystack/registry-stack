...
```text
evidencectl new --profile <PROFILE> <--openapi <OPENAPI>|--transport <TRANSPORT>|--starter <STARTER>> <DIRECTORY>
```
...
| --- | --- |
| Command invocation | Exactly one of `--openapi <OPENAPI>`, `--transport <TRANSPORT>`, `--starter <STARTER>` is required. |
...
| `--starter <STARTER>` | No | n/a | n/a | n/a | Reviewed local starter directory to copy without requiring an API description |
