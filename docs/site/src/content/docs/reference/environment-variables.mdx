...
Secret references use the same two grammars as Relay: `secret:env/<NAME>` names an operator-chosen environment variable and `secret:file/<name>` a file under the `secretProviders.file.root` directory. `secret:env/<NAME>` resolves only when `runtime.yaml` declares `secretProviders.environment`; without that declaration the reference is refused. See [Deploy and operate a registry](../../operate/breg/#write-the-runtime-configuration) for the fields that take a reference.
