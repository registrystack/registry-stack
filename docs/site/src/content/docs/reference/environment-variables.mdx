...
The script verifies the downloaded `breg`, `bregctl`, and `mint` binaries against the release
`SHA256SUMS` before anything reaches the install directory. It installs the three together or
preserves the previous set. The `mint` it installs is the same release asset the Evidence Gateway
toolset installer ships. It does not verify release authenticity.
