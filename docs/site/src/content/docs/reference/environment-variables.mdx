...
Registry Notary and Registry Relay expand `${VAR}` expressions before YAML
parsing.
...
| `REGISTRY_NOTARY_BIND` | Override `server.bind` for direct server startup. Equivalent to `--bind`. Closed product and development actions use the verified bundle listener instead. | Optional. |
