...
- The attachment download operation no longer promises a filename in
  `Content-Disposition`. The route has always sent the bare `attachment`
  disposition, and no stored slot metadata holds a filename, so the served
  OpenAPI document now pins the header to that exact value. A caller that read
  the old description and expected a name to save the bytes under has to choose
  one itself. See [Base Registry Engine API](../reference/breg-api/).
