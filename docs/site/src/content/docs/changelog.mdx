...
- BReg no longer fails an accepted review that the review authority still
  reports as pending. A `202` from the result resource no longer spends the
  result-poll attempt budget and clears an earlier `result-lookup-uncertain`,
...
- BREAKING: `bregctl init --from publicschema` and the four BReg starters
  follow PublicSchema `main` at commit `bd07bda`, version 0.3.0. The
