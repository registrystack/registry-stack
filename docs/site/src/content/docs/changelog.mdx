...
- `breg-mcp` is a new supporting service beside BReg: an MCP gateway a chat
...
{/* Evidence: crates/registry-breg-mcp/src/cli.rs, Command;
...
- `breg-review` is a new server-rendered review page beside the gateway. The
  ...
  listens on `127.0.0.1:8115` by default and stops cleanly on SIGINT or
  SIGTERM. Its hash-chained journal records the action, the
...
{/* Evidence: crates/registry-breg-review/src/lib.rs, command, check, and serve;
    crates/registry-breg-review/src/config.rs, default_listener_bind;
    crates/registry-breg-review/src/signin.rs;
    crates/registry-breg-review/src/journal.rs, record;
...
- `registry-platform-ratelimit` is a new shared crate with two in-memory
  ...
  refill, key cap, pruning, and refusals. The gateway and the review page
  use it for their limits.
...
- Platform crates gain what the gateway and the review page need.
