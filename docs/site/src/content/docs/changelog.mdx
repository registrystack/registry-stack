...
- BREAKING: BReg admits a token carrying a trusted `act` only on an access
  profile, or through an immediate-action permission, that declares
  `actorKind: agent`. A profile that declares no `actorKind` refuses such a
...
- BReg's audit journal records delegated authority on reads and immediate
  actions. A read's terminal record, for records, lists, lookups, revisions,
  history, and snapshots, now carries the `authorization` object for a
  task-grant or delegated token, and an immediate action's terminal record
  carries it for a delegated token. Direct tokens record no such object, as
  before.

{/* Evidence: crates/registry-breg/src/postgres/read.rs;
    crates/registry-breg/src/mutation/action.rs;
    crates/registry-breg/tests/postgres_task_grants.rs;
    products/breg/TASK_GRANTS.md. */}
