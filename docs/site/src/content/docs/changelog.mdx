...
- BReg decides the change-request actions it offers from the settled review
  ...
  approval as application state `expired`, which the Rust, Node.js, and
  Python clients accept, and request lists whose profile discloses
  `review_state` can filter on `reviewOutcome`. `bregctl explain lifecycle`
  reports the check as its `review_outcome` layer. See
...
- BReg no longer contacts the review authority for an apply made under a
  ...
  the proposal: it checks both grants first. The apply itself was already
  refused under such a grant, so this closes an early outbound call, not an
