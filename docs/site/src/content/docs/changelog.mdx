...
- BREAKING: Base Registry Engine (BReg) gives every `reference` field a btree
  ...
  index leads with. The findings never fail a check. An upgraded engine
  refuses an active package whose project has a reference field no index
...
- `breg` logs, and `bregctl doctor` reports under a new `advisories` list,
  ...
  and a note when `pg_stat_statements` is not installed. Advisories never
  refuse startup or change `doctor`'s exit status, and need no extra grant.
...
{/* Evidence: crates/registry-breg/src/postgres/baseline.rs, advise() and connection_budget();
    ...
    crates/registry-breg/tests/postgres_startup.rs, prepared_server_sessions_are_named_bounded_and_advised_with_pg_stat_statements. */}
