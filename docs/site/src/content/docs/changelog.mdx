...
- Registry Casework ships with Registry Stack 0.30.0 and has a documentation set
  ...
  `casework-install.sh` installs `casework`, `caseworkctl`, and `mint` together,
  and `caseworkctl dev` starts a retained local runtime with its PostgreSQL
  container, token issuer, and seeded directory.
