...
- A Base Registry Engine deployment with a statically pinned `jwksSource`
  now names provider-side key rotation in its logs instead of failing
  silently. Access tokens whose signing key is not in the configured JWKS
  keep being refused with the same value-free `authentication.refused`, but
  the server log records on the `registry_breg::auth` target that the
  refused token cites a signing key outside the configured JWKS and points
  ...
  the removed keys stay refused. See
...
- The Base Registry Engine access-token verifier admits both RFC 9068
  ...
  accept tokens whose `typ` header carries either spelling, while every other
  `typ` value is still refused. An issuer switching between the two permitted
  spellings — ThunderID emits `at+jwt`, other providers may emit the full media
  type — no longer needs an engine configuration change. See
  [operating Base Registry Engine](../operate/breg/).
...
- `bregctl dev` compares the `breg` and `mint` it resolved against its own
  version before it starts anything, and refuses a prerequisite from another
  release by name, giving both versions and the fix. Previously the mismatch
  surfaced much later as a refused package or an unready database.
