...
The registry-backed OID4VCI issuance path requires EdDSA `did:jwk` holder proof
of possession. Subject access authorizes the Relay-backed evaluation; it does
not supply evidence.
See [Evidence issuance, end to end](../explanation/evidence-issuance/) for the full credential
lifecycle.

Delegated evaluation, where one Notary asks a trusted peer to evaluate a claim, is admitted only
from statically configured peers. The peer's identity, signature, freshness, and purpose are
verified before any Relay consultation or claim evaluation
(REQ-SEC-G-010).
