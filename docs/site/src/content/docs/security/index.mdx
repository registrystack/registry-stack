...
Evidence exempts only its probes, its OpenAPI document, its issuer JWKS, and its issuer metadata
...
Evidence can serialize the same signed assertion as an SD-JWT VC, where the signed body carries
...
A request may carry an optional `holderKey`, echoed into the `cnf` claim; Evidence issues no
...
Where a deployment has no identity provider, Registry Mint issues the access tokens Evidence
...
Delegated evaluation, where one service asks a trusted peer to evaluate a claim, no longer exists
in this stack: Registry Notary held the only such surface, and REQ-SEC-G-010 is retired with it.
...
Evidence states its own operator obligations in its operator contract rather than through this
posture block.
