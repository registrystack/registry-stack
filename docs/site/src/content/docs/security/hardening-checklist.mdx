...
- Base Registry Engine: escrow the Transit key before the first encrypted field goes live. The
  ...
  irrecoverably: the runtime holds no unwrapped copy and offers no recovery path. A health check
  does not certify that escrow happened, and after enabling there is no second moment at which the
  key is cheap to protect.
- Base Registry Engine: keep the base64 local-file DEK out of production. It exists for local
  assurance only, and `bregctl check --production` refuses a candidate whose `fieldEncryption`
  binding names it.
...
{/* TODO[evidence]: the three Base Registry Engine bullets above are Phase 1 behavior of
    registry-stack issue #1108, whose implementation is not yet in this tree, so no anchor
    resolves today. When the field-encryption milestone lands, anchor the Transit wrapping and
    the refusal of the local-file DEK against the DEK provider, the production check, and
