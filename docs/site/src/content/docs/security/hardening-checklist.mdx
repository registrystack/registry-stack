...
- Keep Notary's Relay connection on HTTPS unless the two services share a network namespace.
  `allow_insecure_localhost` permits only an HTTP IP-loopback origin for that colocated topology;
  remote HTTP origins remain invalid.
