...
- Relay: on Unix, a `secret:file/<name>` target must be a regular file owned by the running user
  with mode exactly `0600` and a link count of one; a hard-linked file is refused under every name.
...
{/* Evidence: RelayRuntime is a closed deny_unknown_fields schema over apiVersion, kind, server,
     ...
     closed Visibility enum, crates/registry-relay-v2/src/contract.rs:1042-1060. */}
