...
- Rotate a compromised source credential at its own source and restart the affected process.
  ...
  secret, remove its fingerprint or the complete registration. Send `SIGHUP` to reload the client
  registry immediately. Already issued bearer tokens remain valid until expiry.
