...
async function createFixture(t, { collision = null } = {}) {
  ...
  await write(rootOutput, 'start/evidence-quickstart/index.html', html('/start/evidence-quickstart/'));
...
test('promotes unchanged released files to root and the exact version route', async (t) => {
  ...
  assert.match(
    await readFile(
      resolve(fixture.docsRoot, 'dist/preview/start/evidence-quickstart/index.html'),
      'utf8',
    ),
    /url=\/start\/evidence-quickstart\//,
  );
});
