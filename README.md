...
- **Registry Scheduling:** a coordinated booking surface over anchored supply:
  published openings, exact-time offerings, arrival windows with channel
...
- **Registry Messaging:** sends one email or SMS to one recipient for an
  ...
  operator connected, and reports what is known about delivery. Why and when to
  send, who the recipient is, and consent to be contacted stay with the
  caller's source of record.
  Docs: [Registry Messaging overview](https://docs.registrystack.org/dev/start/messaging/).
...
`schedulingctl`, `messagingctl`, `relayctl`, `evidencectl`, and `discoveryctl`
adopter tools, unified Node.js and Python clients, and release tooling for validating the
public source model.
...
Messaging joins the release train at `v0.35.0` with
`messaging-<tag>-linux-amd64` and `messagingctl-<tag>-linux-amd64` binaries
and no installer. Container images for `breg`, `casework`, `scheduling`,
`messaging`, `relay`, `evidence`, and `discovery` are published as `ghcr.io/registrystack/<name>:<tag>`. Which
...
- `crates/`: Rust crates and runnable binaries for Base Registry Engine,
  Registry Relay, Evidence Gateway, Registry Casework, Registry Scheduling,
  Registry Messaging, Registry Discovery, Registry Manifest, Registry Platform,
  and the `bregctl`, `caseworkctl`, `schedulingctl`, `messagingctl`, `relayctl`,
  `evidencectl`, and `discoveryctl` adopter tools. Base Registry Engine lives in `crates/registry-breg` with one
  ...
  `casework` binary, Registry Scheduling in `crates/registry-scheduling`
  with one `scheduling` binary, and Registry Messaging in
  `crates/registry-messaging` with one `messaging` binary.
