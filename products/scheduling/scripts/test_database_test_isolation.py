...
def metadata(features=(), required=("postgres-test",), target="postgres_commitments"):
    return {
        "packages": [{"id": "runtime", "name": "registry-scheduling", "targets": [
            {"name": target, "kind": ["test"], "required-features": list(required)},
        ]}],
...
class DatabaseTestIsolationTests(unittest.TestCase):
    ...
    def test_an_ordinary_integration_test_does_not_need_a_gate(self):
        self.assertEqual(MODULE.violations(metadata(required=[], target="http_boundary")), [])
    ...
    def test_cargo_resolves_defaults_and_transitive_forwarding(self):
        with tempfile.TemporaryDirectory(prefix="scheduling-feature-test.") as directory:
            root = Path(directory)
            (root / "src").mkdir()
            (root / "tests").mkdir()
            (root / "src/lib.rs").write_text("", encoding="utf-8")
            (root / "tests/postgres_commitments.rs").write_text("", encoding="utf-8")
