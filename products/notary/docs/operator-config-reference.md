...
A configuration may start with no claims. If claims are configured, the
project must also configure exactly one logical Relay connection. Every claim,
including a claim authorized through subject access or selected by federation,
must resolve to a compiler-pinned Relay consultation. Identity and request
inputs authorize or parameterize that consultation; they are not evidence.
...
Registry Stack project authoring requires and emits the field for string
claims.
...
Citizen and wallet flows use the subject access subject-binding policy.
...
mutation. OID4VCI rejects a credential configuration without complete
Relay-backed provenance, creates and evaluates the registry transaction before
rendering an offer, consumes the
transaction-bound proof nonce at the credential endpoint, and checks stored
provenance before signer access. Delegated subject access is
