...
| --- | --- | --- | --- | --- | --- |
| Replay identifiers | `registry-platform-replay` through `ReplayStores`; Redis or in-memory | `registry_notary_private.replay_identifier` | Insert a scoped identifier exactly once. Concurrent inserts across instances produce one winner. An unexpired row always rejects. An expired row may be replaced atomically. Stable, domain-separated Notary hashes keep a spent identifier authoritative across application-secret rotation. | Absolute token or request expiry. Expired rows are pruned in bounded batches. Backup and restore must retain all unexpired rows or the service must remain offline until every token valid at the restore point has expired. | Store only domain-separated hashes of scope and protocol-generated high-entropy identifiers, never raw JWTs, nonces, tenants, issuers, holders, or subjects. |
...
| Preauthorization transaction-code session | `SingleUseStore<TxCodeSession>`, process-local | `registry_notary_private.preauthorization_tx_code` | Reserve by the stable, domain-separated pre-authorized-code JTI hash used for replay. Replay scope is fixed by the verified Notary issuer from the signed token contract, not mutable service or credential-issuer configuration. The signed pre-authorized code binds whether its exact offer requires a transaction code, so restart or reconfiguration cannot weaken or add the PIN requirement for a live offer. Wrong PIN may read without consuming. Correct PIN redemption atomically inserts the stable pre-authorized-code replay marker and deletes the verifier, producing exactly one winner across instances and preserving one-time use after sensitive-key rotation or unrelated configuration changes. Reservation shares the sensitive-key generation lock and rejects a different live key id in either preauthorization table. | Absolute pre-authorized-code expiry, at most 600 seconds. Delete the PIN verifier on successful code use and on expiry, but retain the replay marker until code expiry. Restore and every readiness probe preserve the same live-key attestation as login state. | Store only stable hashes of the verified Notary issuer scope and protocol-generated high-entropy JTI, plus a keyed PIN verifier made with a separate domain-separated MAC subkey, never reversible PIN ciphertext. Raw code, JTI, and PIN are not stored or logged. |

The six subject access bucket kinds remain distinct and closed:
...
- Domain-separated subkeys are derived for login-state AEAD, transaction-code
  PIN verification, and keyed login-state identifiers. Preauthorization replay
  scope and JTI hashes use the stable Notary replay domains, with scope fixed
  by the verified token issuer, so a redeemed code remains spent after
  sensitive-state key rotation or unrelated service configuration changes.
  The key id is stored with ciphertext and PIN-verifier rows, but key material
  is never stored. Rotation stops new preauthorization issuance, drains or
  prunes all rows for at most the 600-second maximum lifetime, then replaces
  the key before issuance resumes.
...
- Subject access quota locks all applicable bucket rows in deterministic
...
The final v1 fingerprint also fixes pre-authorized-code replay identity to the
verified Notary issuer and stable replay hashes. A prerelease database created
from an earlier development commit cannot transform its keyed markers because
raw JTIs are intentionally absent. Stop issuance, drain outstanding codes, and
install a fresh schema. Do not accept the earlier fingerprint or reuse that
database with the final v1 binary.
...
3. wait for the maximum configured lifetime of outstanding replay, nonce,
   ...
   one-hour subject access window;
...
- every subject access bucket, multi-bucket all-or-nothing debit,
