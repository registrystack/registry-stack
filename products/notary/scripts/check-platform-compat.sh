...
if [[ ! -f "${crosswalk_input}/Cargo.toml" ]]; then
  echo "registry-notary platform compatibility check failed: CROSSWALK_SOURCE_DIR (or deprecated CEL_MAPPING_SOURCE_DIR) does not point to a crosswalk checkout: ${crosswalk_input}" >&2
  exit 2
fi
...
if [[ "${platform_dir}" != "${cargo_platform_dir}" || "${crosswalk_dir}" != "${cargo_crosswalk_dir}" ]]; then
  if ! command -v rsync >/dev/null 2>&1; then
    echo "registry-notary platform compatibility check failed: rsync is required when REGISTRY_PLATFORM_SOURCE_DIR, CROSSWALK_SOURCE_DIR, or deprecated CEL_MAPPING_SOURCE_DIR is not using the default sibling checkout" >&2
    exit 2
  fi
  ...
  echo "==> registry-notary: using temporary compatibility workspace with registry-platform -> ${platform_dir}, crosswalk -> ${crosswalk_dir}"
fi
