...
- The suite runs against a disposable Notary instance and disposable Relay
  consultation fixture, not a shared operator environment.
...
| `evaluation-positive` | authenticated `POST /v1/evaluations` succeeds for a compiler-pinned Relay fixture | runtime evaluation | Response contains the expected claim result and no raw Relay output spillover. |
...
- a fixture Notary config and its compiler-pinned Relay fixture can be started
...
- one evaluation succeeds against the configured Relay consultation;
