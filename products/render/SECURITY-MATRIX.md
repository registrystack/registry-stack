...
| 8 | The ledger lies: records removed, rewritten, or reordered | Keyed HMAC hash chain, sealed non-deleting segments; `registry-render audit-verify` proves the chain end to end | `scaffold::tampered_ledger_fails_audit_verify` (a flipped byte fails verification; the happy path counts real records, not zero) |
