...
The representative `tests/journeys.yaml` includes a declared public bbox query and a nonspatial-profile bbox refusal. Use the [spatial quickstart](../../quickstart/README.md#spatial-service-site-quickstart) to bootstrap PostGIS and run the protected GeoJSON journey with a stock-issuer client. Its optional refresh exercise imports `fixtures/qgis-refresh-service-site.jsonl` through the ordinary batch API.
