...
CLI surface or configuration schema change. The docs generators run the CLIs and
read the committed schemas, so regenerate and commit what they write:
...
Reproduce each with its generator and commit the result beside the source
change. Do not hand-edit any of them.
