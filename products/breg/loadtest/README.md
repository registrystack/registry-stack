...
A self-contained local stack for measuring Base Registry Engine with pinned
...
|---|---|---|
| `cursor-smoke` | one filtered page plus its continuation | Did the harness really execute page two with the Base Registry Engine cursor contract? |
...
| `herd` | 200 VUs, one token and one protected read each | Does a coordinated client restart overload Mint or Base Registry Engine? |
...
```bash
products/breg/loadtest/run.sh --profile steady --ops 75 --duration 15m
...
- `telemetry.jsonl`: one-second Base Registry Engine metrics plus local server and
...
```bash
products/breg/loadtest/dbstats.sh reset
