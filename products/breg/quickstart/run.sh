...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    printf '%s\n' "$1 is required for the Base Registry Engine generic quickstart." >&2
    exit 2
  fi
}
...
mint_pid=""
breg_pid=""
postgres_container="breg-quickstart-${PPID}-$$"
cleanup() {
  if [[ -n "${breg_pid:-}" ]]; then
...
openssl req -x509 -new -nodes -newkey rsa:2048 -sha256 -days 2 \
  -subj '/CN=Base Registry Engine generic quickstart CA' \
...
printf '%s\n' '== Starting Base Registry Engine on loopback'
...
if [[ "$spatial" == true ]]; then
  ...
  if [[ "$mode" == smoke ]]; then
    printf '%s\n' 'Base Registry Engine spatial quickstart smoke passed.'
    exit 0
  fi
else
  ...
  if [[ "$mode" == smoke ]]; then
    printf '%s\n' 'Base Registry Engine generic quickstart smoke passed.'
    exit 0
  fi
fi
