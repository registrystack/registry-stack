#!/usr/bin/env python3
"""Reject authored fixture identifiers in shipped Base Registry Engine source."""
...
def main() -> int:
    ...
    if violations:
        print("Base Registry Engine source-neutrality check failed:", file=sys.stderr)
        print("\n".join(f"- {item}" for item in violations), file=sys.stderr)
        return 1
    print("Base Registry Engine source-neutrality check passed")
    return 0
