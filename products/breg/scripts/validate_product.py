#!/usr/bin/env python3
"""Validate Base Registry Engine's tracked product-contract catalog.
...
try:
    ...
    raise SystemExit("PyYAML is required to validate Base Registry Engine contracts") from exc
...
SECURITY_INVARIANT_IDS = tuple(f"BREG-SEC-{index:02d}" for index in range(1, 106))
ACCEPTANCE_FIXTURES = {
    ...
    "BREG-J03": ("business-establishments", "acceptance/business-establishments"),
    "BREG-J04": ("inspection", "acceptance/inspection"),
    "BREG-J05": ("facility", "acceptance/facility"),
    "BREG-J06": ("business", "acceptance/business"),
    "BREG-J18": ("asset-registration-actions", "fixtures/asset-registration-actions"),
    "BREG-J19": ("household-contact-actions", "fixtures/household-contact-actions"),
...
POSTGRES_TEST_COMMANDS = (
    ...
    "cargo test --locked -p registry-breg --features runtime,tooling,schema --test runtime_config",
    "cargo test --locked -p registry-breg --features runtime,tooling,schema --test startup_http",
    "cargo test --locked -p registry-breg --features runtime,tooling,schema --test startup_ordering",
    "cargo test --locked -p registry-breg --features runtime,tooling,schema --test fixture_tooling",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_kernel",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_compiled_schema",
    ...
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_history_erasure",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_history_rebaseline",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_workspace_metadata",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_mutation",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_mutation_logical_names",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_immediate_actions",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_immediate_action_examples",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_immediate_action_activation",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_webhook_outbox",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_webhook_delivery",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_hook_proposals",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_immediate_action_requirements",
    ...
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_action_evidence_targets",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_action_evidence_retention",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_temporal_corrections",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_batch",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_data_facility",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_data_export",
    ...
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_request_upgrade_retention",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_request_events",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_request_queries",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_request_read_retention",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_review_executor",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_request_activation",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_audit_tooling",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_pilot_acceptance",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_rhai_planner",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_tombstone_revision",
    "cargo test --locked -p registry-breg --features postgres-test,tooling,schema --test postgres_field_encryption",
...
POSTGRES_TLS_TEST_COMMAND = "cargo test --locked -p registry-breg --features postgres-tls-test --test postgres_tls"
...
def validate_definition_of_done(waves: set[str], errors: list[str]) -> set[str]:
    ...
    v1_identifiers = [
        ...
        and re.fullmatch(r"BREG-V1-[0-9]{2}", item["id"])
    ]
    if v1_identifiers != list(V1_REQUIREMENT_IDS):
        errors.append("definition of done: must contain BREG-V1-01 through BREG-V1-44 exactly once in order")
...
def validate_postgres_entrypoint(errors: list[str]) -> None:
    ...
    required_fragments = (
        ...
        "BREG_TEST_DATABASE_URL must be set for PostgreSQL journeys.",
        "exit 2",
...
def validate_postgres_tls_entrypoint(errors: list[str]) -> None:
    ...
    required_fragments = (
        "mktemp -d /tmp/breg-postgres-tls.XXXXXX",
        "trap cleanup EXIT",
        "/tmp/breg-postgres-tls.*)",
        'rm -rf -- "$tls_dir"',
        "docker cp",
        ...
        "BREG_TEST_TLS_CA_DER_PATH",
        "BREG_TEST_TLS_CA_PEM_PATH",
        "BREG_TEST_TLS_WRONG_CA_DER_PATH",
    ...
    required_workflow_fragments = (
        ...
        "run: products/breg/scripts/test-postgres-tls.sh",
    )
...
def validate_all() -> list[str]:
    ...
    if "BREG-W0-CONTRACTS" not in requirements:
...
def main() -> int:
    errors = validate_all()
    if errors:
        print("Base Registry Engine product contract validation failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("Base Registry Engine product contracts are internally complete")
    return 0
