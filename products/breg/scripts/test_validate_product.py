...
SPEC = importlib.util.spec_from_file_location("breg_validate_product", SCRIPT_PATH)
...
class BRegProductCatalogTests(unittest.TestCase):
    ...
    def test_security_range_is_closed_through_field_encryption_invariants(self) -> None:
        ...
        self.assertEqual(
            [f"BREG-NEG-{index:02d}" for index in range(25, 104)],
    ...
    def test_w0_crate_boundary_includes_clients_and_opt_in_runtime(self) -> None:
        ...
        self.assertEqual(
            ...
            [member for member in members if member.startswith("crates/registry-breg")],
        )
        ...
        self.assertEqual(
            ...
            set(breg["features"]["runtime"]),
        )
        self.assertEqual(["runtime"], breg["bin"][0]["required-features"])
        for dependency in (
            "axum",
            "base64",
            "chacha20poly1305",
    ...
    def test_postgres_tls_entrypoint_refuses_to_silently_skip_without_container_id(
        ...
        self.assertTrue(os.access(script, os.X_OK))
        environment = os.environ.copy()
        environment.pop("BREG_TEST_TLS_POSTGRES_CONTAINER_ID", None)
        result = subprocess.run(
            [str(script)],
            cwd=VALIDATOR.REPOSITORY_ROOT,
            env=environment,
    ...
    def test_duplicate_security_identifier_is_rejected(self) -> None:
        ...
        def load_with_duplicate(path: Path):
            value = copy.deepcopy(original(path))
            if path.name == "security-invariant-matrix.yaml":
                value["invariants"][1]["id"] = "BREG-SEC-01"
            return value
    ...
    def test_helper_named_like_a_test_is_not_an_executable_test(self) -> None:
        ...
        VALIDATOR.executable_test_resolves(
            {
                "path": "products/breg/scripts/test_validate_product.py",
    ...
    def test_definition_of_done_cannot_omit_a_v1_requirement(self) -> None:
        ...
        def load_without_requirement(path: Path):
            ...
            if path.name == "definition-of-done.yaml":
                value["requirements"] = [
                    row for row in value["requirements"] if row["id"] != "BREG-V1-44"
                ]
            return value
    ...
    def test_shell_evidence_name_must_match_the_executable_filename(self) -> None:
        ...
        VALIDATOR.executable_test_resolves(
            {
                "path": "products/breg/scripts/check-source-neutrality.sh",
