...
require_env BREG_TEST_TLS_POSTGRES_CONTAINER_ID
require_env BREG_TEST_TLS_DATABASE_URL
require_env BREG_TEST_TLS_HOSTNAME_MISMATCH_DATABASE_URL
require_env BREG_TEST_TLS_DATABASE_HOST

postgres_container_id=$BREG_TEST_TLS_POSTGRES_CONTAINER_ID
database_url=$BREG_TEST_TLS_DATABASE_URL
hostname_mismatch_database_url=$BREG_TEST_TLS_HOSTNAME_MISMATCH_DATABASE_URL
database_host=$BREG_TEST_TLS_DATABASE_HOST
caller_ca_der_path=${BREG_TEST_TLS_CA_DER_PATH:-}
caller_ca_pem_path=${BREG_TEST_TLS_CA_PEM_PATH:-}
...
if [[ ! "$postgres_container_id" =~ ^[0-9a-f]{12,64}$ ]]; then
  printf '%s\n' 'BREG_TEST_TLS_POSTGRES_CONTAINER_ID must be a Docker container ID.' >&2
  exit 2
fi
if [[ ! "$database_host" =~ ^[A-Za-z0-9.-]+$ ]]; then
  printf '%s\n' 'BREG_TEST_TLS_DATABASE_HOST must be a DNS hostname.' >&2
  exit 2
fi
case "$database_url" in
  *"@$database_host:"* | *"@$database_host/"*) ;;
  *)
    printf '%s\n' 'BREG_TEST_TLS_DATABASE_URL must use BREG_TEST_TLS_DATABASE_HOST.' >&2
    exit 2
    ;;
esac
case "$hostname_mismatch_database_url" in
  *"@127.0.0.1:"* | *"@127.0.0.1/"*) ;;
  *)
    printf '%s\n' 'BREG_TEST_TLS_HOSTNAME_MISMATCH_DATABASE_URL must use 127.0.0.1.' >&2
    exit 2
    ;;
esac
validate_caller_output_path BREG_TEST_TLS_CA_DER_PATH "$caller_ca_der_path"
validate_caller_output_path BREG_TEST_TLS_CA_PEM_PATH "$caller_ca_pem_path"

tls_dir=$(mktemp -d /tmp/breg-postgres-tls.XXXXXX)
cleanup() {
  case "${tls_dir:-}" in
    /tmp/breg-postgres-tls.*)
      if [[ -d "$tls_dir" && "$(basename -- "$tls_dir")" == breg-postgres-tls.* ]]; then
        rm -rf -- "$tls_dir"
      fi
      ;;
  esac
...
openssl req -x509 -new -nodes -newkey rsa:2048 -sha256 -days 2 \
  -subj '/CN=Base Registry Engine PostgreSQL TLS test CA' \
...
openssl req -x509 -new -nodes -newkey rsa:2048 -sha256 -days 2 \
  -subj '/CN=Base Registry Engine PostgreSQL TLS wrong CA' \
...
if [[ -n "$caller_ca_der_path" ]]; then
  caller_ca_der_tmp=$(mktemp "$(dirname -- "$caller_ca_der_path")/.breg-postgres-ca-der.XXXXXX")
...
if [[ -n "$caller_ca_pem_path" ]]; then
  caller_ca_pem_tmp=$(mktemp "$(dirname -- "$caller_ca_pem_path")/.breg-postgres-ca-pem.XXXXXX")
...
export BREG_TEST_TLS_HOSTNAME_MISMATCH_DATABASE_URL="$hostname_mismatch_database_url"
export CARGO_INCREMENTAL=0
export CARGO_PROFILE_DEV_DEBUG=0
export CARGO_PROFILE_TEST_DEBUG=0
export RUSTC_WRAPPER=

cargo test --locked -p registry-breg --features postgres-tls-test --test postgres_tls
