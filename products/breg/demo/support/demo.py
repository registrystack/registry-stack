#!/usr/bin/env python3
"""Provision and exercise disposable Base Registry Engine demos."""
...
AUDIENCE = "urn:breg:household-demo"
...
BUSINESS_AUDIENCE = "urn:breg:business-demo"
...
ASSET_SITE_AUDIENCE = "urn:breg:asset-site-demo"
...
ASSET_CHANGE_AUDIENCE = "urn:breg:asset-change-request-demo"
...
FACILITY_AUDIENCE = "urn:breg:facility-demo"
...
INSPECTION_AUDIENCE = "urn:breg:inspection-demo"
...
def _local_project(
    ...
    if webhook:
        ...
        digest = digest_line.removeprefix("    digest: ").strip()
        if len(digest) >= 2 and digest[0] == digest[-1] and digest[0] in {'"', "'"}:
            digest = digest[1:-1]
        if (
            not separator
...
def _expected_webhook_signature(key: bytes, headers: dict[str, str], body: bytes) -> str:
    ...
    # BReg signature contract. It has no access to runtime destination config.
...
def _verify_webhook_request(
    ...
    expected_schema_prefix = (
        f"urn:breg:event-schema:{config['registry_id']}:{hook['entity']}:"
...
def write_handoff(root: Path, fixture_kind: str, out: Path) -> None:
    ...
    breg_origin = (root / "breg-origin").read_text(encoding="ascii").strip()
    personas = []
    ...
    handoff = {
        ...
        "registry": {"id": config["registry_id"], "baseUrl": breg_origin},
        "personas": personas,
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"Base Registry Engine demo failed: {error}", file=sys.stderr)
        return 1
    return 0
