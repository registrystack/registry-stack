...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    printf '%s\n' "$1 is required for the Base Registry Engine demo." >&2
    exit 2
  fi
}
...
breg_pid=""
receiver_pid=""
postgres_container="breg-demo-${PPID}-$$"
cleanup() {
  if [[ -n "${breg_pid:-}" ]]; then
...
if [[ "$webhook" == true ]]; then
  ...
  read -r database_port mint_port breg_port receiver_port <<EOF
$ports
EOF
else
  ports=$(python3 "$support" ports)
  read -r database_port mint_port breg_port <<EOF
$ports
EOF
  receiver_port=""
fi
...
printf '%s\n' '== Starting Base Registry Engine and creating deterministic demo records'
BREG_LOG=error "$breg" --config "$run_dir/runtime.yaml" \
...
if [[ "$fixture_kind" == business-establishments ]]; then
  ...
  printf '\n%s\n' 'Base Registry Engine household demo is ready.'
elif [[ "$fixture_kind" == asset-site ]]; then
  printf '\n%s\n' 'Base Registry Engine asset-site demo is ready.'
elif [[ "$fixture_kind" == asset-change-request ]]; then
  printf '\n%s\n' 'Base Registry Engine asset change-request demo is ready.'
elif [[ "$fixture_kind" == facility ]]; then
  printf '\n%s\n' 'Base Registry Engine facility demo is ready.'
else
  printf '\n%s\n' 'Base Registry Engine inspection demo is ready.'
fi
...
if [[ "$mode" == smoke ]]; then
  printf '%s\n' "Base Registry Engine ${fixture_kind} demo smoke passed."
  exit 0
fi

printf '\n%s\n' 'Leave this terminal running. Press Ctrl-C to stop the services.'
# supervision-signal-handling: wait begin
while kill -0 "$mint_pid" >/dev/null 2>&1 && kill -0 "$breg_pid" >/dev/null 2>&1; do
