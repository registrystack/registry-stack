...
REVIEW_OPERATION_DTO_INVENTORY = {
    ("post", "/v1/review-requests"): ("201", "ReviewRequestAccepted"),
    ("get", "/v1/review-requests/{request_id}"): ("200", "ReviewRequestView"),
    ("get", "/v1/review-requests/{request_id}/result"): ("200", "ReviewResult"),
...
class GeneratedOpenApiTests(unittest.TestCase):
    ...
    def test_review_operation_and_dto_parity_inventory(self) -> None:
        schemas = self.openapi["components"]["schemas"]
        GENERATOR.verify_review_wire_examples(
            self.openapi, self.contract["reviewWireExamples"]
        )
        for (method, path), (status, dto) in REVIEW_OPERATION_DTO_INVENTORY.items():
            with self.subTest(method=method, path=path):
                operation = self.openapi["paths"][path][method]
                self.assertEqual(
                    {"$ref": f"#/components/schemas/{dto}"},
                    operation["responses"][status]["content"]["application/json"][
                        "schema"
                    ],
                )

        create = schemas["ReviewCreateRequest"]
        self.assertNotIn("resultConstraints", create["required"])
        constraints = create["properties"]["resultConstraints"]
        self.assertEqual(
            ["object", "null"],
            [variant["type"] for variant in constraints["anyOf"]],
        )
        portable = constraints["anyOf"][0]
        self.assertEqual(
            -GENERATOR.MAXIMUM_PORTABLE_JSON_INTEGER,
            portable["x-minimum-json-integer"],
        )
        self.assertEqual(
            GENERATOR.MAXIMUM_PORTABLE_JSON_INTEGER,
            portable["x-maximum-json-integer"],
        )

        task_context = schemas["ReviewTaskContext"]
        self.assertNotIn("resultConstraints", task_context["required"])
        self.assertEqual(portable, task_context["properties"]["resultConstraints"])
        review_result = schemas["ReviewResult"]
        self.assertNotIn("result", review_result["required"])
        self.assertEqual(portable, review_result["properties"]["result"])

        state = schemas["ReviewerTaskState"]["oneOf"]
        self.assertEqual("open", state[0]["const"])
        self.assertEqual("decided", state[2]["const"])
        held = state[1]
        self.assertEqual(["held"], held["required"])
        self.assertEqual(
            ["holder"], held["properties"]["held"]["required"]
        )
        self.assertEqual(
            {"$ref": "#/components/schemas/ReviewerTaskState"},
        ...
        self.assertEqual(
            ...
            (
                ...
                task_properties["stageIndex"]["maximum"],
            ),
        )
        self.assertEqual(
            (
            ...
            (
                task_properties["revision"]["minimum"],
                task_properties["revision"]["maximum"],
            ),
        )
        ...
        for path in (
            ...
            with self.subTest(cursor_path=path):
                self.assertEqual(
                    ...
                    parameter_schema(self.openapi, "get", path, "cursor"),
                )
                limit = parameter_schema(self.openapi, "get", path, "limit")
                self.assertEqual((1, 100), (limit["minimum"], limit["maximum"]))

        feed_cursor = schemas["ReviewResultFeedPage"]["properties"]["nextCursor"]
        self.assertEqual(
            {"type": "string", "format": "uuid"},
            feed_cursor,
        )
        ...
        for status in ("202", "404", "410"):
            self.assertNotIn("content", result_operation["responses"][status])
        for path in (
            "/v1/review-requests",
            ...
            operation = next(iter(self.openapi["paths"][path].values()))
            self.assertNotIn(
    ...
    def test_rust_review_wire_examples_detect_structural_openapi_drift(self) -> None:
        broken = copy.deepcopy(self.openapi)
        broken["components"]["schemas"]["ReviewerTaskState"]["oneOf"] = [
            {"const": "open"},
            {"const": "decided"},
        ]
        with self.assertRaisesRegex(ValueError, "ReviewerTaskState refuses"):
            GENERATOR.verify_review_wire_examples(
                broken, self.contract["reviewWireExamples"]
            )
    ...
    def test_checkpoint_specific_schema_bounds_are_explicit(self) -> None:
        ...
        task_context = schemas["ReviewTaskContextData"]
        self.assertEqual("strategy", task_context["discriminator"]["propertyName"])
        self.assertEqual(
            {"strategy", "snapshot"}, set(task_context["oneOf"][0]["required"])
        )
        self.assertEqual(
            {"strategy", "reference", "bindingStatus"},
            set(task_context["oneOf"][1]["required"]),
        )
        self.assertEqual(
            "#/components/schemas/ReviewTaskContextData",
            schemas["ReviewTaskContext"]["properties"]["context"]["$ref"],
        )
        self.assertIn("requestId", schemas["ReviewClockOccurrence"]["required"])
        revoke_parameters = {
            parameter["name"]
            for parameter in self.openapi["paths"][
    ...
    def test_review_result_concealed_or_unknown_response_has_no_problem_body(self) -> None:
        response = self.openapi["paths"][
            "/v1/review-requests/{request_id}/result"
        ]["get"]["responses"]["404"]
        self.assertEqual("The request is concealed or unknown.", response["description"])
    ...
    def test_review_subject_binding_uses_the_serde_renamed_type_key(self) -> None:
        subject = self.openapi["components"]["schemas"]["ReviewSubjectBinding"]
        self.assertEqual(
            {"source", "type", "id", "version", "digest"},
    ...
    def test_review_request_view_requires_every_present_wire_field(self) -> None:
        view = self.openapi["components"]["schemas"]["ReviewRequestView"]
        self.assertEqual(
            {
                "requestId",
                "subject",
        ...
        self.assertNotIn("activeStage", view["required"])

    def test_review_outcome_result_required_is_optional_when_false(self) -> None:
        outcomes = self.openapi["components"]["schemas"]["ReviewKindPolicySnapshot"]
        outcome = outcomes["properties"]["outcomes"]["items"]
        self.assertIn("resultRequired", outcome["properties"])
        self.assertNotIn("resultRequired", outcome["required"])

    def test_review_policy_empty_collections_are_optional(self) -> None:
        for schema_name in ("ReviewKindPolicy", "ReviewKindPolicySnapshot"):
            with self.subTest(schema=schema_name):
                policy = self.openapi["components"]["schemas"][schema_name]
                self.assertIn("clocks", policy["properties"])
                self.assertIn("outcomes", policy["properties"])
                self.assertNotIn("clocks", policy["required"])
                self.assertNotIn("outcomes", policy["required"])
    ...
    def test_review_task_decision_request_is_the_closed_tagged_union(self) -> None:
        ...
        for name in ("reject", "changes_requested", "answer"):
            with self.subTest(decision=name):
                ...
                self.assertEqual(
                    GENERATOR.nullable(GENERATOR.portable_json_object()),
    ...
    def test_review_clock_occurrence_is_the_closed_runtime_shape(self) -> None:
        ...
        self.assertEqual(
            {"type": "string", "format": "uuid"},
            occurrence["properties"]["requestId"],
        )
