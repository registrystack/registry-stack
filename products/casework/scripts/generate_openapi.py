...
MAXIMUM_PORTABLE_JSON_INTEGER = 9_007_199_254_740_991


def portable_json_object() -> dict:
    return {
        "type": "object",
        "additionalProperties": True,
        "x-minimum-json-integer": -MAXIMUM_PORTABLE_JSON_INTEGER,
        "x-maximum-json-integer": MAXIMUM_PORTABLE_JSON_INTEGER,
        "description": (
            "Every nested integer must be in the exactly portable range "
...
def review_schemas() -> dict:
    ...
    integer = {
        "type": "integer",
        "format": "int64",
        "minimum": -MAXIMUM_PORTABLE_JSON_INTEGER,
        "maximum": MAXIMUM_PORTABLE_JSON_INTEGER,
    }
    value = {}
    policy_digest = {"type": "string", "pattern": "^sha256:[0-9a-f]{64}$"}
    result_payload = nullable(portable_json_object())
    outcome = obj(
        {
            "id": text,
            "label": text,
        ...
        ["id", "label", "settlement", "reasonRequired"],
    )
    stage = obj(
        {
            "id": text,
    ...
    subject = obj(
        {"source": text, "type": text, "id": text, "version": text, "digest": policy_digest},
    ...
    review_task_context_data = {
        "oneOf": [
            obj(
                {
                    "strategy": {"const": "submitted"},
                    "snapshot": portable_json_object(),
                },
                ["strategy", "snapshot"],
            ),
    ...
    result = {
        ...
        "ReviewKindPolicy": obj(
            ...
            ["id", "version", "purpose", "contextStrategy", "stages", "retention", "displaySchema"],
        ),
        "ReviewKindPolicySnapshot": obj(
            ...
            ["identity", "purpose", "contextStrategy", "stages", "retention", "displaySchema"],
        ),
        "ReviewKindPolicySnapshotList": array(ref("ReviewKindPolicySnapshot")),
        "ReviewProducerPolicy": obj(
            {"id": text, "profile": text, "issuer": text, "subject": text, "trustedInitiatorIssuer": nullable(text), "initiatorProfile": nullable(text), "sourceNamespaces": array(text), "kinds": array(text), "recoveryDays": {"type": "integer", "minimum": 1, "maximum": 3650}, "completion": nullable(ref("ReviewCompletionDestinationPolicy"))},
        ...
        "ReviewerTask": task,
        "ReviewTaskPage": obj({"items": array(task), "nextCursor": uuid}, ["items"]),
        ...
        "ReviewHistoryEntry": obj({"eventId": uuid, "requestId": uuid, "taskId": uuid, "kind": text, "actorRef": text, "detail": value, "occurredAt": instant}, ["eventId", "requestId", "kind", "detail", "occurredAt"]),
        "ReviewHistoryPage": obj({"items": array(ref("ReviewHistoryEntry")), "nextCursor": uuid}, ["items"]),
        "ReviewNoteRequest": obj({"audience": {"type": "string", "enum": ["reviewers", "requester"]}, "note": review_note}, ["audience", "note"]),
        "ReviewAccountabilityRecord": obj({"eventId": uuid, "requestId": uuid, "taskId": uuid, "actorRef": text, "actor": ref("IssuerPrincipal"), "profileId": text, "decision": text, "privateReason": text, "resultDigest": text, "occurredAt": instant, "retainedUntil": instant}, ["eventId", "requestId", "taskId", "actorRef", "actor", "profileId", "decision", "occurredAt", "retainedUntil"]),
...
def document(contract: dict) -> dict:
    ...
    paths = {
        ...
        "/v1/review-requests/{request_id}/history": {"get": operation("Read audience-filtered review history", "ReviewHistoryPage", source=True, source_required=False, parameters=[REQUEST_ID, parameter("cursor", "query", "Last delivered history event UUID.", {"type": "string", "format": "uuid"}, required=False), parameter("limit", "query", "Bounded page size.", {"type": "integer", "minimum": 1, "maximum": 100}, required=False)], description="Requester history remains bound to the admitted producer. A person selecting a producer's initiatorProfile reads the same requester-visible history, only for a request that producer admitted naming them as its exact initiator; any other request is not found. Human reviewer history requires current Casework queue authority; source context also requires the current caller's source profile and token to disclose the pinned source binding, while submitted context rejects a source profile.")},
    ...
    paths["/v1/review-requests/{request_id}/result"]["get"]["responses"].update(
        {
            ...
            "404": response(description="The request is concealed or unknown."),
    ...
    paths["/v1/review-requests/{request_id}/result"]["get"]["responses"]["404"] = response(
        description="The request is concealed or unknown."
    )
    paths["/v1/review-tasks/{task_id}/draft"]["get"]["responses"]["404"] = response(
        description="No private draft exists for this caller."
    )
...
def load_rust_contract(repository_root: Path) -> dict:
    ...
    required = {"entries", "frameworkProblems", "operations", "reviewWireExamples"}
...
def verify_review_wire_examples(openapi: dict, examples: dict[str, list[object]]) -> None:
    schemas = openapi["components"]["schemas"]
    for schema_name, values in examples.items():
        schema = schemas.get(schema_name)
        if schema is None:
            raise ValueError(f"Rust review wire example has no OpenAPI schema: {schema_name}")
        for value in values:
            if not schema_accepts_example(schema, value, schemas):
                raise ValueError(
                    f"OpenAPI {schema_name} refuses a Rust-serialized review wire example"
                )


def schema_accepts_example(schema: dict, value: object, schemas: dict[str, dict]) -> bool:
    # A structural guard for the exported wire examples, not a general JSON
    # Schema validator. Bounds and cross-field semantics have separate tests.
    if "$ref" in schema:
        return schema_accepts_example(
            schemas[schema["$ref"].rsplit("/", 1)[1]], value, schemas
        )
    if "oneOf" in schema:
        return sum(schema_accepts_example(item, value, schemas) for item in schema["oneOf"]) == 1
    if "anyOf" in schema:
        return any(schema_accepts_example(item, value, schemas) for item in schema["anyOf"])
    ...
    if isinstance(expected_type, list):
        return any(
            schema_accepts_example({**schema, "type": item}, value, schemas)
    ...
    if expected_type == "object":
        ...
        return all(
            ...
            or schema_accepts_example(properties[key], member, schemas)
            for key, member in value.items()
        )
    if expected_type == "array":
        return isinstance(value, list) and all(
            schema_accepts_example(schema.get("items", {}), item, schemas)
            for item in value
        )
...
def main() -> int:
    ...
    try:
        ...
        verify_review_wire_examples(openapi, contract["reviewWireExamples"])
