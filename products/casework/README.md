...
move it) and the review request lifecycle (a request in `reviewing` and the
outcomes it settles into). Each state reports whether it is initial, terminal,
...
Which terminal state a given review reaches is a policy decision, made by the
project's stages, quorum, and exclusions. This report describes the shape
available to every project, not the path one project takes.
