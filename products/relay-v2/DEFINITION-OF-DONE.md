...
8. invalid selected source rows and required transform null/type/length boundaries fail the whole response closed; every Registry proves at least one such refusal, and the coequal suite covers wrong type, missing required value, extra unexpected value, excessive size, partial-string, and date-precision boundaries;
