...
| Issuer portability | Relay accepts conforming tokens from an external authorization server. The optional stock ThunderID journey emits the same Relay audience, operation scopes, and optional authority claims from server-side grants. Relay has no issuer-specific authentication branch, and stock-issuer tooling changes do not block the core Relay V2 Version 1 acceptance path. |
...
- the optional stock ThunderID acceptance journey supplies the social-assistance audience, scope,
  purpose, and service-area binding through the ordinary verifier and access-decision path;
...
- the external-issuer path is complete through the Relay client without an issuer-specific branch;
...
- data and structure have distinct value-free audit surface identifiers, JSON
  ...
  SQLite execution, and terminal audit gates release of exact held bytes;
