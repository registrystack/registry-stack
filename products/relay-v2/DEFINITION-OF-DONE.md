...
| Area | Done when |
|---|---|
| Product boundary | One `relay` runtime and one separate `relayctl` implement the initial product. Relay is a read-only governed registry publisher, not a SQL proxy, write API, policy engine, credential issuer, or signed-assertion service. The existing `registryctl` remains legacy Relay adopter tooling and owns no Relay V2 product command or supported editor-launch path. |
