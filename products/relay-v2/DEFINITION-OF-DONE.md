...
| Business registration | Snapshot SQLite, public and protected access profiles, predefined exact filters, separate list and named Point-bbox search rights, pagination, public semantics | A genuinely public register can isolate protected access while its reviewed pre-derived public view remains discoverable and cacheable. |
...
12. schema-only identification reports, classification inventories, canonical operation explanations, contextual findings, and review-sidecar staleness/tamper refusals are deterministic and value-free; `check --explain` and `generate` return the same explanation.
...
- exact named inclusive bounded `bbox` search, required-bbox enforcement,
  boundary inclusion, malformed, out-of-range, oversize, and antimeridian
  refusal, deterministic pagination, and cursor rejection when operation,
  bbox, access profile, wire format, or format profile changes;
- the public search and separately protected list prove that list and search
  scopes do not imply one another; a protected search access profile is
  independently concealed when denied or unknown;
...
- no GeoJSON negotiation on a nonspatial operation or an access profile
