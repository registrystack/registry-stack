...
`assetCode`. After a module edit, run `registry-serverctl project lock <project>`
...
CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 \
  ...
  check products/registry-server/fixtures/asset-registration-actions

CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 \
  cargo run --locked -p registry-serverctl -- \
  check products/registry-server/fixtures/household-contact-actions
```
...
CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 \
  ...
  products/registry-server/fixtures/household-contact-actions
...
artifact_dir=$(mktemp -d "$repository_root/.registry-server-actions.XXXXXX")

CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 \
  cargo run --locked -p registry-serverctl -- \
  generate actions products/registry-server/fixtures/household-contact-actions \
  --output "$artifact_dir/actions"

CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 \
  cargo run --locked -p registry-serverctl -- \
  generate openapi products/registry-server/fixtures/household-contact-actions \
  --output "$artifact_dir/openapi"

CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 \
  cargo run --locked -p registry-serverctl -- \
  generate schemas products/registry-server/fixtures/household-contact-actions \
  --output "$artifact_dir/schemas"

CARGO_INCREMENTAL=0 CARGO_PROFILE_DEV_DEBUG=0 CARGO_PROFILE_TEST_DEBUG=0 \
  cargo run --locked -p registry-serverctl -- \
  generate sql products/registry-server/fixtures/household-contact-actions \
...
repository temporary directory, calls the real `registry-serverctl test`, and
then removes the resources it created.

With `REGISTRY_SERVER_TEST_DATABASE_URL` and
`REGISTRY_SERVER_TEST_TLS_CA_PEM_PATH` exported by that PostgreSQL setup, run:

```bash
products/registry-server/scripts/test-immediate-action-examples.sh
```
...
Server lifecycle from the [quickstart](quickstart/README.md),
[local demo](demo/README.md), and
[change-request examples](CHANGE_REQUEST_EXAMPLES.md): run `registry-serverctl
test`, package the exact tested candidate, apply it with the migration
authority, verify it, then start `registry-server` with the runtime config used
...
`$HOME/.registry-server-local` with `umask 077`, so bearer tokens are not placed
...
asset_curl_config=$(mktemp "$HOME/.registry-server-local/asset-action.XXXXXX")
printf '%s\n' \
  'silent' \
  'show-error' \
  'request = "POST"' \
  "url = \"$REGISTRY_SERVER_BASE_URL/v1/actions/register-asset-with-inspection\"" \
...
household_create_config=$(mktemp "$HOME/.registry-server-local/household-create.XXXXXX")
printf '%s\n' \
  'silent' \
  'show-error' \
  'request = "POST"' \
  "url = \"$REGISTRY_SERVER_BASE_URL/v1/records/households\"" \
...
service_center_create_config=$(mktemp "$HOME/.registry-server-local/service-center-create.XXXXXX")
printf '%s\n' \
  'silent' \
  'show-error' \
  'request = "POST"' \
  "url = \"$REGISTRY_SERVER_BASE_URL/v1/records/service-centers\"" \
  "header = \"Authorization: Bearer $household_operator_token\"" \
  'header = "Content-Type: application/json"' \
...
condition_curl_config=$(mktemp "$HOME/.registry-server-local/action-condition.XXXXXX")
printf '%s\n' \
  'silent' \
  'show-error' \
  'request = "POST"' \
  "url = \"$REGISTRY_SERVER_BASE_URL/v1/actions/register-household-contact/target-conditions\"" \
...
invoke_curl_config=$(mktemp "$HOME/.registry-server-local/contact-action.XXXXXX")
printf '%s\n' \
  'silent' \
  'show-error' \
  'request = "POST"' \
  "url = \"$REGISTRY_SERVER_BASE_URL/v1/actions/register-household-contact\"" \
  "header = \"Authorization: Bearer $contact_registrar_token\"" \
  'header = "Content-Type: application/json"' \
