...
fixture="$repository_root/products/registry-server/acceptance/household-history"
temporary_root=""
server_pid=""
...
cleanup() {
  if [[ "${REGISTRY_SERVER_HISTORY_KEEP_TEMP:-0}" == "1" ]]; then
  ...
  case "$temporary_root" in
    "$repository_root"/.registry-server-history.*)
...
if [[ -z "${REGISTRY_SERVER_TEST_DATABASE_URL:-}" ]]; then
  printf '%s\n' 'REGISTRY_SERVER_TEST_DATABASE_URL must be set for the historical workflow.' >&2
  exit 2
fi
if [[ -z "${REGISTRY_SERVER_TEST_TLS_CA_PEM_PATH:-}" ]]; then
  printf '%s\n' 'REGISTRY_SERVER_TEST_TLS_CA_PEM_PATH must be set for the historical workflow after the PostgreSQL TLS proof.' >&2
  exit 2
fi
history_tls_ca_pem_path=$REGISTRY_SERVER_TEST_TLS_CA_PEM_PATH
...
temporary_root=$(mktemp -d "$repository_root/.registry-server-history.XXXXXX")
...
if [[ "${REGISTRY_SERVER_SKIP_BUILD:-0}" != "1" ]]; then
...
now = int(time.time())
claims = {
    "aud": "urn:registry-server:history",
...
authentication:
  oidc:
    ...
    audience: urn:registry-server:history
...
derive_history_urls() {
  local database=$1
  python3 - "$REGISTRY_SERVER_TEST_DATABASE_URL" "$database" "$history_migration_role" "$history_runtime_role" "$history_password" <<'PY'
import sys
...
for part in sys.argv[2].split("."):
    value = value[part]
if not isinstance(value, str) or len(value) != 40 or not value.startswith("rs1_"):
...
package_project() {
  ...
  (
    ...
    for selector in openapi schemas manifest metadata sql; do
      "$registry_serverctl" generate "$selector" "$project" --output "./$report_prefix-$selector"
...
start_server() {
  ...
  local url=$2
  local log=$3
  REGISTRY_SERVER_LOG=error "$registry_server" --config "$config" >"$log" 2>&1 &
  server_pid=$!
...
history_schema_test_v1_database="rs_hist_v1_${history_suffix}"
history_schema_test_v2_database="rs_hist_v2_${history_suffix}"
history_schema_test_v3_database="rs_hist_v3_${history_suffix}"
history_production_database="rs_hist_prod_${history_suffix}"
...
history_migration_role="rs_hist_migration_${history_suffix}"
history_runtime_role="rs_hist_runtime_${history_suffix}"
history_author_role="rs_hist_author_${history_suffix}"
history_password="$(python3 - <<'PY'
import secrets
print(secrets.token_hex(18))
PY
)"
history_admin_url=$REGISTRY_SERVER_TEST_DATABASE_URL
...
if [[ "$server_hash_before" != "$server_hash_after" ]]; then
  printf '%s\n' 'registry-server binary changed during the historical workflow.' >&2
  exit 1
fi

checkpoint "all historical workflow checkpoints passed"
printf '%s\n' 'Registry Server historical household workflow passed'
