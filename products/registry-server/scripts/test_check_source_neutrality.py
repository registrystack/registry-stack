...
SPEC = importlib.util.spec_from_file_location("registry_server_source_neutrality", SCRIPT_PATH)
...
class SourceNeutralityTests(unittest.TestCase):
    ...
    def test_fixture_identifier_in_test_code_is_allowed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source.parent.mkdir(parents=True)
            source.write_text("pub struct CompiledRegistry;\n", encoding="utf-8")
            fixture_test = root / "crates/registry-server/tests/asset_fixture.rs"
    ...
    def test_domain_cargo_feature_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            manifest = root / "crates/registry-server/Cargo.toml"
    ...
    def test_domain_migration_and_resource_inputs_are_rejected(self) -> None:
        ...
        for relative, contents in cases:
            with self.subTest(relative=relative), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                source = root / "crates/registry-server" / relative
                source.parent.mkdir(parents=True)
    ...
    def test_public_kernel_contract_canary_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            contract = root / "products/registry-server/contracts/package-layout.yaml"
    ...
    def test_fixture_directories_and_ordinary_docs_are_allowed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            fixture = root / "crates/registry-server/src/fixtures/domain.rs"
            fixture.parent.mkdir(parents=True)
            fixture.write_text("pub struct Farmer;\n", encoding="utf-8")
            docs = root / "crates/registry-server/README.md"
            docs.write_text(
    ...
    def test_fixture_identifier_in_build_script_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            build_script = root / "crates/registry-server/build.rs"
