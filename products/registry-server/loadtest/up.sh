...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --keep)
      ...
      ;;
    *)
      printf '%s\n' "usage: products/registry-server/loadtest/up.sh [--pool-max N] [--keep]" >&2
      exit 2
      ;;
  esac
done
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    printf '%s\n' "$1 is required for the Registry Server load-test environment." >&2
    exit 2
  fi
}
...
if [[ -e "$run_dir/env.json" ]]; then
  ...
  printf '%s\n' "Run products/registry-server/loadtest/down.sh before starting another one." >&2
  exit 2
fi
if [[ -L "$run_dir" ]]; then
...
read -r database_port mint_port server_port metrics_port <<EOF
$ports
EOF
...
container="registry-server-loadtest-${PPID}-$$"
...
openssl req -x509 -new -nodes -newkey rsa:2048 -sha256 -days 2 \
  -subj '/CN=Registry Server load-test CA' \
...
printf '%s\n' '== Starting Registry Server on loopback (background, metrics enabled)'
REGISTRY_SERVER_LOG=info nohup "$registry_server" --config "$run_dir/runtime.yaml" \
...
printf '  Seed next:        products/registry-server/loadtest/seed.py --count 100000\n'
printf '  Then run:         products/registry-server/loadtest/run.sh --profile steady\n'
printf '  Tear down with:   products/registry-server/loadtest/down.sh\n'
