...
case "$action" in
  ...
  *)
    printf '%s\n' 'usage: products/registry-server/loadtest/dbstats.sh reset|snapshot|sample [interval-seconds]|analyze' >&2
    exit 2
    ;;
esac
...
WITH top_statements AS (
  SELECT queryid::text AS "queryId",
         CASE
           ...
           WHEN strpos(lower(query), 'rs_e_') > 0 THEN 'record'
           ELSE 'other'
         END AS category,
         calls,
  ...
  WHERE strpos(relname, 'registry_audit') = 1 OR strpos(relname, 'rs_e_') = 1
