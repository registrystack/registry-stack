# Registry Witness User Stories

## Purpose

These user stories describe the core Registry Witness product journeys. They
...
As an authorized service, I want to ask Registry Witness whether a subject
...
- The caller can request only configured claims, purposes, formats, and
  disclosures.
- Registry Witness returns only the configured claim result or disclosure, not
  raw source rows.
...
- Self-attestation is disabled by default and requires OIDC authentication when
  enabled.
- Registry Witness verifies the token, scopes, client or audience policy, and
...
As a wallet holder, I want Registry Witness to issue a short-lived SD-JWT VC
...
- The requested credential profile, disclosure, and format are allowed by
  configuration.
- When the profile requires holder binding, Registry Witness validates proof of
...
## 4. Witness Evaluates A Claim Through An OpenFn Sidecar Source

As an implementer, I want Registry Witness to evaluate claims using a
...
Registry Witness itself.

Acceptance criteria:

- Registry Witness calls the sidecar through the existing registry data source
  connector contract.
...
- Registry Witness remains the attestation authority for caller auth, claim
