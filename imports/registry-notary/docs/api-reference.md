...
authoritative OpenAPI-described API surface is the document generated by
`cargo run -p registry-notary-bin -- openapi`; non-OpenAPI static and probe
routes are tracked in `security/exposure-manifest.json`. The authoritative set
of application problem codes lives in the server code. When this page and an
