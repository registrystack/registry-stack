use registry_witness_openfn_sidecar::{WorkerCommand, WorkerError, WorkerPool, WorkerPoolConfig};
use serde_json::json;
use std::{
    ffi::OsString,
    path::PathBuf,
...
async fn returns_saturated_when_all_workers_are_busy() {
    ...
    let busy = tokio::spawn(async move {
        busy_pool
            .execute_json(json!({ "mode": "sleep", "sleep_ms": 200 }))
            .await
            .unwrap()
    });
...
fn unique_state_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "registry-witness-openfn-sidecar-worker-fixture-{nanos}.state"
    ))
}
