...
The Relay returns `200 OK` with the catalog. Additional `DatasetSummary` fields are omitted here
for brevity:
...
For a pure API credential flow, use the hosted DHIS2 Notary. This path does not use the browser
wallet sign-in. It evaluates DHIS2 child-program claims, issues an `application/dc+sd-jwt`
...
Set the hosted Notary URL and load the current demo bearer token from the lab JSON:
...
```sh
curl -fsS "$DHIS2_NOTARY_URL/.well-known/evidence/jwks.json" \
  -H "Authorization: Bearer $DHIS2_EVIDENCE_CLIENT_BEARER" \
  -o jwks.json
