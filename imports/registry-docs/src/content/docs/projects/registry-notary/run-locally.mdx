---
title: Run Registry Notary locally
description: How to start a Registry Notary instance on your laptop and confirm the service responds to a claims evaluation request.
status: draft
...
- Rust toolchain (stable) installed and `cargo` on your `PATH`.
  ...
  [Cargo.toml](https://github.com/jeremi/registry-notary/blob/main/Cargo.toml).
...
Registry Notary starts fail-closed: no protected route is served unless static credentials or OIDC auth are configured.
...
[`sd_jwt.rs` tests](https://github.com/jeremi/registry-notary/blob/main/crates/registry-notary-core/src/sd_jwt.rs):
...
([demo/config/registry-notary.yaml:7](https://github.com/jeremi/registry-notary/blob/main/demo/config/registry-notary.yaml)).
The default bind address when no config specifies one is `127.0.0.1:8081`
([config.rs:143](https://github.com/jeremi/registry-notary/blob/main/crates/registry-notary-core/src/config.rs)).
