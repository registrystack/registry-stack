---
title: Registry Notary reference
description: Hand-maintained reference for the Registry Notary claim config schema, HTTP routes, federation config, disclosure modes, source connector types, and audit event shape.
status: draft
...
([README](https://github.com/jeremi/registry-notary/blob/main/README.md))
...
Registry Notary emits OpenID for Verifiable Credential Issuance (OID4VCI) Draft 13 compatible routes when `oid4vci.enabled` is `true` in config. The wider [OID4VCI](../../../reference/standards/) spec alignment is `aligns_with`; the four routes below `emit` the wire shapes the spec defines.
...
- Header fields: `typ = registry-notary-request+jwt`, `alg = EdDSA`, and a `kid` found in the
...
The serving Notary validates the request before source reads. Audit write failure prevents a
