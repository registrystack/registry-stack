...
description: Registry Manifest turns a single YAML metadata manifest into every
  standards-facing artifact a registry needs to publish, validate, and share.
status: current
...
database scopes, or backend credentials.
Those live in Registry Relay configuration; the manifest is portable and Relay-independent.
