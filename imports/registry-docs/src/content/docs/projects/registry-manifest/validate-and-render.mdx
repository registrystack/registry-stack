...
Check a `metadata.yaml` file for errors, inspect individual rendered artifacts (DCAT, BRegDCAT-AP, SHACL, JSON Schemas, etc.), or produce a complete static publication bundle for hosting.

The validator collects all errors across every validation pass before reporting, so a single run surfaces every problem in the file. You don't need to fix one error at a time and re-run.
