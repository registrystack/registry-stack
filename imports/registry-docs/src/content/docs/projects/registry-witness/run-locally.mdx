...
The steps below start Registry Witness with the demo config and use `curl` to probe the claims list, the JWKS endpoint, and the service capabilities document. Full claim evaluation requires a source registry at the configured `base_url`; [configure a claim](../configure-a-claim/) covers that step, and [Registry Lab](../../registry-lab/) wires the full topology together.
