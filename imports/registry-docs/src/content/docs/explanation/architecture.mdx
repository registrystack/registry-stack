...
5. Registry Notary evaluates claims against configured HTTP sources (`registry_data_api`, `dci`,
   or source adapter sidecar connectors — the sidecar uses built-in `http_json`, `http_flow`, or
   `fhir` engines to reach external sources such as DHIS2 or FHIR servers). It renders evaluation results as
