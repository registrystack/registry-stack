...
description: Record of the decision to rename five registry projects to the registry family naming convention and the current state of each rename.
status: current
...
This is a maintenance record. If you're an integrator using the current project names (Registry Relay, Registry Witness, and the rest of the family), you can ignore the legacy column.
...
| `evidence-server` | `registry-witness` | Standalone claim evaluation, credential issuance, and attestation |
| `dataspace-atlas` | `registry-atlas` | Standards-first workbench for inspecting registry discovery artifacts |
| `decentralized-evidence-demo` | `registry-lab` | Compose-based runnable demo of the registry family |

Human-readable brand strings flip in lock-step: "[Registry Relay](../../projects/registry-relay/)" stays; the other four adopt
...
Phase 1 (registry-manifest), Phase 2 (registry-atlas), Phase 3 (registry-witness),
...
- **Access kind enum value**: `evidence-server` to `registry-witness`
- **Media type**: `application/vnd.evidence-server.claim-result+json` to `application/vnd.registry-witness.claim-result+json`
...
updating the Cargo dependency on `registry-manifest-core`, swapping witness contract strings
(`evidence-server` to `registry-witness` in `access.kind`, `conforms_to`, and OpenAPI enum),
...
{/* TODO[evidence]: Confirm whether registry-relay has absorbed Phase 4 contract changes:
     (1) Cargo dep updated from registry-metadata-core to registry-manifest-core at tag v0.1.2,
     (2) access.kind "evidence-server" replaced with "registry-witness" in openapi.rs and validate.rs,
     (3) snapshot files in tests/snapshots/ regenerated.
     The relay evidence packet reports the binary name is correct but does not enumerate schema version
     or kind strings present in the working tree at commit aec4394. */}

### Registry Witness

**Source:** registry-witness evidence packet, "Naming and rename status" section.

The rename is fully landed locally.
Crate names are `registry-witness-core`, `registry-witness-server`, and `registry-witness-bin`.
The media type constant reads `application/vnd.registry-witness.claim-result+json` (model.rs:9).
The default service_id is `registry-witness` (config.rs:249).
The env var is `REGISTRY_WITNESS_ISSUER_JWK` (config.rs:561).
...
The git submodules point to `vendor/registry-relay` and `vendor/registry-witness`
(`.gitmodules`, registry-lab evidence packet "Vendored or third-party content").
Compose service names use `registry-relay:demo` and `registry-witness:demo` images.
...
Docstrings and comments are residual and could be updated to say "[Registry Lab](../../projects/registry-lab/) demo"
...
Registry Legend v0 prose uses the current names in the "Decision" table for all five projects.
