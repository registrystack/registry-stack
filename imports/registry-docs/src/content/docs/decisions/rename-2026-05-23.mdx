...
schema versions (`registry-metadata/v1` to `registry-manifest/v1`),
JSON-LD CURIE prefix (`registry_metadata:` to `registry_manifest:`),
canonical IRI (to `https://jeremi.github.io/registry-manifest/ns#`),
access kind enum value (`evidence-server` to `registry-witness`),
media type (`application/vnd.evidence-server.claim-result+json` to
`application/vnd.registry-witness.claim-result+json`),
JWT audience default, Cargo feature names, and project-labeled env vars.
