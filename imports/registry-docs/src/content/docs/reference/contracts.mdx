...
- The [Registry Relay](../../products/registry-relay/) OpenAPI artifact is a hand-authored abstract contract (`hand-authored-abstract-contract`).
  ...
  Registry Relay also exposes a per-instance OpenAPI dump through `GET /openapi.json`, which callers can fetch from a configured gateway to discover the concrete `{dataset_id}` and `{entity}` shape for that deployment. Runtime deployments gate that route by default unless `openapi_requires_auth` is disabled for demos or controlled tooling.
