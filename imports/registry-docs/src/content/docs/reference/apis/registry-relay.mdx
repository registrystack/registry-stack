...
Evidence source: [Registry Relay](../../../projects/registry-relay/) `main` commit `5a2a8f2`, generated after the `v0.1.1` release and reviewed 2026-05-24.
...
The Redoc artifact is a pinned civil demo snapshot generated from the current Relay runtime and includes catalog, metadata, dataset, policy, and evidence-offering response examples for the configured demo catalog.
...
export. Some optional routes and configurations are not reflected, including OIDC auth, SP DCI adapter,
OGC API Records, DID, schema, and context routes unless the runtime is configured with those features.
