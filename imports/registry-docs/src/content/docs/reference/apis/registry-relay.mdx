...
Evidence source: [Registry Relay](../../../projects/registry-relay/) commit `aec4394300c87e1f0e93b73580c9b12150552111`, reviewed 2026-05-23.

## API surface

Registry Relay is a Rust async HTTP service (Axum framework) that turns sensitive government
tabular files and database tables into protected, read-only, domain-oriented consultation APIs.
The OpenAPI document covers the routes registered in `src/server.rs`.
...
The server validates against a SHA-256 fingerprint stored in an environment variable, not in the config file.
...
and `GET /metrics` are mounted on a separate admin listener (`server.admin_bind`, default
`127.0.0.1:8081`) and require `admin` scope. They do not appear in the public OpenAPI document.

**Artifact snapshot.** The current artifact is a pinned civil demo snapshot, not a release-grade
export. Some configurations (OIDC auth, SP DCI adapter, OGC API Records) may not be reflected.
...
Kafka, pub/sub, or event-streaming backend.
