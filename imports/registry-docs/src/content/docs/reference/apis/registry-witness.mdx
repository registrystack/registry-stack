...
Evidence source: [Registry Witness](../../../projects/registry-witness/) release tag `v0.2.1` (`7b5cc252d1a4d4231f6fe379b4dab67348d4c950`), reviewed 2026-05-24.
...
{/* TODO[evidence]: upstream v0.2.1 generator omits info.license, so Redocly reports one warning. */}
...
The pinned OpenAPI document is generated from the `v0.2.1` source.
The artifact's `info.version` currently reports `0.1.0`; use the release tag as the reviewed version.
Routes are defined in `crates/registry-witness-server/src/api.rs`.
The generated OpenAPI artifact covers the protected API routes and omits the public probe routes
and protected admin no-op route that the runtime still mounts.
