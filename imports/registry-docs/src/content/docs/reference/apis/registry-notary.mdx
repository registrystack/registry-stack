...
description: Narrative context for the Registry Notary OpenAPI reference, covering API surface, auth modes, disclosure profiles, and documented limitations.
status: current
...
[Open Registry Notary API reference](/registry-docs/api/registry-notary.html)
...
Evidence source: [Registry Notary](../../../projects/registry-notary/) `main` commit `5b6be8a`, generated after the `v0.2.1` release and reviewed 2026-05-24.
...
Registry Notary is a standalone Rust workspace for claim evaluation, disclosure policy,
credential issuance, and audit.
Registry Notary runs independently; it does not require Registry Relay to be running.
...
Registry Notary controls what a caller receives through a three-level disclosure system declared in
...
When a caller requests `POST /credentials/issue`, Registry Notary issues an SD-JWT VC credential.
The credential uses:
...
Registry Notary does not emit W3C Verifiable Credentials Data Model envelopes.
The credential format is SD-JWT, not a W3C VC JSON object.
{/* Confirmed absence: notary evidence packet explicitly states "W3C Verifiable Credentials Data Model: NOT PRESENT" (.research/registry-notary.md). The SD-JWT vs W3C VC distinction is a boundary statement, not an evidence gap. */}
...
Registry Notary reads registry facts from external sources over HTTP.
...
There is no generic file-based or database connector in Registry Notary.
Source connectors must reach their targets over HTTP.

## What the OpenAPI specification does not cover

**Plugin rule type.** The `Plugin` rule variant is declared in `crates/registry-notary-core/src/config.rs`
