...
This register lists every standard the registry family touches, the projects that touch it, and the claim level that names how strong the relationship is. The registry family does not introduce a new standard; each project emits, maps to, or aligns with existing ones, and says so explicitly.
