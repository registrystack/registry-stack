...
| 0.1.3 | 2026-06-22 | draft | Document delegated self-attestation, proof-claim gating, delegated denial reasons, and the OID4VCI delegated-token rejection boundary. |
...
- `delegated_attestation`: the authenticated principal is the requester, and a configured dependent target can be evaluated only under a configured relationship and proof claim.
...
REQ-PR-NOTARY-029: For delegated self-attestation, Registry Notary MUST derive the requester from the authenticated principal, derive the relationship from scoped authorization details, and reject caller-supplied `requester`, `relationship`, or `on_behalf_of` fields before any source read.
...
- treats self-attestation as constrained authenticated trust context, including delegated self-attestation only when requester, dependent target, relationship proof, and stored-evaluation metadata all bind to the configured policy (REQ-PR-NOTARY-026, REQ-PR-NOTARY-029, REQ-PR-NOTARY-030, REQ-PR-NOTARY-031);
- exposes OID4VCI as a scoped non-full-issuer subset that requires holder proof of possession and rejects delegated-attestation transaction tokens (REQ-PR-NOTARY-016, REQ-PR-NOTARY-017, REQ-PR-NOTARY-027, REQ-PR-NOTARY-032);
...
- Delegated self-attestation access modes, source capabilities, stored metadata, and denial reasons are implemented in [`model.rs`](https://github.com/jeremi/registry-notary/blob/37215c8543f4704a88b6e7228cb984b3cbf3f5ab/crates/registry-notary-core/src/model.rs).
- Delegated relationship configuration validation is implemented in [`config.rs`](https://github.com/jeremi/registry-notary/blob/37215c8543f4704a88b6e7228cb984b3cbf3f5ab/crates/registry-notary-core/src/config.rs).
- Request context derivation, stored-evaluation revalidation, OID4VCI delegated-token rejection, and proof-gated source reads are implemented in [`api.rs`](https://github.com/jeremi/registry-notary/blob/37215c8543f4704a88b6e7228cb984b3cbf3f5ab/crates/registry-notary-server/src/api.rs) and [`runtime.rs`](https://github.com/jeremi/registry-notary/blob/37215c8543f4704a88b6e7228cb984b3cbf3f5ab/crates/registry-notary-server/src/runtime.rs).
