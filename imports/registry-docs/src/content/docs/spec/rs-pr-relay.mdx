...
description: "The normative HTTP protocol contract for Registry Relay: the configuration-driven service model, authentication and per-dataset scopes, consultation and aggregate routes, scoped metadata publication, evidence offerings, signed response credentials, and audit behavior."
status: draft
owner: registry-docs
source_repos:
  - registry-relay
...
- **Exact routes and schemas.** The generated OpenAPI document, rendered as the [Registry Relay API reference](../../reference/apis/registry-relay/), is the authoritative route and schema reference. This document states behavior a schema cannot, and does not restate request and response shapes that would drift from the generated source.
- **The metadata manifest format.** The `metadata.yaml` schema and its renderers are owned by Registry Manifest; the manifest data model belongs to a future RS-DM-\* specification. This document refers to the manifest only where Relay serves or scopes its compiled artifacts.
- **Security model internals.** Key management, the OIDC trust configuration, and audit-envelope structure belong to a future RS-SEC-\* specification. This document states the authorization behavior a caller observes.
...
REQ-PR-RELAY-001: Registry Relay MUST NOT expose write paths in v1. Sources are read as batch snapshots or table scans; there is no event-stream backend and no write-back. This is the protocol-level form of REQ-ARC-G-003.
...
Registry Relay can serve the manifest's standards-shaped metadata at `/metadata/*`, using the Registry Manifest renderers at request time. Unlike a static publication, the runtime view is filtered by the caller's identity: a caller sees only the datasets and entities its `metadata` scope permits.

REQ-PR-RELAY-010: Where metadata publication is enabled, Registry Relay MAY serve catalog, DCAT, BRegDCAT-AP, CPSV-AP, SHACL, JSON Schema, ODRL, and OGC API Records artifacts at `/metadata/*`. The runtime view MUST be filtered by the caller's `metadata` scope, so a caller whose scope covers one dataset does not receive metadata for datasets it cannot read.
...
REQ-PR-RELAY-013: Signed response credentials are opt-in, enabled by the `provenance` configuration key. When the feature is enabled and a caller requests one of the configured `accepted_media_types` (by default `application/vc+jwt` or `application/jwt`), Registry Relay MAY attach a W3C Verifiable Credentials Data Model (VCDM) 2.0 VC-JWT to an entity-record or aggregate response, and MUST publish the supporting JSON Schemas and JSON-LD contexts under `/.well-known/`. In gateway issuer mode, Registry Relay MUST publish the issuer `did:web` document at `/.well-known/did.json`; in delegated issuer mode it MUST NOT, because the delegating ministry hosts the DID document for its own domain.
...
Several surfaces mount only when the gateway is built and configured with the matching Cargo feature. A reader MUST NOT infer their presence from the route catalog of a different build.

REQ-PR-RELAY-017: Feature-gated surfaces (the OGC API Features, Records, and EDR adapters; the SP DCI sync routes; and the signed response credentials of Section 8) mount only when the gateway is built and configured with the matching feature. A conformance claim against this specification MUST NOT imply a surface that the deployed build does not mount.

REQ-PR-RELAY-018: Admin routes (reload, metrics) MUST be served on a separate, optional admin listener, MUST require the `registry_relay:admin` scope, and MUST NOT appear in the public OpenAPI document. Runtime deployments gate the public OpenAPI document by default; it is exposed without authentication only when `openapi_requires_auth` is disabled for demos or controlled tooling.
...
- **No writes, no streaming.** Registry Relay performs no writes in v1 and reads sources as batch snapshots or table scans; there is no event-stream backend (REQ-PR-RELAY-001).
- **Feature-gated surfaces.** OGC adapters, SP DCI sync, and signed response credentials are present only in builds configured with the matching feature (REQ-PR-RELAY-017).
...
- exposes no write paths and never widens a caller's reach at request time (REQ-PR-RELAY-001, REQ-PR-RELAY-002);
...
- scope-filters the metadata it serves and does not own the manifest format (REQ-PR-RELAY-010, REQ-PR-RELAY-011);
...
- mounts feature-gated and admin surfaces only as built and configured (REQ-PR-RELAY-017, REQ-PR-RELAY-018).
...
- The [standards register](../../reference/standards/) records the adoption mode for DCAT, BRegDCAT-AP, CPSV-AP, SHACL, ODRL, OGC API Records, SDMX, SP DCI, and the W3C standards listed in `standards_referenced`.
