...
For an API you operate, change the source options at project creation time:
...
From the standalone project directory, the rest of the flow is the same as the sections you
already ran: `registryctl start`, `registryctl notary smoke`, load this project's keys with the
`set -a` block, and evaluate the claim against `http://127.0.0.1:4255/v1/evaluations`.
