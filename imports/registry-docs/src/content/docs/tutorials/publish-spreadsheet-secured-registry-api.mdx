...
The installer writes to `~/.local/bin` by default, so make sure that directory is on your
`PATH` before you confirm the install.
...
```sh
cargo install --git https://github.com/jeremi/registry-registryctl --branch main --locked
```
...
- [Verify a claim from your registry API](../verify-claim-registry-api/): add Registry Notary to
  the project and evaluate a claim without exposing the source row.
...
| `registryctl start` fails and the container log shows `failed to parse config YAML ... unknown field` | A locally cached `snapshot` container image is older or newer than the config `registryctl` generates. | Run `docker pull ghcr.io/jeremi/registry-relay:snapshot` (and `docker pull ghcr.io/jeremi/registry-notary:snapshot` if Notary is in the project), then `registryctl start` again. |
