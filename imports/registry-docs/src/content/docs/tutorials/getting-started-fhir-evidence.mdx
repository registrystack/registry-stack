...
or the [`registryctl` Notary tutorial](../verify-claim-registry-api/). The FHIR
source-adapter profile is not scaffolded by `registryctl` yet, so this tutorial
uses `registry-lab` directly.
