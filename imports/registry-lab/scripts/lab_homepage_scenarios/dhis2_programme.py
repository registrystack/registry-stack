...
def preview_step(config: dict[str, Any], step_id: str) -> dict[str, Any]:
    if step_id == "discover":
        ...
        _, display_headers = _headers(config)
        return request_source("GET", url, display_headers)
    ...
    if step_id == "negative-control":
        url = _url(config, "/v1/evaluations")
        body = _tracked_entity_body(NEGATIVE_SUBJECT_ID, ["dhis2-child-program-active"], "predicate", CLAIM_RESULT_FORMAT)
        _, display_headers = _headers(config, {"Content-Type": "application/json", "Data-Purpose": PURPOSE})
        return request_source("POST", url, display_headers, body)
    if step_id == "render-cccev":
        ...
        _, display_headers = _headers(
            config,
            {"Accept": CCCEV_FORMAT, "Content-Type": "application/json", "Data-Purpose": PURPOSE},
        )
        render_url = _url(config, "/v1/evaluations/[evaluation-id]/render")
        render_body = {"claims": PROGRAMME_CLAIMS, "disclosure": "value", "format": CCCEV_FORMAT}
        _, render_display_headers = _headers(config, {"Content-Type": "application/json"})
        return {
