...
`registry-platform` is pre-1.0 and consumed by Registry Relay and Registry Witness through pinned git tags. Security fixes are released on the newest tag line only unless Jeremi explicitly opens a backport lane for a consumer migration.
...
- Impacted consumers, if known: Registry Relay, Registry Witness, or both.
