...
This guide covers the four product clients, their Node.js and Python bindings,
`registry-stack-client`, the unified native-package facades, and shared
`registry-record` DTOs. Apply the owning product guide as well.
...
| Area | Owns |
|---|---|
| `registry-{breg,discovery,evidence,relay}-client` | Canonical Rust product HTTP and response contract |
| `registry-evidence-verifier` | Evidence response formats, payload contract, and relying-party verification |
| Product `-client-node` / `-client-py` crates | Thin napi-rs / PyO3 bindings and language conversion over the Rust decisions |
| `registry-stack-client` | Curated Rust facade with separate product, record, and auth modules |
...
| `registry-breg-client-node` / `registry-breg-client-py` | Internal native BReg bindings used by unified distributions |
...
`<product>` with `breg`, `discovery`, `evidence`, or `relay`:
...
are generated from all four product bindings. When changing that assembly,
...
combines four version-matched internal native wheels with the public facade;
