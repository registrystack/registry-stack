...
if [ "$os_label" = "linux" ]; then
	...
	if ls /lib/ld-musl-*.so.1 >/dev/null 2>&1 ||
		...
		printf 'No musl build of Registry Relay is published for this platform.\n' >&2
	...
	*) detected_libc="" ;;
	...
	'; then
		printf 'This system has GNU libc %s. The published binaries of Registry Relay need %s or newer.\n' \
