...
impl PostgresStore {
    ...
    pub(crate) async fn check_review_task_grant_eligibility(
        ...
        transaction
            ...
            .await?;
        let eligible = template_active(&transaction, template).await?
                ...
                .execute(
                    "UPDATE casework_review_task_grants
                     SET invalidated_at=COALESCE(invalidated_at,now()),
                         invalidation_reason=COALESCE(invalidation_reason,'eligibility')
                     WHERE grant_id=$1",
                    &[&grant.id],
                )
                .await?;
...
impl crate::CaseworkService {
    ...
    async fn active_review_task(
        ...
        if !self
            .store
            .check_review_task_grant_eligibility(grant, template)
            .await?
        {
            return Err(crate::ServiceError::Forbidden);
        }
