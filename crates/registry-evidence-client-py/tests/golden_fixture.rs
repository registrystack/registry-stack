//! Golden fixture for the Python suite's `discover`/`fetch_jwks` stubs and for
//! a direct Rust-side check that a stored response still verifies.
//!
//! A Python test cannot sign an Evidence response itself: the crates that can
...
//! `registry-platform-crypto`, the same way those crates' own tests do, and
//! the same way `crates/registry-evidence-client-node/tests/golden_fixture.rs`
//! does for the Node suite. The two fixture generators are deliberately
//! duplicated rather than shared: factoring the pattern out into a common
//! test-support crate is a real option, but a decision this task did not ask
//! to make.
//!
//! Regenerate with:
//! ```text
...
// This file is a separate integration-test crate, and Cargo auto-links two
// different crates under the identical name `registry_evidence_client` here:
// this package's own compiled library (its `[lib] name`) and the wrapped SDK
// crate (`registry-evidence-client`, a `[dependencies]` entry with no custom
// lib name of its own). Both entries genuinely belong to the extern prelude
// under the same string, so unlike `src/lib.rs` (where the leading `::` picks
// between a same-named crate-root item and a single extern crate), a leading
// `::` alone cannot disambiguate two same-named extern crates. The SDK crate
// is instead pulled in under its own alias, `evidence_client_sdk` (see this
// crate's `Cargo.toml`), so every reference below is unambiguous by
// construction.
...
fn fixture_evidence(issued_at: DateTime<Utc>, valid_until: DateTime<Utc>) -> Evidence {
    Evidence {
        ...
        id: "urn:example:evidence:python-fixture".to_owned(),
