...
binaries=(scheduling schedulingctl)
# Publication packaging replaces this empty value with the asset's canonical tag.
default_version=""
script_name="${BASH_SOURCE[0]:-}"
script_name="${script_name##*/}"
filename_version=""
if [[ "$script_name" =~ ^scheduling-(v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*))-install\.sh$ ]]; then
...
version="${SCHEDULING_VERSION:-$default_version}"
if [ -n "$default_version" ] &&
	[ -n "${SCHEDULING_VERSION:-}" ] &&
	[ "$SCHEDULING_VERSION" != "$default_version" ]; then
	echo "Refusing a release override that does not match the released installer asset." >&2
	exit 1
fi
install_dir="${SCHEDULING_INSTALL_DIR:-$HOME/.local/bin}"
asset_dir="${SCHEDULING_ASSET_DIR:-}"

usage() {
	cat <<EOF
Install the Registry Scheduling runtime and the schedulingctl adopter tooling.

After selecting a release whose manifest lists Scheduling:
  curl -fsSL https://github.com/${repo}/releases/download/<version>/scheduling-<version>-install.sh | bash
...
pinned tag, then rerun with SCHEDULING_ASSET_DIR set to the verified
directory:
  https://github.com/${repo}/blob/<version>/release/VERIFY.md

Environment:
  SCHEDULING_VERSION      Registry Scheduling tag to install. A published installer
                    embeds its tag and refuses a different override.
  SCHEDULING_INSTALL_DIR  Install directory. Defaults to ~/.local/bin.
  SCHEDULING_ASSET_DIR    Read already-downloaded release assets from this directory
...
need() {
	if ! command -v "$1" >/dev/null 2>&1; then
		echo "Scheduling installer needs '$1'." >&2
		exit 1
	fi
}

if [ -z "$version" ]; then
	echo "No Registry Scheduling tag is pinned for this installer copy." >&2
	echo "Set SCHEDULING_VERSION to a pinned vMAJOR.MINOR.PATCH tag, or run a" >&2
	echo "published scheduling-<tag>-install.sh asset." >&2
	exit 1
fi
if [[ ! "$version" =~ ^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$ ]]; then
	echo "Refusing a non-canonical Registry Scheduling tag." >&2
...
if [ "$os_label" = "linux" ]; then
	...
	if [ "$musl_system" = 1 ]; then
		printf 'No musl build of Registry Scheduling is published for this platform.\n' >&2
	...
	*) detected_libc="" ;;
	...
	'; then
		printf 'This system has GNU libc %s. The published binaries of Registry Scheduling need %s or newer.\n' \
...
tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t scheduling)"
...
if [ -n "$asset_dir" ]; then
	printf 'Installing verified local Registry Scheduling %s assets for %s/%s...\n' \
		"$version" "$os_label" "$arch_label"
else
	printf 'Downloading Registry Scheduling %s for %s/%s...\n' \
		"$version" "$os_label" "$arch_label"
fi
...
sha256_file() {
	...
	if command -v shasum >/dev/null 2>&1; then
		...
		echo "Scheduling installer needs 'shasum' or 'sha256sum' for checksum verification." >&2
		exit 1
	fi
	printf '%s\n' "${result%% *}"
}
...
guide first, then rerun this installer with SCHEDULING_ASSET_DIR set to
...
mkdir -p "$install_dir"
stage_dir="$(mktemp -d "$install_dir/.scheduling-toolset.XXXXXX")"
link_stage_dir="$(mktemp -d "$install_dir/.scheduling-links.XXXXXX")"
...
current_link="$install_dir/.scheduling-current"
if [ -e "$current_link" ] && [ ! -L "$current_link" ]; then
	echo "Refusing to replace a non-symbolic Registry Scheduling toolset pointer." >&2
	exit 1
fi
...
if [ ! -L "$current_link" ]; then
	previous_dir="$(mktemp -d "$install_dir/.scheduling-previous.XXXXXX")"
...
# A command not already linked through Scheduling's pointer, such as a shared
...
case ":$PATH:" in
*":$install_dir:"*) ;;
*) echo "Add $install_dir to PATH to run Registry Scheduling from any shell." >&2 ;;
esac
