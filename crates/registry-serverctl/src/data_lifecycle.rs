...
//! Authenticated HTTP data workflows for Registry Server.
//!
...
//! remain in `registry_server::data`.
...
use registry_server::package::{inspect_package_integrity, PackageEnvelope, PackageError};
...
const DATA_HTTP_USER_AGENT: &str = "registry-serverctl-data";
const DATA_STATE_API_VERSION: &str = "registry.registrystack.org/serverctl-data/v1";
const IMPORT_STATE_KIND: &str = "RegistryServerctlDataImportState";
...
pub(crate) fn run_import(
    ...
    let server_url = parse_server_url(request.server_url)?;
    ...
    let (_committed_chunks, committed_items) = run_import_chunks(
        ...
        |data_request| dispatch_http(&client, &server_url, &token, data_request),
    )?;
...
impl InspectedDataPackage {
    fn registry(&self) -> &registry_server::CompiledRegistry {
        &self.registry
    }
}
...
fn parse_server_url(value: &str) -> Result<Url, DataLifecycleError> {
    let mut url = Url::parse(value).map_err(|_| DataLifecycleError::ServerUrl)?;
    if url.username() != ""
    ...
    {
        return Err(DataLifecycleError::ServerUrl);
    }
...
fn atomic_write_temporary_path(parent: &Path, sequence: u64) -> PathBuf {
    parent.join(format!(
        ".registry-serverctl-data-{}-{sequence}.tmp",
        std::process::id()
    ))
}
...
mod tests {
    ...
    fn compiled() -> registry_server::CompiledRegistry {
        let source = json!({
...
"#;
        ...
        assert!(idempotency_key
            .as_deref()
            .is_some_and(|key| key.starts_with("rs-data-v1-")));
...
"#;
        ...
        .unwrap();
        let base = parse_server_url(&format!("http://{address}")).unwrap();
    ...
    fn data_http_client_ignores_ambient_proxy_variables_in_an_isolated_process() {
        if std::env::var_os("REGISTRY_SERVERCTL_DATA_PROXY_CHILD").is_some() {
            return;
        }
        let status = Command::new(std::env::current_exe().unwrap())
            ...
            .arg("--nocapture")
            .env("REGISTRY_SERVERCTL_DATA_PROXY_CHILD", "1")
    ...
    fn data_http_client_ignores_ambient_proxy_variables_child() {
        if std::env::var_os("REGISTRY_SERVERCTL_DATA_PROXY_CHILD").is_none() {
            return;
        }
...
"#;
    ...
    #[test]
    fn server_urls_are_closed_and_remote_http_is_refused() {
        assert!(parse_server_url("https://registry.example.test").is_ok());
        ...
        for refused in [
            ...
            assert!(parse_server_url(refused).is_err(), "{refused}");
        }
    }
