...
use registry_server::contract::ModuleLockSource;
use registry_server::{parse_project_yaml, Diagnostic, DiagnosticSeverity};
...
pub(crate) fn migrate_registry_yaml(bytes: &[u8]) -> Result<MigratedProject, Diagnostic> {
    ...
    // ordinary block-style YAML dialect emitted by Registry Server examples. Ambiguous flow-style
...
fn invalid_legacy() -> Diagnostic {
    Diagnostic {
        ...
        path: "registry.yaml".to_owned(),
        message: "the project is not a valid singular Registry Server authoring document"
            .to_owned(),
    }
}
