...
const CONFIG_VALUE_CANARY: &str = "registry-server-doctor-config-value-canary";
const PATH_VALUE_CANARY: &str = "registry-server-doctor-path-value-canary";
...
impl TestDirectory {
    fn create() -> Self {
        let path = std::env::current_dir()
            ...
            .join(format!(
                "registry-serverctl-doctor-test-{}-{}",
                std::process::id(),
...
fn run(runtime_config: &Path) -> (u8, String, String) {
    ...
    let status = registry_serverctl::run_from(
        [
...
fn doctor_help_names_the_absolute_runtime_configuration_contract() {
    ...
    let mut stderr = Vec::new();
    let status = registry_serverctl::run_from(
        ["registry-serverctl", "doctor", "--help"],
        &mut stdout,
        &mut stderr,
    );
