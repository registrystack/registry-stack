...
fn external_review_status_is_strict_typed_and_value_redacted() {
    ...
    assert_eq!(
        review.submission().recovery_deadline(),
    ...
    assert_eq!(review.application().attempts(), Some(3));
    assert_eq!(review.application().next_attempt_at(), None);
    assert!(!review.application().receipt_recovered());
    let mut recovered = accepted_review();
    recovered["application"]["receiptRecovered"] = json!(true);
    let mut recovered_value = request_metadata(Vec::new());
    recovered_value["review"] = recovered;
    assert!(BRegRequestMetadata::from_value(recovered_value, false)
        .unwrap()
        .review()
        .unwrap()
        .application()
        .receipt_recovered());
    let mut queued = accepted_review();
    queued["application"]["state"] = json!("queued");
    queued["application"]
        .as_object_mut()
        .unwrap()
        .remove("applicationId");
    queued["application"]["nextAttemptAt"] = json!("2026-09-19T01:05:00Z");
    let mut queued_value = request_metadata(Vec::new());
    queued_value["review"] = queued;
    let queued = BRegRequestMetadata::from_value(queued_value, false).unwrap();
    assert_eq!(
        queued.review().unwrap().application().next_attempt_at(),
        Some("2026-09-19T01:05:00Z")
    );
    assert_eq!(
        review.recovery().state(),
        BRegExternalReviewRecoveryState::None
    );
    let debug = format!("{decoded:?} {queued:?}");
    for secret in [
        OTHER_ID,
        RESULT_ID,
        EVENT_ID,
...
fn external_review_status_refuses_partial_or_inconsistent_correlations() {
    ...
    cases.push(non_utc);
    let mut non_utc_deadline = accepted_review();
    non_utc_deadline["submission"]["recoveryDeadline"] = json!("2026-09-26T08:00:00+07:00");
    cases.push(non_utc_deadline);
    let mut time_without_attempts = accepted_review();
    time_without_attempts["application"]["state"] = json!("queued");
    time_without_attempts["application"]
        .as_object_mut()
        .unwrap()
        .remove("attempts");
    time_without_attempts["application"]
        ...
        .unwrap()
        .remove("applicationId");
    time_without_attempts["application"]["nextAttemptAt"] = json!("2026-09-19T01:05:00Z");
    cases.push(time_without_attempts);
    let mut terminal_with_next_attempt = accepted_review();
    terminal_with_next_attempt["application"]["nextAttemptAt"] = json!("2026-09-19T01:05:00Z");
    cases.push(terminal_with_next_attempt);
    let mut too_many_attempts = accepted_review();
    too_many_attempts["application"]["attempts"] = json!(1001);
    cases.push(too_many_attempts);
    let mut private_retry_payload = accepted_review();
    private_retry_payload["application"]["retryToken"] = json!("must-not-pass");
    cases.push(private_retry_payload);
    let mut false_recovery = accepted_review();
    false_recovery["application"]["receiptRecovered"] = json!(false);
    cases.push(false_recovery);
    let mut pending_recovery = accepted_review();
    pending_recovery["application"]["state"] = json!("queued");
    pending_recovery["application"]["receiptRecovered"] = json!(true);
    cases.push(pending_recovery);
    let mut manual_recovery = accepted_review();
    let manual_application = manual_recovery["application"].as_object_mut().unwrap();
    manual_application.insert("mode".to_owned(), json!("manual"));
    manual_application.remove("executor");
    manual_application.remove("attempts");
    manual_application.insert("receiptRecovered".to_owned(), json!(true));
    cases.push(manual_recovery);
    let mut unavailable_result = accepted_review();
    unavailable_result["result"] = json!({"state":"unavailable"});
    cases.push(unavailable_result);
    let mut invented_recovery = accepted_review();
    invented_recovery["recovery"] = json!({"state":"submissionUnknown"});
    cases.push(invented_recovery);
    for review in cases {
        let mut value = request_metadata(Vec::new());
        value["review"] = review;
        assert!(BRegRequestMetadata::from_value(value, false).is_err());
...
#[test]
fn external_review_diagnostics_may_be_absent_before_durable_work_exists() {
    let mut review = accepted_review();
    review["submission"]
        .as_object_mut()
        .unwrap()
        .remove("recoveryDeadline");
    let application = review["application"].as_object_mut().unwrap();
    application.remove("attempts");
    application.remove("nextAttemptAt");
    let mut value = request_metadata(Vec::new());
    value["review"] = review;

    let decoded = BRegRequestMetadata::from_value(value, false).unwrap();
    let review = decoded.review().unwrap();
    assert_eq!(review.submission().recovery_deadline(), None);
    assert_eq!(review.application().attempts(), None);
    assert_eq!(review.application().next_attempt_at(), None);
}

#[test]
fn external_review_submission_correlation_presence_follows_the_state() {
    // Cancelling and cancelled keep the correlation only while the accepted
    // binding survives; both forms remain decodable.
    for state in ["cancelling", "cancelled"] {
        for with_binding in [true, false] {
            let mut review = accepted_review();
            review["submission"]["state"] = json!(state);
            if !with_binding {
                let submission = review["submission"].as_object_mut().unwrap();
                submission.remove("requestId");
                submission.remove("submissionDigest");
                submission.remove("policy");
            }
            let mut value = request_metadata(Vec::new());
            value["review"] = review;
            assert!(BRegRequestMetadata::from_value(value, false).is_ok());
        }
    }

    // Only an accepted (or still-bound cancellation) submission can carry the
    // accepted binding's correlation members.
    for state in ["pending", "uncertain", "failed"] {
        let mut review = accepted_review();
        review["submission"]["state"] = json!(state);
        let mut value = request_metadata(Vec::new());
        value["review"] = review.clone();
        assert!(
            BRegRequestMetadata::from_value(value, false).is_err(),
            "{state} must not carry the accepted correlation members"
        );
        ...
        value["review"] = review;
        assert!(
            BRegRequestMetadata::from_value(value, false).is_ok(),
            "{state} without correlation members is the binding-less form"
        );
    }

    // An accepted submission without its correlation members cannot be
    // correlated later, so it must be refused outright.
    let mut review = accepted_review();
    let submission = review["submission"].as_object_mut().unwrap();
    submission.remove("requestId");
    submission.remove("submissionDigest");
...
fn accepted_review() -> Value {
    json!({
        "submission": {"state":"accepted", "authority":"casework", "requestId":OTHER_ID, "submissionDigest":DIGEST, "recoveryDeadline":"2026-09-26T01:00:00Z", "policy":{"id":"address-review", "version":"1", "digest":DIGEST}},
        ...
        "application": {"mode":"automatic", "state":"applied", "executor":"breg-worker", "applicationId":APPLICATION_ID, "attempts":3},
        "recovery": {"state":"none"},
    })
}
