...
/// tree from exhausting the stack. Writers that create a tree this tool must
/// also be able to remove refuse a depth the removal would not reach.
pub(crate) const MAX_REMOVE_TREE_DEPTH: u32 = 32;
...
mod descriptor {
    ...
    use super::{SafePathError, MAX_REMOVE_TREE_DEPTH};
...
mod tests {
    ...
    #[test]
    fn a_tree_deeper_than_the_removal_bound_is_refused_rather_than_partly_removed() {
        let tree = race_tree();
        let mut deepest = tree.root().join("genuine/staged");
        for level in 0..MAX_REMOVE_TREE_DEPTH {
            deepest = deepest.join(format!("d{level}"));
        }
        fs::create_dir_all(&deepest).expect("deep tree");
        let directory = SafeDir::resolve(&tree.root().join("genuine"))
            .expect("the directory resolves before the removal");

        let refused = directory
            .remove_tree(OsStr::new("staged"))
            .expect_err("a tree deeper than the bound is refused");

        assert!(refused
            .to_string()
            .contains("deeper than this tool removes"));
        assert!(tree.root().join("genuine/staged").exists());
    }
