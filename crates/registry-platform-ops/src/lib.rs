...
fn contains_high_confidence_credential_literal(summary: &str) -> bool {
    ["Bearer", "Basic"]
        .iter()
        .any(|scheme| contains_whole_authorization_value(summary, scheme))
...
fn contains_whole_authorization_value(summary: &str, scheme: &str) -> bool {
    let Some((candidate_scheme, credential)) = summary.split_once(' ') else {
        return false;
    };
