...
struct AuthoredOutputObjectDeclaration {
    #[serde(rename = "type")]
    output_type: AuthoredObjectSchemaType,
    max_bytes: u32,
    fields: BTreeMap<String, AuthoredOutputObjectField>,
}

impl AuthoredOutputObjectDeclaration {
...
struct AuthoredOutputArrayDeclaration {
    #[serde(rename = "type")]
    output_type: AuthoredArraySchemaType,
    max_bytes: u32,
    max_items: u16,
    items: Box<AuthoredOutputSchema>,
}

impl AuthoredOutputArrayDeclaration {
    const fn nullable(&self) -> bool {
        self.output_type.nullable()
    }
}
...
fn validate_authored_output(
    ...
    match output {
        AuthoredOutputDeclaration::Scalar(output) => {
            record_output_schema_node(&path, OUTPUT_SCHEMA_ROOT_DEPTH_V1, nodes)?;
        ...
        AuthoredOutputDeclaration::Object(output) => {
            validate_authored_output_object(&path, output, OUTPUT_SCHEMA_ROOT_DEPTH_V1, nodes)
        }
        AuthoredOutputDeclaration::Array(output) => {
            validate_authored_output_array(&path, output, OUTPUT_SCHEMA_ROOT_DEPTH_V1, nodes)
        }
    }
}
...
fn validate_authored_scalar_output(
    ...
    match scalar {
        AuthoredScalarType::String => {
            let max =
                max_length.ok_or_else(|| anyhow!("{path}.maxLength is required for String"))?;
            if max == 0 || max > 16_384 {
                bail!("{path}.maxLength is outside the v1 bounds");
            }
            if minimum.is_some() || maximum.is_some() {
                bail!("{path} String schemas cannot declare minimum or maximum");
            }
        ...
        AuthoredScalarType::Boolean => {
            if format.is_some()
                ...
                || maximum.is_some()
            {
                bail!("{path} Boolean schema has incompatible constraints");
            }
        }
        AuthoredScalarType::Integer => {
            let minimum =
                minimum.ok_or_else(|| anyhow!("{path}.minimum is required for Integer"))?;
            let maximum =
                maximum.ok_or_else(|| anyhow!("{path}.maximum is required for Integer"))?;
            if minimum > maximum
                ...
                || max_length.is_some()
            {
                bail!("{path} Integer schema has invalid constraints");
            }
        }
        AuthoredScalarType::Null => bail!("{path} cannot have only null type"),
    }
    Ok(())
}

fn record_output_schema_node(path: &str, depth: usize, nodes: &mut usize) -> Result<()> {
    *nodes = nodes
        .checked_add(1)
        .ok_or_else(|| anyhow!("{path} schema node count overflows"))?;
    if depth > MAX_OUTPUT_SCHEMA_DEPTH_V1 {
        bail!("{path} schema exceeds the maximum depth of {MAX_OUTPUT_SCHEMA_DEPTH_V1}");
    }
...
fn validate_authored_output_schema(
    ...
    match schema {
        AuthoredOutputSchema::Scalar(schema) => {
            record_output_schema_node(path, depth, nodes)?;
            validate_authored_scalar_output(
                path,
                &schema.output_type,
                schema.format,
        ...
        AuthoredOutputSchema::Object(schema) => {
            validate_authored_output_object(path, schema, depth, nodes)
        }
        AuthoredOutputSchema::Array(schema) => {
            validate_authored_output_array(path, schema, depth, nodes)
        }
    }
}
...
fn validate_authored_output_object(
    ...
    for (field_name, field) in &object.fields {
        ...
        let child = validate_authored_output_schema(&child_path, &field.schema, depth + 1, nodes)?;
        expanded = expanded
            .checked_add(child)
            .ok_or_else(|| anyhow!("{path} recursive expansion overflows"))?;
    }
...
fn validate_authored_output_array(
    ...
    let child =
        validate_authored_output_schema(&format!("{path}.items"), &array.items, depth + 1, nodes)?;
