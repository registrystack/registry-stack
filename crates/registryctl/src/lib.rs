    ...
    let body = if path.exists() {
        ...
        format!(
            "# Generated by registryctl. Docker Compose uses these values to run product\n\
             # containers as the project owner so private state directories stay writable.\n\
...
fn compose_runtime_identity(dir: &Path) -> Result<(String, String)> {
    ...
    Ok((metadata.uid().to_string(), metadata.gid().to_string()))
}
...
"#;
    ...
    fn assert_runtime_env_matches_project_owner(project: &Path) {
        ...
        #[cfg(unix)]
        {
            ...
            assert_eq!(uid, metadata.uid().to_string());
            assert_eq!(gid, metadata.gid().to_string());
        }
