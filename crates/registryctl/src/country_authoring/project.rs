...
fn validate_project_shape(project: &CountryProject) -> Result<()> {
    ...
    let mut country_claim_ids = BTreeSet::new();
    for (service_id, service) in &project.services {
        ...
        for (claim_id, claim) in &service.claims {
            ...
            if !country_claim_ids.insert(claim_id) {
        ...
        match (&loaded.document.capability, enablement) {
            (CapabilityDeclaration::SandboxedRhai { .. }, Some(enablement))
                if enablement.enabled
                    ...
                    && is_authoring_capability_released(
                        ReleasedAuthoringCapability::SandboxedRhaiV1,
                    ) => {}
            (CapabilityDeclaration::SandboxedRhai { .. }, _) => {
                bail!("sandboxed_rhai requires a released authoring contract and explicit operator-security enablement")
            }
            (_, None) => {}
...
fn is_authoring_capability_released(capability: ReleasedAuthoringCapability) -> bool {
    is_authoring_capability_released_in(capability, RELEASED_AUTHORING_CAPABILITIES)
}

fn is_authoring_capability_released_in(
    capability: ReleasedAuthoringCapability,
    released: &[ReleasedAuthoringCapability],
) -> bool {
