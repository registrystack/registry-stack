...
mod tests {
    ...
    fn run_code_owned_country_conformance(project: &Path) -> Result<Vec<FixtureReport>> {
        ...
        run_code_owned_loaded_country_conformance(&loaded)
    }

    fn run_code_owned_loaded_country_conformance(
    ...
    fn code_owned_rhai_conformance_matches_bounded_http_and_is_deterministic() {
        ...
        let mut unknown_product =
            load_country_project(&rhai_project, None).expect("Rhai golden loads");
        unknown_product
            .integrations
            ...
            .document
            .source
            .product = "previously-unknown-country-system".to_string();
        let unknown_product_report = run_code_owned_loaded_country_conformance(&unknown_product)
    ...
    fn released_rhai_capability_identity_is_not_a_source_product() {
        assert!(is_authoring_capability_released(
            ReleasedAuthoringCapability::SandboxedRhaiV1
        ));
        assert!(!is_authoring_capability_released_in(
            ReleasedAuthoringCapability::SandboxedRhaiV1,
            &[]
        ));
    }
    ...
    fn live_request_resolves_claims_across_services_with_the_same_purpose() {
        let project = country_golden("custom-system");
        let mut loaded = load_country_project(&project, None).expect("golden project loads");
    ...
    #[test]
    fn duplicate_country_claim_ids_fail_before_generation() {
        let project = country_golden("custom-system");
        let mut loaded = load_country_project(&project, None).expect("golden project loads");
        ...
        let error = validate_project_shape(&loaded.project)
            .expect_err("duplicate country claim ids must fail closed");
        assert!(error
            .to_string()
            .contains("claim ids must be unique across country services"));
    }

    #[test]
    fn disclosure_review_classes_are_directional() {
        let loaded = load_country_project(&country_golden("custom-system"), None)
    ...
    fn signed_review_record_validation_rejects_legacy_and_inconsistent_evidence() {
        let loaded = load_country_project(&country_golden("custom-system"), Some("local"))
            .expect("golden project loads");
        let review = compile_country(&loaded, None)
