...
fn musl_system_refuses_before_download() {
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("No musl build of the Evidence toolset is published"),
        "stderr: {stderr}"
    );
...
#[cfg(unix)]
#[test]
fn macos_install_does_not_consult_the_linux_libc_preflight() {
    let fixture = InstallerFixture::new();
    let mut command = fixture.command();
    command
        .env("FAKE_UNAME_S", "Darwin")
        .env("FAKE_UNAME_M", "arm64")
        .env("FAKE_LIBC_MUSL", "1");
    let output = command.output().unwrap();
    // The fixture publishes Linux assets only, so a macOS run stops at the
    // download. Reaching that point is the proof the preflight stayed silent.
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(!stderr.contains("musl"), "stderr: {stderr}");
    assert!(
        stderr.contains("Could not read the published"),
        "stderr: {stderr}"
    );
}
