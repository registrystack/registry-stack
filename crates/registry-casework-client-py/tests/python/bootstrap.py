"""Build and import the Casework PyO3 module for stdlib-only tests."""
...
_MODULE_NAME = "registry_casework_client"
_TARGET_DEBUG = _WORKSPACE_ROOT / "target" / "debug"
_IMPORT_ROOT = _TARGET_DEBUG / "casework_python_module"
...
def _library() -> pathlib.Path:
    ...
    if suffix is None:
        raise RuntimeError("the Casework Python tests support macOS and Linux")
...
def ensure_built() -> None:
    subprocess.run(
        [
            ...
            "--lib",
            "--features",
            "registry-casework-client-py/extension-module",
        ],
        cwd=_WORKSPACE_ROOT,
        check=True,
    )
