...
use crate::model::{
    CompiledAccessInventory, CompiledEntity, CompiledQueryInventory, CompiledQueryKind,
...
fn signed_predecessor_governed_model(
    ...
    let legacy_temporal_scopes = fill_legacy_predecessor_effective_model(&mut value)?;
    ...
    fill_legacy_predecessor_temporal_value_kinds_in_operations(
        &entities_value(&value)?,
        &mut query_value,
    )?;
    let queries: CompiledQueryInventory =
        serde_json::from_value(query_value).map_err(|_| PackageError::Derivation)?;

    if effective_physical_names != physical_names || effective_queries != queries {
        return Err(PackageError::Derivation);
    }
...
fn fill_legacy_predecessor_effective_model(
    value: &mut Value,
) -> Result<BTreeMap<String, Vec<String>>> {
    let legacy_temporal_scopes = normalize_legacy_predecessor_temporal_scope_fields(value)?;
    let entities = entities_value(value)?;
    let query_inventory = value
        .get_mut("queryInventory")
        .ok_or(PackageError::Derivation)?;
    fill_legacy_predecessor_temporal_value_kinds_in_operations(&entities, query_inventory)?;
    Ok(legacy_temporal_scopes)
}

fn entities_value(value: &Value) -> Result<Value> {
    value
        .get("entities")
        .cloned()
        .ok_or(PackageError::Derivation)
...
fn normalize_legacy_predecessor_temporal_scope_fields(
    value: &mut Value,
) -> Result<BTreeMap<String, Vec<String>>> {
    ...
    for (entity_id, entity) in entities {
        let entity_object = entity.as_object_mut().ok_or(PackageError::Derivation)?;
        let scope_fields = {
            let Some(temporal_value) = entity_object.get("temporal") else {
                continue;
            };
            let temporal = temporal_value.as_object().ok_or(PackageError::Derivation)?;
            let scope_fields = temporal_scope_fields(temporal.get("scopeFields"))?;
            if !scope_fields.is_empty() {
                validate_matching_temporal_non_overlap_constraint(
                    entity_object,
                    temporal,
                    &scope_fields,
                )?;
        ...
        entity_object
            ...
            .ok_or(PackageError::Derivation)?
            .remove("scopeFields");
        entity_scopes.insert(entity_id.clone(), scope_fields);
    }

    let query_inventory = value
        .get_mut("queryInventory")
        .ok_or(PackageError::Derivation)?;
    normalize_legacy_predecessor_query_temporal_scope_fields(&entity_scopes, query_inventory)?;
    Ok(entity_scopes)
}

fn normalize_legacy_predecessor_query_temporal_scope_fields(
    entity_scopes: &BTreeMap<String, Vec<String>>,
    query_inventory: &mut Value,
) -> Result<()> {
    let query_inventory = query_inventory
        .as_object_mut()
        .ok_or(PackageError::Derivation)?;
    for operation in query_inventory
        ...
        .ok_or(PackageError::Derivation)?
    {
        let operation = operation.as_object_mut().ok_or(PackageError::Derivation)?;
        if !operation.contains_key("temporal") {
            continue;
        }
        let entity_id = operation
            ...
            .ok_or(PackageError::Derivation)?
            .to_owned();
        let expected = entity_scopes
            .get(&entity_id)
            .ok_or(PackageError::Derivation)?;
        let temporal = operation
            .get_mut("temporal")
            .and_then(Value::as_object_mut)
            .ok_or(PackageError::Derivation)?;
        let query_scopes = temporal_scope_fields(temporal.get("scopeFields"))?;
        if &query_scopes != expected {
            return Err(PackageError::Derivation);
        }
        temporal.remove("scopeFields");
    }
    Ok(())
}

fn validate_matching_temporal_non_overlap_constraint(
    entity: &serde_json::Map<String, Value>,
    temporal: &serde_json::Map<String, Value>,
    scope_fields: &[String],
) -> Result<()> {
    let start_field = temporal
        .get("startField")
        .and_then(Value::as_str)
        .ok_or(PackageError::Derivation)?;
    let end_field = temporal
        .get("endField")
        .and_then(Value::as_str)
        .ok_or(PackageError::Derivation)?;
    let constraints = entity
        ...
        .ok_or(PackageError::Derivation)?;
    let matched = constraints.values().any(|constraint| {
        let Some(constraint) = constraint.as_object() else {
            return false;
        };
        constraint.get("kind").and_then(Value::as_str) == Some("temporal-non-overlap")
            && constraint.get("startField").and_then(Value::as_str) == Some(start_field)
            && constraint.get("endField").and_then(Value::as_str) == Some(end_field)
            && temporal_scope_fields(constraint.get("scopeFields"))
                .is_ok_and(|constraint_scopes| constraint_scopes == scope_fields)
    });
    if matched {
        Ok(())
    } else {
...
fn temporal_scope_fields(value: Option<&Value>) -> Result<Vec<String>> {
    let Some(value) = value else {
        return Ok(Vec::new());
    };
    let fields = value.as_array().ok_or(PackageError::Derivation)?;
    let mut result: Vec<String> = Vec::with_capacity(fields.len());
    for field in fields {
        let field = field.as_str().ok_or(PackageError::Derivation)?;
        if field.is_empty() || result.last().is_some_and(|prior| prior.as_str() >= field) {
            return Err(PackageError::Derivation);
        }
...
fn fill_legacy_predecessor_temporal_value_kinds_in_operations(
    entities: &Value,
    query_inventory: &mut Value,
) -> Result<()> {
    let entities = entities.as_object().ok_or(PackageError::Derivation)?;
    let query_operations = query_inventory
        .get_mut("operations")
        .and_then(Value::as_array_mut)
        .ok_or(PackageError::Derivation)?;

    for operation in query_operations {
        let Some(operation_object) = operation.as_object_mut() else {
            return Err(PackageError::Derivation);
        };
        if !operation_object.contains_key("temporal") {
            continue;
        }
        let entity_id = operation_object
            .get("entityId")
            .and_then(Value::as_str)
            .ok_or(PackageError::Derivation)?
            .to_owned();
        let temporal_object = operation_object
            .get_mut("temporal")
            .and_then(Value::as_object_mut)
            .ok_or(PackageError::Derivation)?;
        if temporal_object.contains_key("valueKind") {
            continue;
        }
        let entity = entities.get(&entity_id).ok_or(PackageError::Derivation)?;
        let start_field = temporal_object
            .get("startField")
            .and_then(Value::as_str)
            .ok_or(PackageError::Derivation)?;
        let end_field = temporal_object
            .get("endField")
            .and_then(Value::as_str)
            .ok_or(PackageError::Derivation)?;
        let value_kind = derive_temporal_value_kind_from_entity(entity, start_field, end_field)?;
        temporal_object.insert("valueKind".to_owned(), Value::String(value_kind.to_owned()));
    }
    Ok(())
}

fn derive_temporal_value_kind_from_entity(
    entity: &Value,
    start_field: &str,
    end_field: &str,
) -> Result<&'static str> {
    let fields = entity
        ...
        .and_then(Value::as_object)
        .ok_or(PackageError::Derivation)?;
    let start_kind = temporal_field_type(fields.get(start_field).ok_or(PackageError::Derivation)?)?;
    let end_kind = temporal_field_type(fields.get(end_field).ok_or(PackageError::Derivation)?)?;
...
fn temporal_field_type(field: &Value) -> Result<&'static str> {
    match field
        .get("fieldType")
        .and_then(Value::as_object)
        .and_then(|field_type| field_type.get("type"))
        .and_then(Value::as_str)
    {
        Some("date") => Ok("date"),
