...
binaries=(registry-server registry-serverctl)
# Publication packaging replaces this empty value with the asset's canonical tag.
default_version=""
script_name="${BASH_SOURCE[0]:-}"
script_name="${script_name##*/}"
filename_version=""
if [[ "$script_name" =~ ^registry-server-(v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*))-install\.sh$ ]]; then
...
version="${REGISTRY_SERVER_VERSION:-$default_version}"
if [ -n "$default_version" ] &&
	[ -n "${REGISTRY_SERVER_VERSION:-}" ] &&
	[ "$REGISTRY_SERVER_VERSION" != "$default_version" ]; then
	echo "Refusing a release override that does not match the released installer asset." >&2
	exit 1
fi
install_dir="${REGISTRY_SERVER_INSTALL_DIR:-$HOME/.local/bin}"
asset_dir="${REGISTRY_SERVER_ASSET_DIR:-}"

usage() {
	cat <<EOF
Install the Registry Server runtime and registry-serverctl adopter tooling.

Quick install:
  curl -fsSL https://github.com/${repo}/releases/latest/download/registry-server-install.sh | bash
...
pinned tag, then rerun with REGISTRY_SERVER_ASSET_DIR set to the verified
directory:
  https://github.com/${repo}/blob/<version>/release/VERIFY.md

Environment:
  REGISTRY_SERVER_VERSION      Registry Server tag to install. A published
                               installer embeds its tag and refuses a different
                               override.
  REGISTRY_SERVER_INSTALL_DIR  Install directory. Defaults to ~/.local/bin.
  REGISTRY_SERVER_ASSET_DIR    Read already-downloaded release assets from this
...
need() {
	if ! command -v "$1" >/dev/null 2>&1; then
		echo "registry-server installer needs '$1'." >&2
		exit 1
	fi
}

if [ -z "$version" ]; then
	echo "No Registry Server tag is pinned for this installer copy." >&2
	echo "Set REGISTRY_SERVER_VERSION to a pinned vMAJOR.MINOR.PATCH tag, or run a" >&2
	echo "published registry-server-<tag>-install.sh asset." >&2
	exit 1
fi
if [[ ! "$version" =~ ^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$ ]]; then
	echo "Refusing a non-canonical Registry Server tag." >&2
...
Darwin/arm64 | Darwin/aarch64)
	...
	;;
*)
	printf 'No prebuilt Registry Server asset is published for %s/%s.\n' "$os" "$arch" >&2
...
tmpdir="$(mktemp -d 2>/dev/null || mktemp -d -t registry-server)"
...
if [ -n "$asset_dir" ]; then
	printf 'Installing verified local Registry Server %s assets for %s/%s...\n' \
		"$version" "$os_label" "$arch_label"
else
	printf 'Downloading Registry Server %s for %s/%s...\n' \
		"$version" "$os_label" "$arch_label"
fi
...
sha256_file() {
	...
	if command -v shasum >/dev/null 2>&1; then
		...
		echo "registry-server installer needs 'shasum' or 'sha256sum' for checksum verification." >&2
		exit 1
	fi
	printf '%s\n' "${result%% *}"
}
...
guide first, then rerun this installer with REGISTRY_SERVER_ASSET_DIR set to
...
mkdir -p "$install_dir"
stage_dir="$(mktemp -d "$install_dir/.registry-server-install.XXXXXX")"
install_started=0
install_complete=0
rollback_install() {
	set +e
...
# interrupted update never leaves a mixed-version Registry Server toolset.
install_started=1
for binary in "${binaries[@]}"; do
...
Try it:
  registry-serverctl init --help
  registry-serverctl check --help
  registry-server --help

EOF

case ":$PATH:" in
*":$install_dir:"*) ;;
*) echo "Add $install_dir to PATH to run Registry Server from any shell." >&2 ;;
esac
