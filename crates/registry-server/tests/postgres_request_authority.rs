...
async fn update_target_site(
    transaction: &registry_server::postgres::GuardedTransaction<'_>,
    placement_table: &str,
    placement_site: &str,
) -> u64 {
    transaction
...
fn claims(
    ...
    let row_boundaries = tenant
        .map(|value| {
            vec![registry_server::postgres::RowBoundaryContext::Equals {
...
fn placement_claims(registry: &registry_server::CompiledRegistry, profile: &str) -> ClaimContext {
    ClaimContext::for_compiled(
        registry,
        "asset-placement",
        Some(profile.to_owned()),
