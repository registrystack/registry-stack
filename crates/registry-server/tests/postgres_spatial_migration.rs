...
async fn first_compiled_statement_error(
    ...
    for statement in registry.ddl().statements.iter().filter(|statement| {
        statement.kind != registry_server::generated_ddl::DdlStatementKind::Schema
    }) {
...
async fn runtime_postgis_steps(database: &TestDatabase) -> String {
    ...
    let bbox_role = registry_server::postgres::spatial_bbox_role(&database.runtime_role);
        ...
        .query(
            "SELECT schemaname, viewname
              ...
              WHERE schemaname IN ('registry_derived', 'registry_source')
                 OR (schemaname = 'registry_context' AND viewname LIKE 'rs_spcand\\_%' ESCAPE '\\')
...
async fn create_candidate_views_for_fingerprint(
    ...
    for statement in candidate.ddl().statements.iter().filter(|statement| {
        ...
        if statement
            .sql
            .starts_with("CREATE VIEW registry_context.\"rs_spcand_")
        {
            let bbox_role = registry_server::postgres::spatial_bbox_role(runtime_role);
    ...
    for view in &candidate.ddl().views {
        if view.schema == "registry_context" && view.name.starts_with("rs_spcand_") {
            continue;
        }
...
fn synthetic_backup_sql(
    ...
    let site_views = registry
        ...
        .filter(|view| !(view.schema == "registry_context" && view.name.starts_with("rs_spcand_")))
        .collect::<Vec<_>>();
...
async fn cleanup_bbox_role(database: &TestDatabase, bbox_role: &SqlIdentifier) {
    database
        .admin
        .batch_execute(&format!(
            "DO $registry_server_spatial_cleanup$\n\
             DECLARE\n\
                 policy record;\n\
                 role_oid oid;\n\
             BEGIN\n\
                 ...
                 END LOOP;\n\
             END;\n\
             $registry_server_spatial_cleanup$;\n\
             DROP OWNED BY {};\n\
