...
async fn real_postgres_batch_is_bounded_authorized_atomic_and_exactly_replayable() {
    ...
    for (label, uri, changed_claims, changed_body) in [
        (
            ...
            json!({"items":[
                ...
                {"operation":"patch", "recordId":seed_id, "ifMatch":"\"rs-different\"", "patch":[
                    {"op":"replace", "path":"/data/label", "value":"batch-patched"}
                ]}
            ]}),
        ),
    ...
    let invalid_later = json!({"items": [
        ...
        {"operation":"patch", "recordId":seed_id, "ifMatch":"\"rs-stale\"", "patch":[
    ...
    for (key, body) in [
        ...
        (
            "invalid-uuid",
            json!({"items":[{"operation":"patch","recordId":"NOT-A-UUID","ifMatch":"\"rs-etag\"","patch":[
                {"op":"replace","path":"/data/label","value":"never"}
            ]}]}),
        ),
        (
    ...
    let patch_without_grant = send_json(
        ...
        Some(authorized_claims.clone()),
        "create-only-profile",
        json!({"items":[{"operation":"patch","recordId":seed_id,"ifMatch":"\"rs-stale\"","patch":[
            {"op":"replace","path":"/data/label","value":"never"}
        ]}]}),
    )
    .await;
