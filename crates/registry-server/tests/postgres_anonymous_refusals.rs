...
use registry_server::contract::parse_project_yaml;
...
use registry_server::startup::with_request_timeout_and_metrics_for_test;
...
async fn anonymous_refusals_are_counted_while_principal_refusals_stay_chained() {
    ...
    assert!(
        body.contains("# TYPE registry_server_anonymous_refusals_total counter\n"),
        "the anonymous refusal counter is exposed: {body}"
    );
    assert!(
        body.contains(
            "registry_server_anonymous_refusals_total{route=\"/v1/records/cases\",method=\"GET\",reason=\"read_concealed\"} 3\n"
        ),
    ...
    assert!(
        body.contains(
            "registry_server_anonymous_refusals_total{route=\"/v1/records/cases\",method=\"GET\",reason=\"read_refused\"} 1\n"
        ),
