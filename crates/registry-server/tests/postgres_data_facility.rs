...
const PROFILE: &str = "facility-operator";

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn real_postgres_facility_import_is_authenticated_chunked_resumable_and_race_safe() {
    ...
    let facility_id = create(
        &harness,
        &north_token,
        "/v1/records/facilities",
        "data-facility-seed",
        json!({
            "facilityCode":"F-DATA", "displayName":"Data import operator",
            "administrativeBoundary":"north-district"
        }),
    )
    .await;
    let permit_id = create(
        &harness,
        &north_token,
        "/v1/records/permits",
        "data-permit-seed",
        json!({
            "permitNumber":"H-DATA", "facility":facility_id, "permitType":"water-discharge",
            "validFrom":"2026-01-01", "administrativeBoundary":"north-district",
            "importSource":"data-seed", "sourceRecordId":"permit"
        }),
    )
    .await;
    ...
    let input = (0..5)
        .map(|index| {
            serde_json::to_string(&installation_item(
                &permit_id,
            ...
            .expect("facility import item serializes")
        })
        .collect::<Vec<_>>()
        .join("\n")
        + "\n";
    ...
    .expect("facility import validates against the compiled model");
    assert_eq!(
        plan.chunks().len(),
        2,
    ...
    let unauthorized_input = serde_json::to_string(&installation_item(
        &permit_id,
        "P-CONCEALED-CANARY",
...
fn one_item_plan(
    ...
    let input = serde_json::to_string(&installation_item(
        permit_id,
        installation_code,
        source,
        source_record_id,
...
fn installation_item(
    ...
    json!({"operation":"create", "data":{
        "installationCode":installation_code, "permit":permit_id,
        "administrativeBoundary":"north-district",
...
async fn effect_counts(harness: &PilotHarness) -> Counts {
    let table = &harness.registry.entities()["installation"].physical_table;
    let row = harness
        .database
        .admin
        .query_one(
...
async fn race_key_count(harness: &PilotHarness) -> i64 {
    let entity = &harness.registry.entities()["installation"];
