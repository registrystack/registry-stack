...
fn rhai_planner_authoring_refuses_closed_contract_violations() {
    ...
    assert!(compile_codes(&both, vec![owned_asset()]).contains("change_request.plan.exclusive"));
    ...
    assert!(compile_codes(&wrong_abi, vec![owned_asset()])
        .contains("change_request.planner.abi_invalid"));
    ...
    assert!(compile_codes(&unknown_request_field, vec![owned_asset()])
        .contains("change_request.planner.request_field_unknown"));
    ...
    unknown_write_field["entities"][1]["changeRequest"]["planner"]["writes"][0]["fields"]
        ...
        .push(json!("ambient-field"));
    assert!(compile_codes(&unknown_write_field, vec![owned_asset()])
        .contains("change_request.planner.write_field_unknown"));
