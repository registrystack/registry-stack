...
use registry_server::postgres::{ConnectionConfig, PoolBounds};
...
async fn custom_ca_requires_a_trusted_tls_server() {
    let database_url = required_env("REGISTRY_SERVER_TEST_TLS_DATABASE_URL");
    let hostname_mismatch_url =
        required_env("REGISTRY_SERVER_TEST_TLS_HOSTNAME_MISMATCH_DATABASE_URL");
    let trusted_ca = read_required_der("REGISTRY_SERVER_TEST_TLS_CA_DER_PATH");
    let wrong_ca = read_required_der("REGISTRY_SERVER_TEST_TLS_WRONG_CA_DER_PATH");
    assert_ne!(
        trusted_ca, wrong_ca,
