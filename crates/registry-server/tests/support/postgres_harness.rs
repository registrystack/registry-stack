...
impl TestDatabase {
    pub async fn create(pool_size: usize) -> Self {
        let url = env::var("REGISTRY_SERVER_TEST_DATABASE_URL").expect(
            "REGISTRY_SERVER_TEST_DATABASE_URL is required for the real PostgreSQL kernel test",
        );
        let admin_root = Config::from_str(&url)
            .expect("REGISTRY_SERVER_TEST_DATABASE_URL must be a valid PostgreSQL URL");
        let suffix = unique_suffix();
        let database = SqlIdentifier::parse(&format!("rs_test_{suffix}"))
            .expect("generated database identifier is valid");
        let migration_role = SqlIdentifier::parse(&format!("rs_migration_{suffix}"))
            .expect("generated migration role identifier is valid");
        let runtime_role = SqlIdentifier::parse(&format!("rs_runtime_{suffix}"))
            .expect("generated runtime role identifier is valid");
        let intruder_role = SqlIdentifier::parse(&format!("rs_intruder_{suffix}"))
