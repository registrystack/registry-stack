//! Bounded resolution of runtime secret references.
...
/// The maximum size of a resolved secret value.
...
impl ProtectedSecret {
    /// Explicitly borrow the secret for the smallest possible consumer scope.
...
/// Resolves only the providers explicitly enabled by runtime configuration.
#[derive(Debug)]
...
mod tests {
    ...
    fn environment_secret_is_bounded_and_debug_is_redacted() {
        ...
        const NAME: &str = "REGISTRY_EVIDENCE_SECRET_RESOLVER_TEST";
        env::set_var(NAME, "environment-canary");
        let resolver =
            SecretResolver::new([SecretProvider::Environment], "").expect("resolver builds");
        let secret = resolver
            .resolve("secret:env/REGISTRY_EVIDENCE_SECRET_RESOLVER_TEST")
            .expect("secret resolves");
        env::remove_var(NAME);

        assert!(
            secret.expose_secret() == b"environment-canary",
            "resolved environment secret bytes differ"
        );
    ...
    mod unix {
        ...
        fn file_secret_uses_open_file_owner_and_exact_mode_checks() {
            ...
            assert!(
                secret.expose_secret() == b"file-canary",
                "resolved file secret bytes differ"
            );
