...
async fn unrelated_package_activation_preserves_pending_request_application() {
    ...
    .await;
    assert_eq!(
        erased_request.body["data"]["request"]["bregState"],
        "applied"
    );
    ...
    assert_eq!(retained["bregState"], "applied");
...
async fn prepare_successor_package(
    database: &TestDatabase,
    prior: &Arc<registry_breg::CompiledRegistry>,
    active: &ExpectedRegistryIdentity,
    candidate: &Arc<registry_breg::CompiledRegistry>,
) -> PublishedPackage {
...
fn request_contract_fingerprint(registry: &registry_breg::CompiledRegistry) -> String {
