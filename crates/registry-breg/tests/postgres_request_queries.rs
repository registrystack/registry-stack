...
async fn real_postgres_request_queues_filter_count_and_page_on_server_owned_state() {
    ...
    let submitted_page = response_parts(
        send(
            &app,
            Method::GET,
            "/v1/records/correction-requests?accessProfile=reviewer&$select=reason&$filter=bregState%20eq%20'submitted'&$orderby=proposalVersion&$top=1&$count=true",
            Some(reviewer.clone()),
            &[],
            Vec::new(),
        )
    ...
    assert_eq!(submitted_items[0]["request"]["bregState"], "submitted");
    assert_ne!(submitted_items[0]["recordIdentifier"], draft_request.id);
    assert!(submitted_items[0]["domainData"].get("bregState").is_none());
    ...
    assert_eq!(continuation_items[0]["request"]["bregState"], "submitted");
    assert_ne!(
    ...
    assert_eq!(
        draft_count.body["items"][0]["request"]["bregState"],
        "draft"
    );
    ...
    let selected_breg_state = response_parts(
        send(
            &app,
            Method::GET,
            "/v1/records/correction-requests?accessProfile=reviewer&$select=bregState",
            Some(reviewer.clone()),
            &[],
            Vec::new(),
        )
    ...
    .await;
    assert_eq!(selected_breg_state.status, StatusCode::BAD_REQUEST);
    assert_eq!(selected_breg_state.body["code"], "query.invalid");

    let target_state_filter = response_parts(
        send(
            &app,
            Method::GET,
            "/v1/records/placements?accessProfile=steward&$filter=bregState%20eq%20'submitted'&$count=true",
            Some(steward),
            &[],
            Vec::new(),
        )
...
fn request_query_router(
    ...
    let audit = registry_breg::audit::test_support::capturing(
        AuditProfile::production_from_secret_bytes(vec![0x4d; 32].into())
