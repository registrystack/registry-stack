...
fn compiled_webhooks(destinations: &[(&str, u32, u8)]) -> registry_breg::CompiledRegistry {
...
fn strict_runtime_file_loads_and_constructs_existing_runtime_inputs() {
    ...
    assert_eq!(verifier.audiences, vec!["urn:breg:test"]);
    assert_eq!(
    ...
    std::env::remove_var("BREG_RUNTIME_CONFIG_DATABASE_URL");
...
fn runtime_config_errors_expose_stable_value_free_metadata() {
    ...
    for (error, code, path) in cases {
        ...
        for canary in [
            ...
            AUDIT_KEY_CANARY,
            "BREG_RUNTIME_CONFIG_DATABASE_URL",
            "registry_runtime",
...
fn jwks_source_is_a_strict_tagged_oidc_member() {
    ...
    parse_runtime_config_with_env(
        &runtime_with_static_jwks_ref(&fixture, "secret:env/BREG_RUNTIME_CONFIG_JWKS"),
        env_lookup,
    )
...
fn env_lookup(name: &str) -> Option<String> {
    match name {
        "BREG_RUNTIME_CONFIG_DATABASE_URL" => Some(DATABASE_URL_CANARY.to_owned()),
        "BREG_RUNTIME_CONFIG_MIGRATION_DATABASE_URL" => {
...
impl RuntimeFixture {
    fn new() -> Self {
        ...
        let root = std::env::temp_dir()
            ...
            .join(format!(
                "breg-runtime-config-test-{suffix}-{}-{counter}",
                std::process::id()
            ));
