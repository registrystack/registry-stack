...
async fn restore_row_level_security(client: &Client, registry: &registry_breg::CompiledRegistry) {
...
async fn seed_domain_rows(client: &Client, registry: &registry_breg::CompiledRegistry) {
    client
        .execute(
            "SELECT set_config('registry.active_package_revision', 'package-1', false)",
            &[],
        ...
        .query(
            "SELECT member.entity_id, member.record_id, member.record_revision
              ...
              WHERE revision_commit.origin_kind = 'migration'
                AND revision_commit.system_origin = 'breg-request-retention-erasure-v1'
...
fn request_fingerprint(registry: &registry_breg::CompiledRegistry) -> String {
