...
use registry_breg::request_events::{insert_request_lifecycle_events, RequestLifecycleEvent};
use registry_breg::runtime_config::parse_runtime_config;
use registry_breg::webhook::{WebhookDeliveryService, WebhookWorkOutcome};
...
async fn authenticated_webhook_service_event_material_does_not_grant_request_action_authority() {
    ...
    .await;
    assert_eq!(submitter_view.body["data"]["request"]["bregState"], "draft");
    assert_eq!(
...
authentication:
  oidc:
    ...
    audience: urn:breg:request-events
