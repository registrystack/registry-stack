...
use registry_breg::contract::{parse_module_json, parse_project_json, ModuleAssetSource};
...
impl SpatialHarness {
    async fn create(compiled: registry_breg::CompiledRegistry) -> Self {
    ...
    assert!(
        nodes.iter().any(|node| {
            node["spatialIndexCondition"] == true
                && node["indexName"]
                    .as_str()
                    .is_some_and(|name| name.contains("breg_spgix_"))
        }),
...
fn compiled_plain_geojson_registry() -> registry_breg::CompiledRegistry {
