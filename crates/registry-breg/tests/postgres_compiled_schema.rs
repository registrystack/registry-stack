...
async fn vocabulary_code_addition_replaces_the_check_and_keeps_existing_rows() {
    ...
    let candidate = vocabulary_catalog_registry(&["open", "closed", "archived"]);
    let change_set = compiled_registry_change_set(&previous, &candidate, PRIOR_REVISION);
    ...
    // the column constraint alone.
    upgraded
        .admin
        .execute(&insert, &[&RECORD_ALPHA, &"closed", &"minor"])
        .await
        .expect("a row holding a previous code is stored");
    assert!(
        upgraded
            .admin
            .execute(&insert, &[&RECORD_BETA, &"archived", &"minor"])
            .await
            .is_err(),
        "the previous check refuses the code the candidate adds"
    );
    for statement in &plan.statements {
        upgraded_migration
            .batch_execute(&statement.sql)
    ...
    upgraded
        ...
        .execute(&insert, &[&RECORD_BETA, &"archived", &"minor"])
        .await
        .expect("the replaced check accepts the added code");
    for (record, status_code, kind_code, reason) in [
        (
        ...
        (
            ...
            "open",
            "archived",
            "the unchanged field keeps its own check",
        ),
    ] {
        assert!(
            upgraded
                .admin
                .execute(&insert, &[&record, &status_code, &kind_code])
                .await
                .is_err(),
            "{reason}"
        );
    ...
    let kept: Vec<(String, String)> = upgraded
        ...
        .query(
            &format!(
                "SELECT record_id::text, {status} FROM registry_data.{table} ORDER BY record_id"
            ),
            &[],
        )
        .await
    ...
    assert_eq!(
        kept,
        [
            (RECORD_ALPHA.to_owned(), "closed".to_owned()),
            (RECORD_BETA.to_owned(), "archived".to_owned()),
        ]
    );
    let candidate_catalog = ExpectedManagedCatalog::compiled(&candidate);
    let upgraded_fingerprint = managed_schema_fingerprint(
...
async fn insert_row(
    ...
    entity: &registry_breg::model::CompiledEntity,
    record_id: &str,
    tenant_value: &str,
) {
    let table = quote_identifier(&entity.physical_table);
...
fn additive_catalog_registry(variant: AdditiveCatalogVariant) -> registry_breg::CompiledRegistry {
...
fn vocabulary_catalog_registry(status_values: &[&str]) -> registry_breg::CompiledRegistry {
    let project = serde_json::json!({
        ...
        "vocabularies": [
            {"id": "status", "values": status_values},
            {"id": "kind", "values": ["minor", "major"]}
        ],
        "entities": [{
            "id": "entry",
            "primaryDataset": "test-dataset",
            ...
            "mutationMode": "mutable",
            "fields": [
                {"id": "status", "type": "vocabulary-code", "vocabulary": "status",
    ...
    let project_bytes = serde_json::to_vec(&project).expect("fixture serializes");
    let project = parse_project_json(&project_bytes).expect("vocabulary catalog fixture parses");
    compile_project(&project, &[], CompileProfile::Authoring)
        .expect("vocabulary catalog fixture compiles")
}
