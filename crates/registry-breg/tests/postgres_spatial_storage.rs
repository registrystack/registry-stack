...
use registry_breg::generated_ddl::{
...
fn bbox_grant_derives_postgis_projection_policy_and_inventory() {
    ...
    assert!(registry.ddl().statements.iter().any(|statement| {
        statement.kind == DdlStatementKind::Index
            ...
            && statement.sql.contains("WHERE \"breg_spgeom_")
    }));
    ...
    let source_view_sql = registry
        ...
        .sql
        .as_str();
    assert!(!source_view_sql.contains("breg_spgeom_"));
...
fn compiled_spatial_registry() -> registry_breg::CompiledRegistry {
    compile_project(
        &parse_project_json(
            br#"{
...
fn crs84_point_without_bbox_keeps_non_gis_ddl_and_inventory_stable() {
    ...
    assert!(!script.contains("breg_spgeom_"));
    assert!(registry
        .ddl()
        .tables
        .iter()
...
mod live_postgres {
    ...
    async fn real_postgres_exact_jsonb_residual_rejects_double_rounding_bbox_admission() {
        ...
        .await;
        assert!(
            plan.contains("Index Cond") && plan.contains("breg_spgeom_"),
    ...
    async fn real_postgres_forced_rls_bbox_policy_uses_gist_for_representative_volume() {
        ...
        .await;
        assert!(
            plan.contains("Index Cond") && plan.contains("breg_spgeom_") && plan.contains("gist"),
    ...
    async fn assert_verify_postgis_rejects_role_and_schema_drift(
        ...
        let upstream = unique_role("breg_spatial_parent");
        database
            .admin
            .batch_execute(&format!(
    ...
    fn claims(registry: &registry_breg::CompiledRegistry) -> ClaimContext {
        claims_for(registry, "site")
    }

    fn claims_for(registry: &registry_breg::CompiledRegistry, entity_id: &str) -> ClaimContext {
    ...
    fn spatial_geometry_column(registry: &registry_breg::CompiledRegistry) -> String {
        let sql = registry
            .ddl()
            .statements
            .iter()
            .find(|statement| statement.sql.contains("\"breg_spgeom_"))
            .expect("spatial geometry column appears in DDL")
            .sql
            .as_str();
        let start = sql.find("\"breg_spgeom_").expect("spatial column starts") + 1;
        let rest = &sql[start..];
