...
async fn handler(State(state): State<TestState>, request: Request<Body>) -> Response<Body> {
    ...
    match state.mode {
        ...
        Mode::ConcealedNotFound => registered_problem(RegistryServerProblemCode::ResourceNotFound),
...
fn registered_problem(code: RegistryServerProblemCode) -> Response<Body> {
    let body = serde_json::to_vec(&json!({
        ...
        "status": code.status(),
        "detail": match code {
            RegistryServerProblemCode::ResourceNotFound => "The requested resource was not found.",
...
async fn test_client(
    ...
    maximum: u64,
) -> (RegistryServerClient, Arc<Mutex<Vec<CapturedRequest>>>) {
    ...
    let mut config = RegistryServerClientConfig::new(
    ...
    (RegistryServerClient::new(config).expect("client"), requests)
}
...
fn json_ld_options() -> ServerRecordOptions {
...
#[tokio::test]
async fn server_paths_queries_credentials_and_record_representations_are_product_exact() {
    ...
    let ordinary = client
        .get_record("companies", RECORD_ID, &ServerRecordOptions::default())
        .await
    ...
    client
        ...
        .await
        .expect("next page");
    let lookup = ServerLookupRequest::new("by-number")
        .expect("selector")
        .options(json_ld_options())
...
#[tokio::test]
async fn invalid_server_record_identifiers_are_rejected_before_authentication_or_io() {
    let provider = Arc::new(CountingToken(AtomicUsize::new(0)));
...
async fn record_header_and_media_contracts_fail_closed() {
    for (mode, call, expected) in [
        ...
        let error = match call {
            ...
            Call::List => client
                .list_records("companies", &ServerListRequest::default())
                .await
    ...
    for mode in [
        ...
        assert!(matches!(
            ...
            RegistryServerClientError::Protocol {
                failure: RegistryServerProtocolFailure::TraceContext,
                ..
            }
        ));
    }
...
async fn redirects_with_bearer_and_oversize_bodies_are_refused_without_retry() {
    ...
    assert!(matches!(error, RegistryServerClientError::Protocol { .. }));
    ...
    assert!(matches!(
        error,
        RegistryServerClientError::Transport {
