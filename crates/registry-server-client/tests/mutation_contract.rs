...
fn idempotency_key_uses_the_exact_header_grammar_and_redacts_debug() {
    for value in ["a", "request-42_!", &"k".repeat(256)] {
        let key = RegistryServerIdempotencyKey::parse(value).expect("valid key");
        assert_eq!(key.as_str(), value);
    }
    let canary = "secret-idempotency-canary-42";
    let key = RegistryServerIdempotencyKey::parse(canary).unwrap();
    ...
    for value in [
        ...
        let error = RegistryServerIdempotencyKey::parse(value).expect_err("invalid key");
...
fn mutation_debug_never_exposes_field_names_or_values() {
    let create = ServerCreateRequest::new(Map::from_iter([(
...
fn patch_builder_refuses_raw_paths_test_only_and_excessive_documents() {
    for invalid in [
        ...
        let error = ServerPatchRequest::builder()
            .replace(invalid, Value::Null)
            .expect_err("invalid API field name");
        assert_eq!(error, ServerMutationRequestError::InvalidFieldName);
        if !invalid.is_empty() {
    ...
    assert_eq!(
        ServerPatchRequest::builder()
            ...
            .build()
            .unwrap_err(),
        ServerMutationRequestError::PatchRequiresMutation
    );

    let mut builder = ServerPatchRequest::builder();
    for _ in 0..MAXIMUM_SERVER_PATCH_OPERATIONS {
        builder = builder.test("status", Value::Null).unwrap();
    }
    assert_eq!(
        builder.add("status", json!("active")).unwrap_err(),
        ServerMutationRequestError::TooManyPatchOperations
    );
}
...
fn mutation_values_reject_inexact_integers_at_every_depth() {
    ...
    let create_error = ServerCreateRequest::new(Map::from_iter([(
        "payload".into(),
        json!({"nested": [inexact.clone()]}),
    )]))
    .unwrap_err();
    assert_eq!(create_error, ServerMutationRequestError::InvalidJsonValue);

    let patch_error = ServerPatchRequest::builder()
        .replace("payload", json!({"nested": [inexact]}))
        .unwrap_err();
    assert_eq!(patch_error, ServerMutationRequestError::InvalidJsonValue);

    ServerCreateRequest::new(Map::from_iter([("payload".into(), exact)]))
...
fn create_body_enforces_field_and_encoded_body_bounds() {
    assert_eq!(
        ServerCreateRequest::new(Map::from_iter([("bad-field".into(), Value::Null)])).unwrap_err(),
        ServerMutationRequestError::InvalidFieldName
    );

    let oversized = "x".repeat(2 * 1024 * 1024);
    assert_eq!(
        ServerCreateRequest::new(Map::from_iter([("payload".into(), json!(oversized))]))
            .unwrap_err(),
        ServerMutationRequestError::BodyTooLarge
    );
}
