...
const SERVER_ETAG: &str = "\"rs-record-000000000001\"";
...
async fn test_client(responses: Vec<MockResponse>) -> TestClient {
    ...
    tokio::spawn(async move {
        ...
        .await
        .expect("serve mock Registry Server");
    });
    ...
    TestClient {
        client: RegistryServerClient::new(config).expect("client"),
        requests,
        token,
    }
}
...
fn record_body(format: ServerRecordFormat, record_identifier: &str) -> Value {
    let mut value = json!({
        "data": {
            "recordIdentifier": record_identifier,
            "revisionIdentifier": "1",
            "domainData": {"legalName": "Example Ltd"},
            "snapshot": format!("rs1_{record_identifier}")
        },
        "meta": {
    ...
    if format == ServerRecordFormat::JsonLd {
...
fn mutation_response(
    ...
    let media_type = match format {
        ServerRecordFormat::Json => "application/json",
        ServerRecordFormat::JsonLd => "application/ld+json",
    };
...
fn create_request() -> ServerCreateRequest {
    ServerCreateRequest::new(Map::from_iter([(
        "legalName".to_owned(),
        json!("Created Ltd"),
    )]))
...
fn patch_request() -> ServerPatchRequest {
    ServerPatchRequest::builder()
...
fn key(value: &str) -> RegistryServerIdempotencyKey {
    RegistryServerIdempotencyKey::parse(value).expect("valid idempotency key")
}
...
async fn metadata_selected_create_and_patch_use_the_exact_http_contract() {
    let fixture = test_client(vec![
        ...
        mutation_response(StatusCode::OK, ServerRecordFormat::JsonLd, RECORD_ID, false),
    ])
    .await;
...
async fn direct_write_success_headers_and_status_fail_closed() {
    for (case, expected) in [
        ...
        let mut response = if patch_case {
            mutation_response(StatusCode::OK, ServerRecordFormat::Json, RECORD_ID, true)
        } else {
            mutation_response(
                StatusCode::CREATED,
                ServerRecordFormat::Json,
        ...
        response = match case {
            ...
            DirectResponseCase::WeakEtag => response
                .without_header("etag")
                .with_header("etag", "W/\"rs-record-000000000001\""),
        ...
        let error = if patch_case {
            fixture
                ...
                .patch_record(
                    ...
                    Uuid::parse_str(RECORD_ID).unwrap(),
                    &RegistryServerEtag::parse(SERVER_ETAG).unwrap(),
                    &patch_request(),
                    &key("header-case"),
                    ServerRecordFormat::Json,
                )
        ...
        assert!(
            matches!(
                error,
                RegistryServerClientError::Protocol { failure, .. } if failure == expected
            ),
            "{case:?}: {error:?}"
        );
    }
...
async fn mutation_records_and_action_receipts_are_validated_against_the_selected_operation() {
    for body in [
        {
            let mut body = record_body(ServerRecordFormat::Json, RECORD_ID);
        ...
        assert!(matches!(
            ...
            RegistryServerClientError::Protocol {
                failure: RegistryServerProtocolFailure::Body,
                ..
            }
        ));
    }
...
async fn every_registered_problem_is_accepted_exactly_for_a_direct_write() {
    let responses = std::iter::once(metadata_response())
        .chain(
            RegistryServerProblemCode::ALL
                .into_iter()
                .map(problem_response),
        )
        .collect();
    ...
    assert_eq!(
        ...
        1 + RegistryServerProblemCode::ALL.len()
    );
}

fn problem_response(code: RegistryServerProblemCode) -> MockResponse {
    MockResponse::json(
...
async fn redirects_and_failures_are_never_followed_or_retried() {
    let redirect = MockResponse::json(
        StatusCode::FOUND,
        json!({
            "type": "urn:registry-server:problem:redirect",
            "title": "Redirect",
            "status": 302,
    ...
    assert!(matches!(
        error,
        RegistryServerClientError::Protocol {
            failure: RegistryServerProtocolFailure::Problem,
            ..
        }
    ));
...
async fn source_mismatch_and_invalid_bodies_are_refused_before_token_or_io() {
    ...
    assert!(errors
        .iter()
        .all(|error| matches!(error, RegistryServerClientError::InvalidRequest { .. })));
    ...
    let empty_create = ServerCreateRequest::new(Map::new()).unwrap();
    let invalid_patch = ServerPatchRequest::builder()
    ...
    for error in [
        ...
        assert!(matches!(
            error,
            RegistryServerClientError::InvalidRequest { .. }
        ));
    }
