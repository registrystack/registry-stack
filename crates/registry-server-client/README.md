# Registry Server Rust client

`registry-server-client` is the canonical bounded Rust client for Registry
Server. Its public API uses the Registry Server technical family and the
`RegistryServerClient` entry point.
...
use registry_server_client::{
    RegistryServerClient, RegistryServerClientConfig, StaticToken,
};
use url::Url;

# fn build() -> Result<RegistryServerClient, Box<dyn std::error::Error>> {
...
```rust,no_run
use registry_server_client::{ServerListRequest, ServerRecordFormat, ServerRecordOptions};

# async fn run(client: &registry_server_client::RegistryServerClient) -> Result<(), registry_server_client::RegistryServerClientError> {
...
Server uses the neutral `registry-record` DTOs for ordinary JSON and JSON-LD
...
metadata. Server-specific response metadata, ETags, operations, and lifecycle
...
one canonical lower-case W3C Trace Context v0 `traceparent`, Server's exact
response media type, and the closed Server Problem vocabulary. Registry Metadata
