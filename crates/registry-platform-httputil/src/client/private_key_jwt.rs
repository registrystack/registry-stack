...
const CLIENT_KEY_PROBE: &[u8] = b"registry-platform-httputil client key usability probe";
...
impl PrivateKeyJwt {
    ...
    pub(crate) fn with_clock(
        ...
        let audience_is_token_endpoint = config.audience.is_none();
        Ok(Self {
            http,
            audience: config
                .audience
    ...
    async fn acquire(&self, now: i64, monotonic_now: Instant) -> Result<AcquiredToken, TokenError> {
        ...
        let media_type = exact_content_type(response.headers()).map(str::to_owned);
...
mod tests {
    ...
    use super::*;
    use crate::client::{OAuthErrorCode, TokenError, TokenProvider, TransportKind};

    #[test]
    fn token_response_content_type_must_appear_exactly_once() {
        let mut headers = HeaderMap::new();
        assert_eq!(exact_content_type(&headers), None);
        headers.append(CONTENT_TYPE, "application/json".parse().expect("header"));
        assert_eq!(exact_content_type(&headers), Some("application/json"));
        headers.append(CONTENT_TYPE, "application/json".parse().expect("header"));
        assert_eq!(exact_content_type(&headers), None);
    }
    ...
    /// A fresh ES256 client key in the shape adopter tooling writes.
    ...
    ///
    /// ES256 is the case an adopter meets first: it is what adopter tooling writes
    ...
    async fn an_unusable_token_response_is_refused() {
        let cases = [
            ...
            (
                "a credential the service request cannot present",
    ...
    fn debug_output_withholds_userinfo_the_caller_put_in_the_token_endpoint() {
        let candidate = config(
            Url::parse("https://client:s3cr3t@issuer.example.org/token?secret-query=canary")
                .expect("the token endpoint parses"),
            client_key(Some(KEY_ID)),
        );
        ...
        let provider = PrivateKeyJwt::new(config(
            Url::parse("https://issuer.example.org/token?secret-query=canary")
                .expect("the token endpoint parses"),
            client_key(Some(KEY_ID)),
        ))
        .expect("a query-bearing token endpoint remains usable");
        let rendered = format!("{provider:?}");
        assert!(!rendered.contains("secret-query"), "{rendered}");
        assert!(!rendered.contains("canary"), "{rendered}");
        assert!(rendered.contains("issuer.example.org/token"), "{rendered}");
