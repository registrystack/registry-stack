//! Bearer credential acquisition for outbound service requests.
//!
...
impl BearerToken {
    ...
    /// Build a sensitive `Authorization: Bearer` value without exposing token text.
    #[must_use]
    pub fn authorization_header_value(&self) -> HeaderValue {
        let mut encoded = Zeroizing::new(String::with_capacity(7 + self.0.len()));
        encoded.push_str("Bearer ");
        encoded.push_str(&self.0);
        let mut value = HeaderValue::from_str(&encoded)
            .expect("BearerToken construction guarantees a valid header value");
...
mod tests {
    ...
    async fn a_static_provider_returns_the_configured_credential() {
        ...
        assert_eq!(authorization.to_str().unwrap(), "Bearer header-safe-token");
