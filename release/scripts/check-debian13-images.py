...
def discover_maintained_surfaces(root: Path) -> tuple[Path, ...]:
    ...
    return tuple(
        sorted(path for path in paths if not is_excluded(path) and (root / path).is_file() and not (root / path).is_symlink())
    )
...
def assignment(path: Path, line: str) -> tuple[str, str] | None:
    ...
    return (name, match.group("value")) if normalized.endswith("image") or normalized == "container" else None
...
def policy_image_name(name: str) -> bool:
    value, markers = name.casefold().replace("$", ""), {"base", "builder", "debian", "runtime"}
    return re.sub("[^a-z0-9]", "", value) == "image" or value.startswith(tuple(markers)) or bool(set(re.split(r"[_-]+", value)) & markers)
...
def logical_lines(
    lines: list[str], flags: list[bool]
...
def scan_surface(path: Path, text: str) -> list[str]:
    if len(text.encode()) > MAX_TEXT_FILE_BYTES:
        raise ImageSurfaceError(f"{path}: text file exceeds {MAX_TEXT_FILE_BYTES} bytes")
    ...
    for number, line in enumerate(lines, 1):
        ...
        consumer = is_container_consumer(line) and (code_file or markdown_code) and not comment
        reference_context = (
            not comment and (is_dockerfile(path)
            or path.suffix in {".yaml", ".yml"}
            or markdown_code
            or path.suffix in {".sh", ".bash"} or item is not None
            or consumer)
        )
        if reference_context:
            for reference in references(line):
                ...
                if "trixie" not in reference.casefold() and re.search(
                    r"debian-?13", reference, re.IGNORECASE
                ) is None:
                    failures.append(
        ...
        if item:
            ...
            dependencies = {
                found.group("name").casefold()
                for found in IMAGE_VAR_RE.finditer(value)
            }
            ...
            computed = not positional and (
                ...
                or re.search(r"\s[+%]\s|`|\$\(|\.format\(|\bf[\"']", value)
                is not None
            )
            strict = (
                path.suffix in STRICT_ASSIGNMENT_SUFFIXES
                and policy_image_name(name)
            )
            records.append(
                (number, canonical, value, dependencies, computed, strict)
            )
    ...
    for number, line, markdown_code in logical_lines(lines, markdown_flags):
        if line.lstrip().startswith(("#", "//")) or not (code_file or markdown_code) or not is_container_consumer(line):
            continue
        variables = {
...
def product_contracts(texts: dict[Path, str], failures: list[str]) -> None:
    for path in PRODUCT_DOCKERFILES:
        ...
        require(text, f"FROM {DISTROLESS_RUNTIME} AS runtime", path, "Distroless Debian 13 non-root final runtime", failures)
        require(runtime, "HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3", path, "binary healthcheck", failures)
        for forbidden in ("\nRUN ", "apt-get", "/bin/sh", "curl ", "wget "):
            if forbidden in runtime:
                failures.append(f"{path}: final Distroless runtime contains {forbidden.strip()!r}")
    for path in PRODUCT_DOCKERFILES[:3]:
        require(texts.get(path, ""), f"FROM {RUST_BUILDER} AS builder", path, "pinned Debian 13 Rust builder", failures)
    for path in PRODUCT_DOCKERFILES[3:]:
        text = texts.get(path, "")
        if not text.startswith(f"# syntax={DOCKERFILE_FRONTEND}\n"):
            failures.append(f"{path}: pinned Dockerfile frontend must be the first line")
        for needle, detail in (
            ...
            ("RUN --mount=type=bind,source=dist/image-bin,target=/workspace/image-bin", "ephemeral release input mount"),
            ('find /workspace/runtime-root -exec touch -h --date="@${SOURCE_DATE_EPOCH}" {} +', "normalized release filesystem metadata"),
        ):
            require(text, needle, path, detail, failures)
    for path in RELAY_DOCKERFILES:
        text = texts.get(path, "")
        require(text, "/usr/local/bin/registry-relay-rhai-worker", path, "Relay worker binary", failures)
        require(runtime_stage(text), 'ENTRYPOINT ["/usr/local/bin/registry-relay"]', path, "absolute Relay entrypoint", failures)
    require(texts.get(PRODUCT_DOCKERFILES[2], ""), 'ARG REGISTRY_NOTARY_FEATURES="registry-notary-cel,pkcs11"', PRODUCT_DOCKERFILES[2], "PKCS#11-enabled product build", failures)
    for path in NOTARY_DOCKERFILES:
        text = texts.get(path, "")
        for source, needle, detail in (
            (text, "registry-notary-cel-worker", "Notary CEL worker binary"),
            (runtime_stage(text), 'ENTRYPOINT ["/usr/local/bin/registry-notary"]', "absolute Notary entrypoint"),
            (text, "chown -R 65532:65532", "numeric nonroot-owned Notary runtime directories"),
            (runtime_stage(text), "WORKDIR /var/lib/registry-notary", "Notary working directory"),
        ):
            require(source, needle, path, detail, failures)
        if re.search(r"^\s*(?:COPY|ADD)\b[^\n]*(?:\.so\b|pkcs11[^/\s]*module)", text, re.IGNORECASE | re.MULTILINE):
            failures.append(f"{path}: vendor PKCS#11 modules must remain external read-only mounts")
    ...
    for source, needle, path, detail in (
        (tutorial, 'LINUX_TARGET="$REPO_ROOT/target/registryctl-tutorial-linux-amd64"', TUTORIAL_SCRIPT, "registryctl tutorial linux target path"),
        (tutorial, 'CARGO_HOME_DIR="$REPO_ROOT/target/registryctl-tutorial-cargo-home"', TUTORIAL_SCRIPT, "registryctl tutorial Cargo home path"),
        (cache, "hashFiles('docs/site/scripts/check-registryctl-tutorials.sh')", CI_WORKFLOW, "registryctl tutorial cache builder identity"),
        (cache, "target/registryctl-tutorial-linux-amd64", CI_WORKFLOW, "registryctl tutorial linux target cache path"),
        (cache, "target/registryctl-tutorial-cargo-home", CI_WORKFLOW, "registryctl tutorial Cargo home cache path"),
    ):
        require(source, needle, path, detail, failures)
    if "restore-keys:" in cache:
        failures.append(f"{CI_WORKFLOW}: registryctl tutorial cache must not restore from pre-builder-identity keys")
...
def check_repository(root: Path = ROOT) -> list[str]:
    ...
    for path in dockerfiles:
        ...
        for base, stage in bases:
            if base.casefold() not in stages | {"scratch"} and not DIGEST_RE.search(base):
                failures.append(f"{path}: upstream base is not pinned by immutable digest: {base}")
            if stage:
