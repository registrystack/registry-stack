...
- Correct duplicate Notary claim ids, invalid default disclosure modes, and
  claim rules that refer to undeclared consultation outputs before startup.
...
- Treat each product's `config_trust.antirollback_state_path` as durable state.
  ...
  boot.
- Remove retired Notary source-adapter, connector, and governed runtime target
  configuration. Notary now evaluates self-attested claims or compiler-pinned
  Relay consultation outputs and does not connect to registry sources directly.
...
- Replace legacy `registryctl.yaml` authoring files with the
  `registry-stack.yaml` project contract. A source-free Notary-only service has
  no consultation. A combined deployment defines a product-neutral integration
  and binds each Notary service consultation to that integration.
