...
jq -e --arg tag "${tag}" '
  ($tag | capture("^v(?<major>[0-9]+)\\.(?<minor>[0-9]+)\\.(?<patch>[0-9]+)$") |
    {major: (.major | tonumber), minor: (.minor | tonumber),
     patch: (.patch | tonumber)}) as $version |
  (if ($version.major > 0 or $version.minor > 30 or
       ($version.minor == 30 and $version.patch >= 1))
   then ["breg", "casework", "discovery", "evidence", "relay"]
   elif $version.minor >= 30 then ["breg", "casework", "discovery", "evidence", "mint", "relay"]
...
Mint is retired from `v0.30.1`; historical releases retain their original roster.
...
`v0.30.0`. Mint reports are excluded from `v0.30.1` onward; `v0.19.x` and
