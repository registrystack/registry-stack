...
jq -e --arg tag "${tag}" '
  ...
  (if ($version.major > 0 or $version.minor >= 31)
...
Mint is retired from `v0.31.0`; historical releases retain their original roster.
...
`v0.30.0`. Mint reports are excluded from `v0.31.0` onward; `v0.19.x` and
