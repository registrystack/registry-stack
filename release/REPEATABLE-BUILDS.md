...
cmake package needed to build `aws-lc-fips-sys`, the FIPS cryptography backend
the runtimes link, from a dated Debian snapshot inside the pinned builder
container. The source commit therefore fixes both the builder image and the
additional build packages instead of consulting Debian's mutable package
indexes.
