...
suite('Registry Stack extension', () => {
  suiteTeardown(() => {
    fs.rmSync(path.resolve(__dirname, '../../dist/registryctl-path'), { force: true });
  });

  test('uses the installed registryctl for every trusted Registry Stack workspace folder', async () => {
